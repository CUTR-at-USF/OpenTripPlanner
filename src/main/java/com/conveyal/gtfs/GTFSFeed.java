...
/**
 * All entities must be from a single feed namespace. Composed of several
 * GTFSTables.
 */
public class GTFSFeed {
	...
	DB db = DBMaker.newTempFileDB().transactionDisable().asyncWriteEnable()
			.compressionEnable().make(); // TODO db.close();

	public String feedId = null;

	/*
	 * Some of these should be multimaps since they don't have an obvious unique
	 * key.
	 */
	...
	public final ConcurrentNavigableMap<Tuple2, StopTime> stop_times = db
			.getTreeMap("stop_times");

	/*
	 * A fare is a fare_attribute and all fare_rules that reference that
	 * fare_attribute.
	 */
	public final Map<String, Fare> fares = Maps.newHashMap();

	/*
	 * A service is a calendar entry and all calendar_dates that modify that
	 * calendar entry.
	 */
	public final Map<String, Service> services = Maps.newHashMap();

	/*
	 * A place to accumulate errors while the feed is loaded. Tolerate as many
	 * errors as possible and keep on loading.
	 */
	public List<GTFSError> errors = Lists.newArrayList();

	/**
	 * The order in which we load the tables is important for two reasons. 1. We
	 * must load feed_info first so we know the feed ID before loading any other
	 * entities. This could be relaxed by having entities point to the feed
	 * object rather than its ID String. 2. Referenced entities must be loaded
	 * before any entities that reference them. This is because we check
	 * referential integrity while the files are being loaded. This is done on
	 * the fly during loading because it allows us to associate a line number
	 * with errors in objects that don't have any other clear identifier.
	 *
	 * Interestingly, all references are resolvable when tables are loaded in
	 * alphabetical order.
	 */
	private void loadFromFile(ZipFile zip) throws Exception {
		new FeedInfo.Loader(this).loadTable(zip);
		// maybe we should just point to the feed object itself instead of its
		// ID, and null out its stoptimes map after loading
		if (feedId == null) {
			LOG.info("Feed ID is undefined."); // TODO log an error, ideally
												// feeds should include a feedID
		}
		...
		new StopTime.Loader(this).loadTable(zip); // comment out this line for
													// quick testing using NL
													// feed
	...
	public void findPatterns() {
		// A map from a list of stop IDs (the pattern) to a list of trip IDs
		...
		for (String trip_id : trips.keySet()) {
			...
			Map<Fun.Tuple2, StopTime> tripStopTimes = stop_times.subMap(
					Fun.t2(trip_id, null), Fun.t2(trip_id, Fun.HI));
			List<String> stops = Lists.newArrayList();
			// In-order traversal of StopTimes within this trip. The 2-tuple
			// keys determine ordering.
			for (StopTime stopTime : tripStopTimes.values()) {
				stops.add(stopTime.stop_id);
			}
			// Fetch or create the tripId list for this stop pattern, then add
			// the current trip to that list.
