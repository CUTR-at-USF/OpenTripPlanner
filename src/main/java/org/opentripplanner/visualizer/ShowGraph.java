...
/**
 * Processing applet to show a map of the graph. The user can: - Use mouse wheel
 * to zoom (or right drag, or ctrl-drag) - Left drag to pan around the map -
...
public class ShowGraph extends PApplet implements MouseWheelListener {
	...
	// how many edges to draw before checking whether we need to move on to the
	// next frame
	private final int BLOCK_SIZE = 1000;
	// how many edges to skip over (to ensure a sampling of edges throughout the
	// visible area)
	private final long DECIMATE = 40;
	// 800 instead of 1000 msec, leaving 20% of the time for work other than
	// drawing.
	...
	// these queues are filled by a search in another thread, so must be
	// threadsafe
	...
	private static final DecimalFormat latFormatter = new DecimalFormat(
			"00.0000°N ; 00.0000°S");

	private static final DecimalFormat lonFormatter = new DecimalFormat(
			"000.0000°E ; 000.0000°W");

	private final SimpleDateFormat shortDateFormat = new SimpleDateFormat(
			"HH:mm:ss z");
	...
	class SPTNode {
		...
		public void drawRecursive(int height,
				HashMap<Vertex, Integer> vertexStatesEncountered) {
			colorMode(HSB);

			// get the number of states we've already drawn from this vertex
			Integer vertexHeight = vertexStatesEncountered.get(this.state
					.getVertex());
			if (vertexHeight == null) {
				vertexHeight = 0;
			}

			// if it's larger than the 'height' of the state we're about to
			// draw, bump the state's visual height
			// up to the number of states it has to climb over
			if (vertexHeight > height) {
				height = vertexHeight;
			}

			// increment the counter of the number of times we've encountered
			// this vertex
			vertexStatesEncountered.put(this.state.getVertex(),
					vertexHeight + 1);
			...
			if (state.getBackEdge() != null) {
				...
				strokeWeight((float) (sptThickness * Math.pow(weight,
						sptFlattening)));
		...
		private int colorRamp(int aa) {
			...
			int hue = hueIndex * (HUELEN / NHUES); // convert that to a hue
													// value
	...
	/*
	 * Constructor. Call processing constructor, and register the listener to
	 * notify when the user selects vertices.
	 */
	...
	public void setup() {
		...
		/*
		 * Set model bounds to encompass all vertices in the index, and then
		 * some
		 */
	...
	/*
	 * Zoom in/out. Translate the viewing window such that the place under the
	 * mouse pointer is a fixed point. If p is null, zoom around the center of
	 * the viewport.
	 */
	void zoom(double f, Point p) {
		...
		if (p != null) {
			// Note: Graphics Y coordinates increase down the screen, hence the
			// opposite signs.
	...
	/*
	 * Iterate through all vertices and their (outgoing) edges. If they are of
	 * 'interesting' types, add them to the corresponding spatial index.
	 */
	...
	private synchronized void findVisibleElements() {
		...
		for (Edge de : (Iterable<Edge>) edgeIndex.query(modelBounds)) {
			...
			} else if (de instanceof PathwayEdge
	...
	private int drawEdge(Edge e) {
		...
		for (int i = 0; i < coords.length; i++)
			vertex((float) toScreenX(coords[i].x),
					(float) toScreenY(coords[i].y));
		endShape();
		return coords.length; // should be used to count segments, not edges
								// drawn
	}
	...
	private void drawGraphPath(GraphPath gp) {
		...
		for (State s : gp.states) {
			...
			if (e instanceof StreetEdge) {
				StreetTraversalPermission stp = ((StreetEdge) e)
						.getPermission();
	...
	private void labelState(State s, String str) {
		...
		drawVertex(v, 8);
		str += " "
				+ shortDateFormat.format(new Date(s.getTimeSeconds() * 1000));
		str += " [" + (int) s.getWeight() + "]";
	...
	private void drawVertices() {
		...
		boolean closeEnough = (modelBounds.getHeight() * METERS_PER_DEGREE_LAT
				/ this.width < 4);
		/* Draw selected visible vertices */
		fill(60, 60, 200);
		for (Vertex v : visibleVertices) {
			if (drawTransitStopVertices && closeEnough
					&& v instanceof TransitStationStop) {
				drawVertex(v, 5);
			}
	...
	private void drawHighlighted() {
		...
		if (drawHighlighted && highlightedEdges != null) {
			try {
				...
				// The edge list was cleared or added to while it was being
				// drawn, no harm done.
			}
		}
	...
	private double toScreenY(double y) {
		return map(y, modelBounds.getMinY(), modelBounds.getMaxY(),
				getSize().height, 0);
	}

	private double toScreenX(double x) {
		return map(x, modelBounds.getMinX(), modelBounds.getMaxX(), 0,
				getSize().width);
	}
	...
	public void mouseClicked() {
		...
		Envelope env = new Envelope(toModelX(screenEnv.getMinX()),
				toModelX(screenEnv.getMaxX()), toModelY(screenEnv.getMinY()),
	...
	private double toModelY(double y) {
		return map(y, 0, getSize().height, modelBounds.getMaxY(),
				modelBounds.getMinY());
	}

	private double toModelX(double x) {
		return map(x, 0, getSize().width, modelBounds.getMinX(),
				modelBounds.getMaxX());
	}

	/**
	 * A version of ellipse that takes double args, because apparently Java is
	 * too stupid to downgrade automatically.
	 * 
	 * @param d
	 * @param e
	 * @param f
	...
	/**
	 * Set the Vertex selector to newSelector, and store the old selector on the
	 * stack of selectors
	 * 
	 * @param newSelector
	 */
