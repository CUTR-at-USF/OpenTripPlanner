...
/**
 * DisplayVertex holds a vertex, but has a toString value that's a little more
 * useful.
 */
class DisplayVertex {
...
/**
 * This is a ListModel that holds Edges. It gets its edges from a
 * PatternBoard/PatternAlight, hence the iterable.
 */
...
/**
 * A simple visualizer for graphs. It shows (using ShowGraph) a map of the
 * graph, intersections and TransitStops only, and allows a user to select
 * stops, examine incoming and outgoing edges, and examine trip patterns. It's
 * meant mainly for debugging, so it's totally OK if it develops (say) a bunch
 * of weird buttons designed to debug specific cases.
 */
public class GraphVisualizer extends JFrame implements VertexSelectionListener {

	private final class ComparePathStatesClickListener implements
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			stateListModel.addElement("elapsedTime:"
					+ st.getElapsedTimeSeconds());
	...
	private final class OnPopupMenuClickListener implements ActionListener {
		private final class DiffListCellRenderer extends
		...
		private int[] diffPaths() {
			...
			for (int i = 0; i < minlen; i++) {
				...
				Vertex v2 = secondComparePath.states.get(l2 - i - 1)
						.getVertex();
				if (!v1.equals(v2)) {
					convergence = i - 1;
					break;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (diff[0] >= 0) {
				firstComparePathStates
						.setCellRenderer(new DiffListCellRenderer(diverge,
								firstComparePath.states.size() - converge - 1));
				secondComparePathStates
						.setCellRenderer(new DiffListCellRenderer(diverge,
								secondComparePath.states.size() - converge - 1));
			}
		}
	}
	...
	class PathPrinter {
		...
		public String toString() {
			SimpleDateFormat shortDateFormat = new SimpleDateFormat(
					"HH:mm:ss z");
			String startTime = shortDateFormat.format(new Date(gp
					.getStartTime() * 1000));
			String endTime = shortDateFormat.format(new Date(
					gp.getEndTime() * 1000));
			return "Path (" + startTime + "-" + endTime + ") weight:"
					+ gp.getWeight() + " dur:" + (gp.getDuration() / 60.0)
					+ " walk:" + gp.getWalkDistance() + " nTrips:"
					+ gp.getTrips().size();
		}
	}
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(GraphVisualizer.class);
	...
	private Container makeDiffTab() {
		...
		firstComparePathStates
				.addListSelectionListener(new ComparePathStatesClickListener(
						firstStateData));
		...
		// A button that executes the 'dominates' function between the two
		// states
		...
		dominateButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				System.out.println("s1 dominates s2:"
						+ MultiShortestPathTree.dominates(s1, s2));
				System.out.println("s2 dominates s1:"
						+ MultiShortestPathTree.dominates(s2, s1));
			}
		});
		pane.add(dominateButton);

		// A button that executes the 'traverse' function leading to the last
		// clicked state
	...
	private Container makeMainTab() {
		...
		sptServiceFactory.setTraverseVisitor(new VisualTraverseVisitor(
				showGraph));
	...
	private JComponent makePrefsPanel() {
		...
		JLabel softWalkLimitPenaltyLabel = new JLabel(
				"Soft walk-limiting penalty:");
		pane.add(softWalkLimitPenaltyLabel);
		softWalkPenalty = new JTextField("60.0");
		pane.add(softWalkPenalty);

		// row: soft walk-limit overage
		JLabel softWalkLimitOverageLabel = new JLabel(
				"Soft walk-limiting overage:");
		...
		ItemListener onChangeVizPrefs = new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				showGraph
						.setShowMultistateVertices(showMultistateVerticesCheckbox
								.isSelected());
				showGraph.setShowHightlights(showHighlightedCheckbox
						.isSelected());
		...
		showMultistateVerticesCheckbox = new JCheckBox(
				"show multistate vertices");
	...
	protected void setLongDistanceMode(boolean selected) {
		if (selected) {
			this.pathservice = new LongDistancePathService(graphService,
					sptServiceFactory);
		} else {
			this.pathservice = new ParetoPathService(graphService,
					sptServiceFactory);
	...
	private void initRightPanel(Container pane) {
		...
		// when you select a path component state, it prints the backedge's
		// metadata
		...
		annotationMatches.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				@SuppressWarnings("unchecked")
				JList<GraphBuilderAnnotation> theList = (JList<GraphBuilderAnnotation>) e
						.getSource();
	...
	private void initControlButtons() {
		...
		zoomToNodeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String nodeName = (String) JOptionPane.showInputDialog(frame,
						"Node id", JOptionPane.PLAIN_MESSAGE);
		...
		zoomToLocationButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String result = JOptionPane
						.showInputDialog("Enter the location (lat lon)");
		...
		findEdgeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String edgeName = (String) JOptionPane.showInputDialog(frame,
						"Edge name like", JOptionPane.PLAIN_MESSAGE);
				for (Vertex gv : getGraph().getVertices()) {
					for (Edge edge : gv.getOutgoing()) {
						if (edge.getName() != null
								&& edge.getName().contains(edgeName)) {
		...
		findEdgeByIdButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String edgeIdStr = (String) JOptionPane.showInputDialog(frame,
						"Edge ID", JOptionPane.PLAIN_MESSAGE);
		...
		snapButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String locString = (String) JOptionPane.showInputDialog(frame,
						"Location string", "");
				GenericLocation loc = GenericLocation
						.fromOldStyleString(locString);
	...
	private void reactToEdgeSelection(Edge selected, boolean outgoing) {
		...
		if (!vertices.contains(v)) {
			...
			nearbyVertices.setModel(nearbyModel); // this should just be an
													// event, but for
													// some reason, JList
													// doesn't implement
													// the right event.
		...
		if (selected instanceof StreetEdge) {
			...
			// getMetadata(((StreetEdge)
			// selected).getElevationProfileSegment());
		}
	...
	private void initVertexInfoSubpanel() {
		...
		vertexDataPanel.setLayout(new BoxLayout(vertexDataPanel,
				BoxLayout.PAGE_AXIS));
		...
		nearbyVertices.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				...
				DisplayVertex selected = (DisplayVertex) nearbyVertices
						.getSelectedValue();
				if (selected != null) {
					Vertex nowSelected = selected.vertex;
					showGraph.highlightVertex(nowSelected);
					outgoingEdges.setModel(new EdgeListModel(nowSelected
							.getOutgoing()));
					incomingEdges.setModel(new EdgeListModel(nowSelected
							.getIncoming()));
				}
			}
		});
		...
		// when a different edge is selected, change up the pattern pane and
		...
		pathsList.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				if (SwingUtilities.isRightMouseButton(e)) {
					@SuppressWarnings("unchecked")
					JList<PathPrinter> list = (JList<PathPrinter>) e
							.getSource();
	...
	private void initRoutingSubpanel() {
		...
		// option: don't use graphical callback. useful for doing a quick
		// profile
	...
	protected void checkGraph() {
		...
		System.out.println("After investigation, visited "
				+ seenVertices.size() + " of " + allVertices.size());
	...
	protected void route(String from, String to) {
		...
		options.setWalkBoardCost(Integer.parseInt(boardingPenaltyField
				.getText()) * 60); // override low 2-4 minute values
		// TODO LG Add ui element for bike board cost (for now bike = 2 * walk)
		options.setBikeBoardCost(Integer.parseInt(boardingPenaltyField
				.getText()) * 60 * 2);
		...
		options.softWalkOverageRate = (Float
				.parseFloat(this.softWalkOverageRate.getText()));
		...
		// set up a visitor to the path service so we can get the SPT as it's
		// generated
	...
	protected void findAnnotation() {
		Set<Class<? extends GraphBuilderAnnotation>> gbaClasses = Sets
				.newHashSet();
		...
		System.out.println("Found " + annotationMatchesModel.getSize()
				+ " annotations of type " + variety);
