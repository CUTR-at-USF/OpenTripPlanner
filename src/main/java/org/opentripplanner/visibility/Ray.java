...
class Ray {
	...
	LineSegment intersection(LineSegment line_segment_temp, double epsilon) {
		...
		LineSegment seg_approx = new LineSegment(base_point,
				base_point.plus(new VLPoint(R * Math.cos(bearing.get()), R
						* Math.sin(bearing.get()))));
		LineSegment intersect_seg = line_segment_temp.intersection(seg_approx,
				epsilon);
		// Make sure point closer to ray_temp's base_point is listed first.
		if (intersect_seg.size() == 2
				&& intersect_seg.first().distance(base_point()) > intersect_seg
						.second().distance(base_point())) {
