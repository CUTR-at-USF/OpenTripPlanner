...
class Ray {
    ...
    public double distance(VLPoint point_temp) {
        return point_temp.distance(point_temp.projection_onto(this));
    }
    ...
    public Ray(VLPoint base_point_temp, VLPoint bearing_point) {
        ...
        bearing = new Angle(bearing_point.y - base_point_temp.y, bearing_point.x
                - base_point_temp.x);
    }
    ...
    public boolean equals(Object o) {
        ...
        Ray ray2 = (Ray) o;
        if (base_point().equals(ray2.base_point) && bearing().equals(ray2.bearing))
            return true;
        else
            return false;
    }

    LineSegment intersection(LineSegment line_segment_temp, double epsilon) {
        ...
        LineSegment seg_approx = new LineSegment(base_point, base_point.plus(new VLPoint(R
                * Math.cos(bearing.get()), R * Math.sin(bearing.get()))));
        ...
        if (intersect_seg.size() == 2
                && intersect_seg.first().distance(base_point()) > intersect_seg.second().distance(
                        base_point())) {
