...
class PolarPoint extends VLPoint implements Comparable<VLPoint> {
	...
	public PolarPoint(VLPoint polar_origin_temp, VLPoint point_temp,
		...
		if (polar_origin.distance(point_temp) <= epsilon) {
			...
			bearing = new Angle(point_temp.y - polar_origin_temp.y,
	...
	public void set_polar_origin(VLPoint polar_origin_temp) {
		PolarPoint newPoint = new PolarPoint(polar_origin_temp, new VLPoint(x,
				y));
	...
	public void set_x(double x_temp) {
		PolarPoint newPoint = new PolarPoint(polar_origin, new VLPoint(x_temp,
				y));
		setFromPolarPoint(newPoint);
	}

	public void set_y(double y_temp) {
		PolarPoint newPoint = new PolarPoint(polar_origin, new VLPoint(x,
				y_temp));
		setFromPolarPoint(newPoint);
	}
	...
	public boolean equals(Object o) {
		...
		return polar_origin.equals(polar_point2.polar_origin)
				&& range == polar_point2.range
	...
	public String toString() {
		return "PolarPoint(" + bearing + "  " + range + ") of "
				+ super.toString();
	}
