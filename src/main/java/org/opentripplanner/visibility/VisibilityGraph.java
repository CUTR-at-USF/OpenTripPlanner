...
public class VisibilityGraph {
	...
	public VisibilityGraph(Environment environment, double epsilon,
		...
		for (int k1 = 0; k1 < n; k1++) {
			...
			if (origins != null && !origins.contains(point1))
				continue;
			VLPolygon polygon_temp = new VisibilityPolygon(point1, environment,
					epsilon);
			for (int k2 = 0; k2 < n; k2++) {
				if (k1 == k2)
					...
					if (origins == null || origins.contains(point2)) {
						adjacency_matrix[k1][k2] = adjacency_matrix[k2][k1] = point2
								.in(polygon_temp, epsilon);
					}
				}
			}
		}
	...
	public VisibilityGraph(ArrayList<VLPoint> points, Environment environment,
			double epsilon) {
		vertex_counts = new ArrayList<Integer>(environment.h());
		n = points.size();
		...
		for (int k1 = 0; k1 < n; k1++) {
			polygontemp = new VisibilityPolygon(points.get(k1), environment,
					epsilon);
			for (int k2 = 0; k2 < n; k2++) {
				if (k1 == k2)
					adjacency_matrix[k1][k1] = true;
				else
					adjacency_matrix[k1][k2] = adjacency_matrix[k2][k1] = points
							.get(k2).in(polygontemp, epsilon);
			}
		}
	}

	public boolean get(int polygon1, int vertex1, int polygon2, int vertex2) {
		return adjacency_matrix[get_vertex_index(polygon1, vertex1)][get_vertex_index(
				polygon2, vertex2)];
	}
