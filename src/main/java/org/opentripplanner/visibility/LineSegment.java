...
public class LineSegment {
	...
	/*
	 * assignment operator -- need to audit what's going on with this -DMT
	 * LineSegment& operator = (const LineSegment& line_segment_temp) { //Makes
	 * sure not to delete dynamic vars before they're copied.
	 * if(this==&line_segment_temp) return *this; delete [] endpoints;
	 * switch(line_segment_temp.size_){ case 0: endpoints = null; size_ = 0;
	 * break; case 1: endpoints = new Point[1]; endpoints[0] =
	 * line_segment_temp.endpoints[0]; size_ = 1; break; case 2: endpoints = new
	 * Point[2]; endpoints[0] = line_segment_temp.endpoints[0]; endpoints[1] =
	...
	public boolean equals(Object o) {
		...
		if (size() != line_segment2.size() || size() == 0
				|| line_segment2.size() == 0)
			return false;
		else
			return (first().equals(line_segment2.first()) && second().equals(
					line_segment2.second()));
	}
	...
	boolean equivalent(LineSegment line_segment2, double epsilon) {
		if (size() != line_segment2.size() || size() == 0
			...
			return (first().distance(line_segment2.first()) <= epsilon && second()
					.distance(line_segment2.second()) <= epsilon)
					|| (first().distance(line_segment2.second()) <= epsilon && second()
							.distance(line_segment2.first()) <= epsilon);
	}
	...
	double boundary_distance(VLPolygon polygon) {
		...
		if (polygon.n() > 1)
			for (int i = 0; i < polygon.n(); i++) {
				double d = distance(new LineSegment(polygon.get(i),
						polygon.get(i + 1)));
				if (running_min > d)
					running_min = d;
			}
	...
	LineSegment intersection(LineSegment line_segment2, double epsilon) {
		...
		if (intersect_proper(line_segment2, epsilon)) {
			// Use formula from O'Rourke's "Computational Geometry in C", p.
			// 221.
			// Note D=0 iff the line segments are parallel.
			double D = a.x * (d.y - c.y) + b.x * (c.y - d.y) + d.x
					* (b.y - a.y) + c.x * (a.y - b.y);
			double s = (a.x * (d.y - c.y) + c.x * (a.y - d.y) + d.x
					* (c.y - a.y))
					/ D;
