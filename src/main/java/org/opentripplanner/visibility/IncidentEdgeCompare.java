...
/**
 * This is *reversed* from the original since Java PQs are min-heaps while STL
 * PQs are max-heaps
 * 
 */
class IncidentEdgeCompare implements Comparator<PolarEdge> {
	...
	public IncidentEdgeCompare(VLPoint observer,
			PolarPointWithEdgeInfo current_vertex, double epsilon_temp) {
	...
	public int compare(PolarEdge e1, PolarEdge e2) {
		PolarPoint k1, k2;
		LineSegment xing1 = new Ray(observer_pointer,
				current_vertex_pointer.bearing).intersection(new LineSegment(
				e1.first, e1.second), epsilon);

		LineSegment xing2 = new Ray(observer_pointer,
				current_vertex_pointer.bearing).intersection(new LineSegment(
				e2.first, e2.second), epsilon);
