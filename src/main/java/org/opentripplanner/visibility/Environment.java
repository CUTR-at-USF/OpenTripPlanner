...
/**
 * \brief environment represented by simple polygonal outer boundary with simple
 * polygonal holes
 * 
 * \remarks For methods to work correctly, the outer boundary vertices must be
 * listed ccw and the hole vertices cw
 */
public class Environment {
	...
	boolean is_in_standard_form() {
		if (outer_boundary.is_in_standard_form() == false
				|| outer_boundary.area() < 0)
			return false;
		for (int i = 0; i < holes.size(); i++)
			if (holes.get(i).is_in_standard_form() == false
					|| holes.get(i).area() > 0)
				return false;
		return true;
	}
	...
	public boolean is_valid(double epsilon) {
		...
		if (!outer_boundary.is_simple(epsilon)) {
			 ...
			 * "The outer boundary is not simple." << "\x1b[0m" << std::endl;
			 */
			return false;
		}
		for (int i = 0; i < h(); i++)
			if (!holes.get(i).is_simple(epsilon)) {
				/*
				 * std::cerr << std::endl << "\x1b[31m" << "Hole " << i <<
				 * " is not simple." << "\x1b[0m" << std::endl;
				 */
				return false;
			}
		...
		for (int i = 0; i < h(); i++) {
			...
			for (int j = 0; j < holes.get(i).n(); j++) {
				if (!holes.get(i).get(j).in(outer_boundary, epsilon)) {
					/*
					 * std::cerr << std::endl << "\x1b[31m" << "Vertex " << j <<
					 * " of hole " << i << " is not within the outer boundary."
					 * << "\x1b[0m" << std::endl;
					 */
					return false;
				}
				...
				for (int k = 0; k < h(); k++)
					if (i != k && holes.get(i).get(j).in(holes.get(k), epsilon)) {
						/*
						 * std::cerr << std::endl << "\x1b[31m" << "Vertex " <<
						 * j << " of hole " << i << " is in hole " << k << "."
						 * << "\x1b[0m" << std::endl;
						 */
						return false;
					}
		...
		if (outer_boundary.area() <= 0) {
			/*
			 * std::cerr << std::endl << "\x1b[31m" <<
			 * "The outer boundary vertices are not listed ccw." << "\x1b[0m" <<
			 * std::endl;
			 */
			return false;
		}
	...
	ArrayList<VLPoint> random_points(int count, double epsilon) {
		...
		VLPoint pt_temp = new VLPoint(Util.uniform_random_sample(
				bounding_box.x_min, bounding_box.x_max),
				Util.uniform_random_sample(bounding_box.y_min,
						bounding_box.y_max));
		while (pts_in_environment.size() < count) {
			while (!pt_temp.in(this, epsilon)) {
				pt_temp.set_x(Util.uniform_random_sample(bounding_box.x_min,
						bounding_box.x_max));
				pt_temp.set_y(Util.uniform_random_sample(bounding_box.y_min,
						bounding_box.y_max));
			}
	...
	void update_flattened_index_key() {
		...
		for (int i = 0; i <= h(); i++) {
			for (int j = 0; j < get(i).n(); j++) {
				pair<Integer, Integer> pair_temp = new pair<Integer, Integer>(
						i, j);
