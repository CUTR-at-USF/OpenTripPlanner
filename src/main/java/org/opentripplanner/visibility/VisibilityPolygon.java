...
public class VisibilityPolygon extends VLPolygon {
	private static Logger log = LoggerFactory
			.getLogger(VisibilityPolygon.class);
	VLPoint observer;

	public boolean is_spike(VLPoint observer, VLPoint point1, VLPoint point2,
			VLPoint point3, double epsilon) {

		return (
		// Make sure observer not colocated with any of the points.
		observer.distance(point1) > epsilon
				&& observer.distance(point2) > epsilon
				...
				&& ((observer.distance(point2) >= observer.distance(point1) && observer
						...
						.distance(point2) <= observer.distance(point1) && observer
						.distance(point2) <= observer.distance(point3)))
	...
	public void chop_spikes_at_back(VLPoint observer, double epsilon) {
		...
		while (vertices.size() >= 3
				&& is_spike(observer, vertices.get(vertices.size() - 3),
						...
						vertices.get(vertices.size() - 1), epsilon)) {
	...
	void chop_spikes_at_wrap_around(VLPoint observer, double epsilon) {
		...
		while (vertices.size() >= 3
				&& is_spike(observer, vertices.get(vertices.size() - 2),
						vertices.get(vertices.size() - 1), vertices.get(0),
						epsilon)) {
	...
	public VisibilityPolygon(VLPoint observer, Environment environment_temp,
			double epsilon) {
		...
		// The visibility polygon cannot have more vertices than the
		// environment.
		...
		for (int i = 0; i <= environment_temp.h(); i++) {
			...
			for (int j = 0; j < polygon.n(); j++) {
				...
				if (observer.in_relative_interior_of(new LineSegment(ppoint1,
						ppoint2), epsilon)) {
				...
				// Else if the observer is on first vertex of edge.
					...
					right_wall_bearing = ppoint2.bearing.clone();
					elp.add(new PolarEdge(new PolarPoint(observer, observer),
							ppoint2));
					continue;
				}
				// Else if the observer is on second vertex of edge.
					...
					left_wall_bearing = ppoint1.bearing.clone();
					elp.add(new PolarEdge(ppoint1, new PolarPoint(observer,
							observer)));
					continue;
				}
				...
				// If edge not horizontal (w/in epsilon).
					...
					if (0 < t && t < 1
						...
						if (ppoint1.y > observer.y)
							continue;
						// Otherwise split the edge, making sure angles are
						// assigned
						// correctly on each side of the split point.
						split_bottom = new PolarPoint(observer, new VLPoint(t
								* ppoint1.x + (1 - t) * ppoint2.x, observer.y));
						split_top = new PolarPoint(observer, new VLPoint(t
								* ppoint1.x + (1 - t) * ppoint2.x, observer.y));
		...
		// Priority queue of edges, where lower (first) priority indicates
		// closer
		// range to observer along current ray (of ray sweep).
		IncidentEdgeCompare my_iec = new IncidentEdgeCompare(observer,
				current_vertex, epsilon);
		PriorityQueue<PolarEdge> q2 = new PriorityQueue<PolarEdge>(elp.size(),
				my_iec);
		...
		while (!q1.isEmpty()) {
			...
			if (current_vertex.incident_edge.equals(active_edge)
				...
				while (!q2.isEmpty()) {
					...
					if ((current_vertex.bearing.get() <= e.second.bearing.get())
							// For robustness.
							&& new Ray(observer, current_vertex.bearing)
									.distance(e.second) >= epsilon
					/*
					 * was && std::min( distance(Ray(observer,
					 * current_vertex.bearing), e.second),
					 * distance(Ray(observer, e.second.bearing), current_vertex)
					 * ) >= epsilon
					 */
					) {
						// Find intersection point k of ray (through
						// current_vertex) with edge e.
						xing = new Ray(observer, current_vertex.bearing)
								.intersection(
										new LineSegment(e.first, e.second),
										epsilon);
			...
			if (current_vertex.is_first) {
				...
				xing = new Ray(observer, current_vertex.bearing).intersection(
						new LineSegment(active_edge.first, active_edge.second),
						epsilon);
				if (xing.size() == 0
				...
				// Insert e into q2 as long as it doesn't contain the
				// observer.
				if (observer.distance(e.first) > epsilon
						&& observer.distance(e.second) > epsilon) {
