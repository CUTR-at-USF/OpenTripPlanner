...
public class GtfsLibrary {
	...
	public static final char ID_SEPARATOR = ':'; // note this is different than
													// what OBA GTFS uses to
													// match our 1.0 API
	...
	public static GtfsContext readGtfs(File path, String defaultAgencyId)
			throws IOException {
	...
	/*
	 * Using in index since we can't modify OBA libs and the colon in the
	 * expected separator in the 1.0 API.
	 */
	public static AgencyAndId convertIdFromString(String value) {
		int index = value.indexOf(ID_SEPARATOR);
		if (index == -1) {
			throw new IllegalArgumentException("invalid agency-and-id: "
					+ value);
		} else {
	...
	/**
	 * @return the route's short name, or the long name if the short name is
	 *         null.
	 */
	...
	public static TraverseMode getTraverseMode(Route route) {
		...
		/*
		 * TPEG Extension
		 * https://groups.google.com/d/msg/gtfs-changes/keT5rTPS7Y0/71uMz2l6ke0J
		 */
		if (routeType >= 100 && routeType < 200) { // Railway Service
		...
		} else if (routeType >= 300 && routeType < 500) { // Suburban Railway
															// Service and Urban
															// Railway service
			return TraverseMode.RAIL;
		} else if (routeType >= 500 && routeType < 700) { // Metro Service and
															// Underground
															// Service
			return TraverseMode.SUBWAY;
		} else if (routeType >= 700 && routeType < 900) { // Bus Service and
															// Trolleybus
															// service
			return TraverseMode.BUS;
		} else if (routeType >= 900 && routeType < 1000) { // Tram service
			return TraverseMode.TRAM;
		} else if (routeType >= 1000 && routeType < 1100) { // Water Transport
															// Service
			return TraverseMode.FERRY;
		} else if (routeType >= 1100 && routeType < 1200) { // Air Service
			throw new IllegalArgumentException("Air transport not supported"
					+ routeType);
			...
			throw new IllegalArgumentException("Taxi service not supported"
					+ routeType);
