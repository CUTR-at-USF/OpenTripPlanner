...
public class PlanGenerator {
	...
	public TripPlan generate(RoutingRequest options) {
		...
		// TODO: this seems to only check the endpoints, which are usually
		// auto-generated
	...
	TripPlan generatePlan(List<GraphPath> paths, RoutingRequest request) {
		...
		Place from = new Place(tripStartVertex.getX(), tripStartVertex.getY(),
				startName);
		Place to = new Place(tripEndVertex.getX(), tripEndVertex.getY(),
				endName);
	 ...
	 */
	private Itinerary adjustItinerary(RoutingRequest request,
			Itinerary itinerary) {
	...
	/**
	 * Generate an itinerary from a {@link GraphPath}. This method first slices
	 * the list of states at the leg boundaries. These smaller state arrays are
	 * then used to generate legs. Finally the rest of the itinerary is
	 * generated based on the complete state array.
	 *
	 * @param path
	 *            The graph path to base the itinerary on
	...
	public Itinerary generateItinerary(GraphPath path,
		...
		for (State[] legStates : legsStates) {
			itinerary.addLeg(generateLeg(graph, legStates,
		...
		if (itinerary.transfers > 0
				&& !(states[0].getVertex() instanceof OnboardDepartVertex)) {
			itinerary.transfers--;
		}
		...
		for (Edge edge : edges) {
			...
			if (geometry != null) {
				if (coordinates.size() == 0) {
					...
					coordinates.extend(geometry.getCoordinates(), 1); // Avoid
																		// duplications
				}
			}
		}
	...
	/**
	 * Slice a {@link State} array at the leg boundaries. Leg switches occur
	 * when: 1. A LEG_SWITCH mode (which itself isn't part of any leg) is seen
	 * 2. The mode changes otherwise, for instance from BICYCLE to WALK 3. A
	 * PatternInterlineDwell edge (i.e. interlining) is seen
	 *
	 * @param states
	 *            The one-dimensional array of input states
	 * @return An array of arrays of states belonging to a single leg (i.e. a
	...
	private State[][] sliceStates(State[] states) {
		...
		for (int i = 1; i < states.length - 1; i++) {
			...
			if (backMode == TraverseMode.LEG_SWITCH
					|| forwardMode == TraverseMode.LEG_SWITCH) {
				if (backMode != TraverseMode.LEG_SWITCH) { // Start of leg
															// switch
					legIndexPairs[1] = i;
				} else if (forwardMode != TraverseMode.LEG_SWITCH) { // End of
																		// leg
																		// switch
				...
				legsIndexes.add(legIndexPairs);
				legIndexPairs = new int[] { i, states.length - 1 };
			} else if (edge instanceof PatternInterlineDwell) { // Interlining
																// => leg switch
				legIndexPairs[1] = i;
				legsIndexes.add(legIndexPairs);
	 ...
	 *
	 * @param states
	 *            The array of states to base the leg on
	 * @param showIntermediateStops
	 *            Whether to include intermediate stops in the leg or not
	 * @return The generated leg
	 */
	private Leg generateLeg(Graph graph, State[] states,
			boolean showIntermediateStops) {
		Leg leg = new Leg();
		...
		leg.agencyTimeZoneOffset = timeZone.getOffset(leg.startTime
				.getTimeInMillis());
		...
		leg.rentedBike = states[0].isBikeRenting()
				&& states[states.length - 1].isBikeRenting();
	...
	private void addFrequencyFields(State[] states, Leg leg) {
		/*
		 * TODO adapt to new frequency handling. if (states[0].getBackEdge()
		 * instanceof FrequencyBoard) { State preBoardState =
		 * states[0].getBackState();
		 * 
		 * FrequencyBoard fb = (FrequencyBoard) states[0].getBackEdge();
		 * FrequencyBasedTripPattern pt = fb.getPattern(); int boardTime; if
		 * (preBoardState.getServiceDay() == null) { boardTime = 0; //TODO why
		 * is this happening? } else { boardTime =
		 * preBoardState.getServiceDay().secondsSinceMidnight(
		 * preBoardState.getTimeSeconds()); } int period =
		 * pt.getPeriod(fb.getStopIndex(), boardTime); //TODO fix
		 * 
	...
	/**
	 * Add a {@link WalkStep} {@link List} to a {@link Leg} {@link List}. It's
	 * more convenient to process all legs in one go because the previous step
	 * should be kept.
	 *
	 * @param legs
	 *            The legs of the itinerary
	 * @param legsStates
	 *            The states that go with the legs
	 */
	private void addWalkSteps(Graph graph, List<Leg> legs, State[][] legsStates) {
		WalkStep previousStep = null;

		for (int i = 0; i < legsStates.length; i++) {
			List<WalkStep> walkSteps = generateWalkSteps(graph, legsStates[i],
					previousStep);
	...
	/**
	 * This was originally in TransitUtils.handleBoardAlightType. Edges that
	 * always block traversal (forbidden pickups/dropoffs) are simply not ever
	 * created.
	 */
	...
	/**
	 * Fix up a {@link Leg} {@link List} using the information available at the
	 * leg boundaries. This method will fill holes in the arrival and departure
	 * times associated with a {@link Place} within a leg and add board and
	 * alight rules. It will also ensure that stop names propagate correctly to
	 * the non-transit legs that connect to them.
	 *
	 * @param legs
	 *            The legs of the itinerary
	 * @param legsStates
	...
	private void fixupLegs(List<Leg> legs, State[][] legsStates) {
		for (int i = 0; i < legsStates.length; i++) {
			boolean toOther = i + 1 < legsStates.length
					&& legs.get(i + 1).interlineWithPreviousLeg;
			...
			for (int j = 1; j < legsStates[i].length; j++) {
				if (legsStates[i][j].getBackEdge() instanceof PatternEdge) {
					PatternEdge patternEdge = (PatternEdge) legsStates[i][j]
							.getBackEdge();
					...
					int boardType = (fromIndex != null) ? (tripPattern
							.getBoardType(fromIndex)) : 0;
					int alightType = (toIndex != null) ? (tripPattern
							.getAlightType(toIndex)) : 0;
			...
			if (legs.get(i).isTransitLeg()) {
				if (boardRule != null && !fromOther) { // If boarding in some
														// other leg
					legs.get(i).boardRule = boardRule; // (interline), don't
														// board now.
				}
				if (alightRule != null && !toOther) { // If alighting in some
														// other
					legs.get(i).alightRule = alightRule; // leg, don't alight
															// now.
				}
			}
		}
	 ...
	 * @param states
	 *            The states that go with the itinerary
	 */
	...
	/**
	 * Calculate the elevationGained and elevationLost fields of an
	 * {@link Itinerary}.
	 *
	 * @param itinerary
	 *            The itinerary to calculate the elevation changes for
	 * @param edges
	...
	private void calculateElevations(Itinerary itinerary, Edge[] edges) {
		for (Edge edge : edges) {
			...
			PackedCoordinateSequence coordinates = edgeWithElevation
					.getElevationProfile();

			if (coordinates == null)
				continue;
			// TODO Check the test below, AFAIU current elevation profile has 3
			// dimensions.
			if (coordinates.getDimension() != 2)
				continue;

			for (int i = 0; i < coordinates.size() - 1; i++) {
				double change = coordinates.getOrdinate(i + 1, 1)
						- coordinates.getOrdinate(i, 1);
	 ...
	 *            Whether to include intermediate stops in the leg or not
	 */
	private void addPlaces(Leg leg, State[] states, Edge[] edges,
			boolean showIntermediateStops) {
		...
		leg.from = makePlace(states[0], firstVertex, edges[0], firstStop,
				tripTimes);
		leg.from.arrival = null;
		leg.to = makePlace(states[states.length - 1], lastVertex, null,
				lastStop, tripTimes);
		...
		if (showIntermediateStops) {
			...
			for (int i = 1; i < edges.length; i++) {
				...
				leg.stop.add(makePlace(states[i], vertex, edges[i],
						currentStop, tripTimes));
			}
		}
	}
	 ...
	 */
	private Place makePlace(State state, Vertex vertex, Edge edge, Stop stop,
			TripTimes tripTimes) {
		// If no edge was given, it means we're at the end of this leg and need
		// to work around that.
	...
	private void addRealTimeData(Leg leg, State[] states) {
		...
		if (tripTimes != null && !tripTimes.isScheduled()) {
			...
			if (leg.from.stopIndex != null) {
				leg.departureDelay = tripTimes
						.getDepartureDelay(leg.from.stopIndex);
			}
	 ...
	 * 
	 * @param previous
	 *            a non-transit leg that immediately precedes this one
	 *            (bike-walking, say), or null
	 * 
	 * @return
	 */
	public static List<WalkStep> generateWalkSteps(Graph graph, State[] states,
			WalkStep previous) {
		List<WalkStep> steps = new ArrayList<WalkStep>();
		WalkStep step = null;
		double lastAngle = 0, distance = 0; // distance used for appending
											// elevation profiles
		int roundaboutExit = 0; // track whether we are in a roundabout, and if
								// so the exit number
		...
		for (int i = 0; i < states.length - 1; i++) {
			...
			// elevator narrative generation occurs when alighting). We don't
			// need to know what came
			...
			if (edge instanceof ElevatorAlightEdge) {
				...
				// tell the user where to get off the elevator using the exit
				// notation, so the
				// i18n interface will say 'Elevator to <exit>'
				// what happens is that the webapp sees name == null and ignores
				// that, and it sees
				// exit != null and uses to <exit>
				// the floor name is the AlightEdge name
				// reset to avoid confusion with 'Elevator on floor 1 to floor
				// 1'
			...
			if (step == null) {
			...
			} else if (((step.streetName != null && !step.streetNameNoParens()
					.equals(streetNameNoParens)) && (!step.bogusName || !edge
					.hasBogusName()))
					||
					// if we are on a roundabout now and weren't before, start a
					// new step
					edge.isRoundabout() != (roundaboutExit > 0)
					|| isLink(edge)
					&& !isLink(backState.getBackEdge())) {
				/*
				 * street name has changed, or we've changed state from a
				 * roundabout to a street
				 */
				if (roundaboutExit > 0) {
					// if we were just on a roundabout,
					// make note of which exit was taken in the existing step
					step.exit = Integer.toString(roundaboutExit); // ordinal
																	// numbers
																	// from
				...
				if (edge.isRoundabout()) {
					...
					roundaboutExit = 1;
					roundaboutPreviousStreet = backState.getBackEdge()
							.getName();
					idx = roundaboutPreviousStreet.indexOf('(');
					if (idx > 0)
						roundaboutPreviousStreet = roundaboutPreviousStreet
								.substring(0, idx - 1);
				}
				double thisAngle = DirectionUtils.getFirstAngle(geom);
				...
				RelativeDirection direction = WalkStep.getRelativeDirection(
						lastAngle, thisAngle, edge.isRoundabout());
				boolean optionsBefore = backState.multipleOptionsBefore();
				if (edge.isRoundabout()) {
					// we are on a roundabout, and have already traversed at
					// least one edge of it.
					if (optionsBefore) {
				...
				if (edge.isRoundabout()
					...
					// we are continuing almost straight, or continuing along a
					// roundabout.
					// just append elevation info onto the existing step.

				} else {
					// we are not on a roundabout, and not continuing straight
					// through.

					// figure out if there were other plausible turn options at
					// the last
					// intersection
					// to see if we should generate a "left to continue"
					// instruction.
					boolean shouldGenerateContinue = false;
					if (edge instanceof StreetEdge) {
						// the next edges will be PlainStreetEdges, we hope
						double angleDiff = getAbsoluteAngleDiff(thisAngle,
								lastAngle);
						for (Edge alternative : backState.getVertex()
							...
							double altAngle = DirectionUtils
									.getFirstAngle(alternative.getGeometry());
							double altAngleDiff = getAbsoluteAngleDiff(
									altAngle, lastAngle);
							if (angleDiff > Math.PI / 4
									|| altAngleDiff - angleDiff < Math.PI / 16) {
								shouldGenerateContinue = true;
								break;
							}
						}
					} else {
						double angleDiff = getAbsoluteAngleDiff(lastAngle,
								thisAngle);
						// FIXME: this code might be wrong with the removal of
						// the edge-based graph
						State twoStatesBack = backState.getBackState();
						Vertex backVertex = twoStatesBack.getVertex();
						for (Edge alternative : backVertex
								.getOutgoingStreetEdges()) {
			...
			if (createdNewStep && !disableZagRemovalForThisStep
				...
				if (last >= 2) {
					...
					if (twoBack.distance < MAX_ZAG_DISTANCE
							&& lastStep.streetNameNoParens().equals(
									threeBack.streetNameNoParens())) {
						...
						if (((lastStep.relativeDirection == RelativeDirection.RIGHT || lastStep.relativeDirection == RelativeDirection.HARD_RIGHT) && (twoBack.relativeDirection == RelativeDirection.RIGHT || twoBack.relativeDirection == RelativeDirection.HARD_RIGHT))
							...
							// in this case, we have two left turns or two right
							// turns in quick
							...
							// in this case, we're definitely staying on the
							// same street
							// (since it's zag removal, the street names are the
							// same)
							...
							if (twoBack.elevation != null) {
								if (step.elevation == null) {
									...
									for (P2<Double> d : twoBack.elevation) {
										step.elevation.add(new P2<Double>(
												d.first + step.distance,
												d.second));
									}
								}
							}
	...
	private static double getAbsoluteAngleDiff(double thisAngle,
			double lastAngle) {
	...
	private static WalkStep createWalkStep(Graph graph, State s) {
		...
		step.angle = DirectionUtils
				.getFirstAngle(s.getBackEdge().getGeometry());
	...
	private static List<P2<Double>> encodeElevationProfile(Edge edge,
			double offset) {
		if (!(edge instanceof StreetEdge)) {
		...
		Coordinate[] coordArr = elevEdge.getElevationProfile()
				.toCoordinateArray();
	...
	/**
	 * Returns the first trip of the service day. Currently unused. TODO This
	 * should probably be done with a special time value.
	 */
	...
	/**
	 * Return the last trip of the service day. Currently unused. TODO This
	 * should probably be done with a special time value.
	 */
