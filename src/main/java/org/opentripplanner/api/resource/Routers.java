...
/**
 * This REST API endpoint allows remotely loading, reloading, and evicting graphs on a running
 * server.
 * 
 * A GraphService maintains a mapping between routerIds and specific Graph objects. The HTTP verbs
 * are used as follows to manipulate that mapping:
 * 
 ...
 * 
 * The HTTP request URLs are of the form /ws/routers/{routerId}, where the routerId is optional. If
 * a routerId is supplied in the URL, the verb will act upon the mapping for that specific routerId.
 * If no routerId is given, the verb will act upon all routerIds currently registered.
 * 
 * For example:
 * 
 * GET http://localhost/otp-rest-servlet/ws/routers will retrieve a list of all registered routerId
 * -> Graph mappings and their geographic bounds.
 * 
 * GET http://localhost/otp-rest-servlet/ws/routers/london will return status code 200 and a brief
 * description of the 'london' graph including geographic bounds, or 404 if the 'london' routerId is
 * not registered.
 * 
 * PUT http://localhost/otp-rest-servlet/ws/routers will reload the graphs for all currently
 * registered routerIds from disk.
 * 
 * PUT http://localhost/otp-rest-servlet/ws/routers/paris will load a Graph from a sub-directory
 * called 'paris' and associate it with the routerId 'paris'.
 * 
 * DELETE http://localhost/otp-rest-servlet/ws/routers/paris will release the Paris Graph and
 * de-register the 'paris' routerId.
 * 
 * DELETE http://localhost/otp-rest-servlet/ws/routers will de-register all currently registered
 * routerIds.
 * 
 * The GET methods are not secured, but all other methods are secured under ROLE_ROUTERS. See
 * documentation for individual methods for additional parameters.
 */
@Path("/routers")
@PermitAll
...
public class Routers {
     ...
     *        memory usage from increasing during the reload, but routing will be unavailable on
     *        this routerId for the duration of the operation.
     *
    ...
    public Response postGraphOverWire(@PathParam("routerId") String routerId,
            @QueryParam("preEvict") @DefaultValue("true") boolean preEvict,
            @QueryParam("loadLevel") @DefaultValue("FULL") LoadLevel level, InputStream is) {
        if (preEvict) {
            LOG.debug("pre-evicting graph");
            server.graphService.evictGraph(routerId);
        }
    ...
    /**
     * Save the graph data, but don't load it in memory. The file location is based on routerId. If
     * the graph already exists, the graph will be overwritten.
     */
    @RolesAllowed({ "ROUTERS" })
    @POST
    @Path("/save")
    @Produces({ MediaType.TEXT_PLAIN })
    @Consumes(MediaType.APPLICATION_OCTET_STREAM)
    public Response saveGraphOverWire(@QueryParam("routerId") String routerId, InputStream is) {
     ...
     * 
     * @return status code 200 if the routerId was de-registered, 404 if the routerId was not
     *         registered.
     */
    @RolesAllowed({ "ROUTERS" })
    @DELETE
