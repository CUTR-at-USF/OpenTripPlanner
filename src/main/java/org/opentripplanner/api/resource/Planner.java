 ...
 * the query string. These parameters are defined in the abstract SearchResource superclass, which
 * also has methods for building routing requests from query parameters. In order for inheritance to
 * work, the REST resources are actually request-scoped rather than singleton-scoped.
 * 
 * Some parameters may not be honored by the trip planner for some or all itineraries. For example,
 * maxWalkDistance may be relaxed if the alternative is to not provide a route.
 * 
...
public class Planner extends RoutingResource {
    ...
    public Response getItineraries(@Context OTPServer otpServer, @Context UriInfo uriInfo) {
         ...
         * or vertex ids to planner (or error back to user) TODO:
         * org.opentripplanner.routing.impl.PathServiceImpl has COOORD parsing. Abstract that out so
         * it's used here too...
         */
