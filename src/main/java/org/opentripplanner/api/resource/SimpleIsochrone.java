...
/**
 * This class provides a vector isochrone generator for places without good OSM
 * street connectivity, or for graphs that are too large-scale to bother with
 * modeling streets (e.g. all of China). It assumes constant speed through both
 * streets and "free space" around vertices, yielding isochrones that are the
 * union of many circles.
 * 
 * The origin snapping option used here (connect to all stations within M meters
 * of a lat/lon) should be made into a standard query option in OTP. Instead of
 * specifying an origin station search radius, it could be more coherent to use
 * all stations within (maxIsochroneTime * speed) meters, and apply a travel
 * time to reach them.
 * 
 * The GET methods both use makeContours which in turn uses makePoints (where
 * range checking occurs). Putting "@Setter" on all the parameters would allow
 * resource classes to be used outside Jersey.
 */
@Path("/routers/{routerId}/simpleIsochrone")
public class SimpleIsochrone extends RoutingResource {

	private static final Logger LOG = LoggerFactory
			.getLogger(SimpleIsochrone.class);
	...
	/**
	 * Whether the results should be left in the indicated CRS or de-projected
	 * back to WGS84.
	 */
	 ...
	 */
	// We are not using CoordinateReferenceSystem StringReaderProvider here
	// because
	...
	/**
	 * @return a map from each vertex to minimum travel time over the course of
	 *         the day.
	 */
	private Map<Vertex, Double> makePoints() throws Exception {
		...
		if (originCoord == null)
			return null;
		List<TransitStop> stops = graph.streetIndex.getNearbyTransitStops(
				originCoord, radiusMeters);
		if (stops.isEmpty()) {
		...
		StreetVertex origin = new IntersectionVertex(graph, "iso_temp",
				originCoord.x, originCoord.y);
		...
		for (int r = 0; r < nRequests; r++) {
			request.dateTime = date.getTime() / 1000 + r
					* requestSpacingMinutes * 60;
			LOG.debug("date: {} {}", new Date(request.dateTime),
					request.dateTime);
	...
	/**
	 * @return a map from contour (isochrone) thresholds in seconds to
	 *         geometries.
	 */
	private Map<Integer, Geometry> makeContours() throws Exception {
		...
		/*
		 * Set up transforms into projected coordinates (necessary for buffering
		 * in meters)
		 */
		...
		for (int c = 0; c < nContours; ++c) {
			...
			for (Map.Entry<Vertex, Double> vertexSeconds : points.entrySet()) {
				double remainingSeconds = thresholdSeconds
						- vertexSeconds.getValue();
				if (remainingSeconds > 60) { // avoid degenerate geometries
					double remainingMeters = remainingSeconds
							* request.walkSpeed;
					Geometry point = geomf.createPoint(vertexSeconds.getKey()
							.getCoordinate());
		...
		for (Integer threshold : bufferLists.keys()) {
			...
			Geometry geom = geomf.buildGeometry(buffers); // make geometry
															// collection
			geom = geom.union(); // combine all individual buffers in this
									// contour into one
			if (!resultsProjected)
	...
	private SimpleFeatureCollection makeContourFeatures() throws Exception {
		...
		/*
		 * Stage the features in memory, in order from bottom to top, biggest to
		 * smallest
		 */
		DefaultFeatureCollection featureCollection = new DefaultFeatureCollection(
				null, contourSchema);
	...
	/**
	 * @return Evenly spaced travel time contours (isochrones) as a zipped
	 *         shapefile.
	 */
	@GET
	...
	public Response zippedShapefileGet(
		...
		/*
		 * "FeatureSource is used to read features, the subclass FeatureStore is
		 * used for read/write access. The way to tell if a File can be written
		 * to in GeoTools is to use an instanceof check.
		 */
	...
	/**
	 * A HashMap that has been extended to track the greatest or smallest value
	 * for each key.
	 */
	public static class MinMap<K, V extends Comparable<V>> extends
			HashMap<K, V> {
		private static final long serialVersionUID = -23L;

		/**
		 * Put the given key-value pair in the map if the map does not yet
		 * contain the key, or if the value is less than the existing value for
		 * the same key.
		 * 
		...
		/**
		 * Put the given key-value pair in the map if the map does not yet
		 * contain the key, or if the value is greater than the existing value
		 * for the same key.
		 * 
		 * @return whether the key-value pair was inserted in the map.
		 */
