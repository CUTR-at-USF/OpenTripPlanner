...
public class TimeGridWs extends RoutingResource {
	...
	@GET
	@Produces({ "image/png" })
	public Response getTimeGridPng(
		...
		if (coordinateOrigin != null)
			tgRequest.coordinateOrigin = new GenericLocation(null,
		...
		// We force to 8 bits channel depth, some clients won't support more
		// than 8
		// (namely, HTML5 canvas...)
		ImageInfo imgInfo = new ImageInfo(cols, rows, 8, true, false, false);
		/**
		 * TODO: PNGJ allow for progressive (ie line-by-line) writing. Thus we
		 * could theorically prevent having to allocate a bit pixel array in the
		 * first place, but we would need a line-by-line iterator on the sample
		 * grid, which is currently not the case.
		 */
		...
		pw.getMetadata().setText(PngChunkTextVar.KEY_Creation_Time,
				new Date().toString());
		pw.getMetadata().setText(PngChunkTextVar.KEY_Description,
				"Sample grid bitmap");

		String gridCornerStr = String.format(
				Locale.US,
				"%.8f,%.8f",
				sampleGrid.getCenter().y + sampleGrid.getYMin()
						* sampleGrid.getCellSize().y, sampleGrid.getCenter().x
						+ sampleGrid.getXMin() * sampleGrid.getCellSize().x);
		String gridCellSzStr = String.format(Locale.US, "%.12f,%.12f",
				sampleGrid.getCellSize().y, sampleGrid.getCellSize().x);
		...
		for (ZSamplePoint<WTWD> p : sampleGrid) {
			...
			if (Double.isInfinite(zz)) {
				...
				iz = ImageLineHelper.clampTo_0_65535((int) Math
						.round(zz * unit));
				if (iz == 65535)
					iz = 65534; // Clamp
			}
			...
			int id = ImageLineHelper.clampTo_0_255((int) Math.round(z.d
					/ precisionMeters * 100));
			...
			/*
			 * Keep the alpha channel at 255, otherwise the RGB channel will be
			 * downsampled on some rendering clients (namely, JS canvas).
			 */
		...
		return Response.ok().cacheControl(cc).entity(data)
				.header(OTPA_GRID_CORNER, gridCornerStr)
