...
public abstract class QueryParameter {
	...
	public static void checkRangeInclusive(double x, double min, double max)
			throws Exception {
		boolean ok = (x >= min && x <= max);
		if (!ok)
			throw new Exception(String.format("%f is not in range [%f,%f]", x,
					min, max));
	}
	...
	public QueryParameter(String value) throws WebApplicationException {
		try {
			...
			String message = String.format("Error parsing parameter: %s (%s)",
					value, e.toString());
