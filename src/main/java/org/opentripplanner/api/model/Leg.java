...
public class Leg {
	...
	/**
	 * For transit leg, the offset from the scheduled departure-time of the
	 * boarding stop in this leg. "scheduled time of departure at boarding stop"
	 * = startTime - departureDelay
	 */
	public int departureDelay = 0;
	/**
	 * For transit leg, the offset from the scheduled arrival-time of the
	 * alighting stop in this leg. "scheduled time of arrival at alighting stop"
	 * = endTime - arrivalDelay
	 */
	 ...
	 * important for real-time trips, strict frequency trips, and scheduled
	 * trips with empirical headways.
	 */
	...
	/**
	 * For transit legs, the route of the bus or train being used. For
	 * non-transit legs, the name of the street being traversed.
	 */
	@XmlAttribute
	@JsonSerialize
	public String route = "";
	...
	/**
	 * For transit legs, the type of the route. Non transit -1 When 0-7: 0 Tram,
	 * 1 Subway, 2 Train, 3 Bus, 4 Ferry, 5 Cable Car, 6 Gondola, 7 Funicular
	 * When equal or highter than 100, it is coded using the Hierarchical
	 * Vehicle Type (HVT) codes from the European TPEG standard Also see
	...
	/**
	 * For transit legs, if the rider should stay on the vehicle as it changes
	 * route names.
	 */
	@XmlAttribute
	@JsonSerialize
	...
	/**
	 * For transit leg, the trip's short name (if one exists). For non-transit
	 * legs, null.
	 */
	@XmlAttribute
	@JsonSerialize
	...
	/**
	 * For transit leg, the trip's block ID (if one exists). For non-transit
	 * legs, null.
	 */
	@XmlAttribute
	@JsonSerialize
	...
	/**
	 * For transit legs, the ID of the transit agency that operates the service
	 * used for this leg. For non-transit legs, null.
	 */
	@XmlAttribute
	@JsonSerialize
	...
	/**
	 * For transit legs, the service date of the trip. For non-transit legs,
	 * null.
	 */
	@XmlAttribute
	@JsonSerialize
	 ...
	 * This field is optional i.e. it is always null unless
	 * "showIntermediateStops" parameter is set to "true" in the planner
	 * request.
	 */
	...
	/**
	 * A series of turn by turn instructions used for walking, biking and
	 * driving.
	 */
	...
	public Boolean isTransitLeg() {
		if (mode == null)
			...
			return false;
		else if (mode.equals(TraverseMode.CUSTOM_MOTOR_VEHICLE.toString()))
			return false;
		else
			return true;
	}
	...
	public double getDuration() {
		return endTime.getTimeInMillis() / 1000.0 - startTime.getTimeInMillis()
				/ 1000.0;
	}
