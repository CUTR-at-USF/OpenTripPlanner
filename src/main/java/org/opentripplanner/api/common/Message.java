...
public enum Message {
	...
	OUTSIDE_BOUNDS(400), PATH_NOT_FOUND(404), NO_TRANSIT_TIMES(406), REQUEST_TIMEOUT(
			408), BOGUS_PARAMETER(413), GEOCODE_FROM_NOT_FOUND(440), GEOCODE_TO_NOT_FOUND(
			450), GEOCODE_FROM_TO_NOT_FOUND(460), TOO_CLOSE(409), LOCATION_NOT_ACCESSIBLE(
			470),

	GEOCODE_FROM_AMBIGUOUS(340), GEOCODE_TO_AMBIGUOUS(350), GEOCODE_FROM_TO_AMBIGUOUS(
			360),

	UNDERSPECIFIED_TRIANGLE(370), TRIANGLE_NOT_AFFINE(371), TRIANGLE_OPTIMIZE_TYPE_NOT_SET(
			372), TRIANGLE_VALUES_NOT_SET(373), ;
	...
	public String get(String def, Locale l) {
		try {
			...
			Properties.LOG
					.warn("No entry in Message.properties file could be found for string "
							+ name());
		}
		return def;
	}
	 ...
	 * ... NOTE: not sure where this will be used...from legacy code in case
	 * it's ever needed...
	 */
