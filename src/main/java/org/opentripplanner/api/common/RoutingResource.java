...
public abstract class RoutingResource {
    ...
    protected RoutingRequest buildRequest(int n) throws ParameterException {
        ...
        request.setWaitAtBeginningFactor(get(waitAtBeginningFactor, n,
                request.getWaitAtBeginningFactor()));
        ...
        request.setShowIntermediateStops(get(showIntermediateStops, n,
                request.isShowIntermediateStops()));
        ...
        request.setOtherThanPreferredRoutesPenalty(get(otherThanPreferredRoutesPenalty, n,
                request.getOtherThanPreferredRoutesPenalty()));
        ...
        HashMap<AgencyAndId, BannedStopSet> bannedTripMap = makeBannedTripMap(get(bannedTrips, n,
                null));
        ...
        request.setNonpreferredTransferPenalty(get(nonpreferredTransferPenalty, n,
                request.getNonpreferredTransferPenalty()));
        ...
        if (request.getIntermediatePlaces() != null
                && (request.getModes().isTransit() || (request.getModes().getWalk() && request
                        .getModes().getBicycle())))
     ...
     * elements, or the default value if the list is empty or null. Throughout buildRequest() you
     * will see the following idiom: request.setParamX(get(paramX, n, request.getParamX));
     * 
     * This checks a query parameter field from Jersey (which is a list, one element for each
     * occurrence of the parameter in the query string) for the nth occurrence, or the one with the
     * highest index. If a parameter was supplied, it replaces the value in the RoutingRequest under
     * construction (which was cloned from the prototypeRoutingRequest). If not, it uses the value
     * already in that RoutingRequest as a default (i.e. it uses the value cloned from the
     * PrototypeRoutingRequest).
     * 
