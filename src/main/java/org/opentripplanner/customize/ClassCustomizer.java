...
/**
 * This creates a subclass of a given class by adding a new interface to it, and
 * allows users to add any necessary fields
 * 
 * The intended use is to allow optimizations or extensions that require
 * per-edge data to massage an existing graph to meet their needs, without
 * adding additional fields to the core.
 * 
 * @author novalis
 * 
 */
public class ClassCustomizer {
	 ...
	 */
	public ClassCustomizer(Class<?> iface, String oldlassName,
			String newClassName) {
	...
	/**
	 * Creates a clone of original but with the class NewClass (which extends
	 * original's class)
	 */
	...
	/**
	 * Add a simple getter with signature "double getFoo()" to the class, which
	 * simply returns the value of the field fieldName
	 * 
	 * @param ctClass
	 * @param classFile
	 * @param fieldName
	...
	private void addDoubleGetter(ClassFile classFile, String fieldName)
		...
		// double getFoo()
		MethodInfo getter = new MethodInfo(constPool, "get"
				+ ucfirst(fieldName), "()D");
	...
	private void addDoubleSetter(ClassFile classFile, String fieldName)
		...
		// void setFoo(double)
		MethodInfo setter = new MethodInfo(constPool, "set"
				+ ucfirst(fieldName), "(D)V");
