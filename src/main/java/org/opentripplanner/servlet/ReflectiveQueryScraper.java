...
/**
 * A Servlet filter that constructs new objects of a given class, using
 * reflection to pull their field values directly from the query parameters in
 * an HttpRequest. It then seeds the request scope by storing a reference to the
 * constructed object as an attribute of the HttpRequest itself.
 * 
 * An instance of the requested class is first instantiated via its 0-argument
 * constructor. Any initialization and defaults should be handled at this point.
 * 
 * Next, field and setter method names are matched with query parameters in the
 * incoming HttpRequest. Fields whose declared type has a constructor taking a
 * single String argument (including String itself) will be set from the query
 * parameter having the same name, if one exists. Setter methods will also be
 * considered if 1) they have a single argument, and 2) that argument's class
 * has a constructor with a single String argument.
 * 
 * Query parameters are matched with setter methods according to the usual
 * convention: changing the first character to upper case and prepending 'set'.
 * A setter method invocation will be preferred to directly setting the field
 * with the corresponding name, if it exists.
 * 
 * @author andrewbyrd
 */
public class ReflectiveQueryScraper {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ReflectiveQueryScraper.class);
	...
	public ReflectiveQueryScraper(Class<?> targetClass) {
		...
		/*
		 * Scan methods after fields, so they will override fields with the same
		 * name.
		 */
	...
	private static abstract class Target {
		...
		// NOTE: hashCode and equals reference only the param name. Collisions
		// are intentional.
		@Override
		public int hashCode() {
	...
	private static class FieldTarget extends Target {
		...
		public String toString() {
			return String.format("%s %s = %s('%s')", target.getType()
					.getSimpleName(), target.getName(), constructor.getName(),
					param);
		}
	}

	// TODO: setters match param names with query parameters (allowing multiple
	// parameters);
	...
	private static class MethodTarget extends Target {
		...
		static Target instanceFor(Method method) {
			...
			if (methodName.length() == 3)
				return null;
			String baseName = methodName.substring(3, 4).toLowerCase()
		...
		@Override
		public String toString() {
			return String.format("%s(%s('%s'))", target.getName(),
