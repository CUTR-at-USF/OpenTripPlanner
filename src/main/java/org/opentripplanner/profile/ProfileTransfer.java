...
/**
 * A transfer as used in profile routing. For now, only the best transfer
 * between any two patterns.
 */
public class ProfileTransfer implements Comparable<ProfileTransfer> {

	private static final Logger LOG = LoggerFactory
			.getLogger(ProfileTransfer.class);

	public TripPattern tp1, tp2;
	public StopCluster sc1, sc2;
	public int distance; // meters

	public ProfileTransfer(TripPattern tp1, TripPattern tp2, StopCluster s1,
			StopCluster s2, int distance) {
		this.tp1 = tp1;
		this.tp2 = tp2;
		this.sc1 = s1;
		this.sc2 = s2;
	...
	/**
	 * Keeps track of the best N transfers, including all those that are within
	 * the same stop cluster.
	 */
		...
		public void add(ProfileTransfer xfer) {
			...
			while (iter.hasNext()) {
				...
				if (xfer.distance > curr.distance * SLOP) {
					return; // this transfer is a lot worse than an existing
							// one. bail out without saving it.
				}
				if (curr.distance > xfer.distance * SLOP) {
					iter.remove(); // this transfer is a lot better than an
									// existing one. remove the existing one.
				}
			}
