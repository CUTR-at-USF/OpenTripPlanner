 ...
 * 
 * TODO this could just be a RouteRide key mapped to lists of patternRides with the same
 * characteristics. its constructor would pull out the right fields.
 * 
 * TODO if patternRides can be added later, and stats and transfers are computed after the fact,
 * then maybe we no longer need a round-based approach. We need to put _groups_ of patternRides into
 * the queue, grouped by back ride and beginning stop.
 */
public class Ride {
    ...
    public String toStringVerbose() {
        return String.format("ride route %s from %s to %s (%d patterns)", route.getLongName(),
                from.getName(), to.getName(), patternRides.size());
    }

    public String toString() {
        return String.format("route %3s from %4s to %4s (%d)", route.getShortName(),
                from.getCode(), to.getCode(), patternRides.size());
    }
        ...
        for (PatternRide patternRide : patternRides) {
            for (TripTimes tt : patternRide.pattern.getScheduledTimetable().getTripTimes()) {
                if (window.servicesRunning.get(tt.serviceCode)) {
                    int t = arrivals ? tt.getArrivalTime(patternRide.toIndex) : tt
                            .getDepartureTime(patternRide.fromIndex);
    ...
    /**
     * Produce stats about boarding an initial Ride, which has no previous ride. This assumes
     * arrival times are uniformly distributed during the window. The Ride must contain some trips,
     * and the window must have a positive duration.
     */
    ...
    /**
     * Calculates Stats for the transfer to the given ride from the previous ride. This should only
     * be called after all PatternRides have been added to the ride. Distances can be stored in
     * rides, including the first and last distance. But waits must be calculated from full sets of
     * patterns, which are not known until a round is over.
     */
    public Stats statsForTransfer(TimeWindow window, double walkSpeed) {
        ...
        if (previous == null)
            return this.statsForBoarding(window);
        List<Integer> departures = getSortedStoptimes(window, false);
