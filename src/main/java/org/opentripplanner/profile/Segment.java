 ...
 * Strings and ints here. TODO rename to TransitSegment
 */
public class Segment {
	...
	public static class SegmentPattern implements Comparable<SegmentPattern> {
		...
		public SegmentPattern(PatternRide patternRide) {
			...
			this.nTrips = patternRide.stats.num; // this stats has time window
													// applied
		}
	...
	public Segment(Ride ride) {
		...
		for (PatternRide patternRide : ride.patternRides) {
			...
			// LOG.info("   pattern {} {} from {} to {}",
			// patternRide.pattern.mode, patternRide.pattern.getCode(),
			// patternRide.getFromStop(), patternRide.getToStop());
		}
		Collections.sort(segmentPatterns);
		// Note that despite the fact that multiple patterns from different
		// routes will appear in the same ride,
		// in practice all the patterns within a ride will be from the same
		// operator and mode because they all pass
		// through the same stops.
		mode = ride.patternRides.get(0).pattern.mode;
		for (PatternRide pr : ride.patternRides) {
			if (pr.pattern.mode != mode)
