...
public class Option {
	...
	// The fares are outside the transit segments because a fare can apply to
	// multiple segments so there is no one-to-one
	// correspondance. For example, when you transfer from one subway to another
	// and pay one fare for the two segments.

	public Option(Ride tail, Collection<StopAtDistance> accessPaths,
			Collection<StopAtDistance> egressPaths) {
		access = StreetSegment.list(accessPaths);
		egress = StreetSegment.list(egressPaths);
		// Include access and egress times across all modes in the overall
		// travel time statistics for this option.
		// FIXME In the event that there is only access, N will still be 1 which
		// is strange.
		stats.add(access);
		stats.add(egress);
		...
		if (!rides.isEmpty()) {
			...
			rides.get(0).accessTime = 0; // Avoid double-inclusion of the access
											// time for the first leg.
			rides.get(0).accessDist = 0; // Just to make dist coherent with the
											// accessTime in the result object.
		...
		// Really should be one per segment, with transfers to the same operator
		// having a price of 0.
	...
	public String generateSummary() {
		...
		for (Segment segment : transit) {
			...
			for (RouteShort rs : segment.routes) {
				String routeName = rs.shortName == null ? rs.longName
						: rs.shortName;
