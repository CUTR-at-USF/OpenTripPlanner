...
public class ProfileRouter {
    ...
    /**
     * Adds a new PatternRide to the PatternRide's destination stop. If a Ride already exists there
     * on the same route, and with the same previous Ride, then the new PatternRide is grouped into
     * that existing Ride. Otherwise a new Ride is created. PatternRide may be null, indicating an
     * empty PatternRide with no trips. In this case no Ride is created or updated, and this method
     * returns null. A new set of rides is produced in each round.
     * 
    ...
    private Ride addRide(PatternRide pr) {
        ...
        for (Ride ride : rides.get(pr.getToStop())) {
            if (ride.previous == pr.previous && ride.route == pr.pattern.route
                    && ride.to == pr.getToStop()) { // FIXME: how could it not be equal? To-stops
                                                    // should always match in our rides collections
    ...
    /**
     * Complete a partial PatternRide (with only pattern and beginning stop) which was enqueued in
     * the last round. This is done by scanning through the Pattern, creating rides to all
     * downstream stops that are near the destination or have relevant transfers.
     */
    private void makeRides(PatternRide pr) {
        ...
        for (int s = pr.fromIndex + 1; s < stops.size(); ++s) {
            ...
            if (targetStop != null && targetStop == stop) {
                ...
                if (!pathContainsStop(pr.previous, stop)) { // move this check outside the
                                                            // conditionals?
    ...
    // TimeWindow could actually be resolved and created in the caller, which does have access to
    // the profiledata.
    public ProfileResponse route() {
        ...
        for (Entry<TripPattern, StopAtDistance> entry : fromStops.entrySet()) {
            ...
            for (int i = 0; i < pattern.getStops().size(); ++i) {
                if (pattern.getStops().get(i) == sd.stop) {
                    ...
                    ProfileTransfer xfer = new ProfileTransfer(null, pattern, null, sd.stop,
                            sd.distance);
            ...
            if (round != finalRound) {
                ...
                for (Ride ride : rides.values()) {
                    ...
                    for (ProfileTransfer tr : graph.index.transfersForStop.get(ride.to)) {
                        ...
                        if (round == penultimateRound && !toStops.containsKey(tr.tp2))
                            continue;
                        if (ride.containsPattern(tr.tp1)) {
                            if (pathContainsRoute(ride, tr.tp2.route))
                                continue;
                            if (tr.s1 != tr.s2 && pathContainsStop(ride, tr.s2))
                                continue;
        ...
        for (Ride ride : targetRides) {
            ...
            Option option = new Option(ride, dist, window, request.walkSpeed); // TODO Convert
                                                                               // distance to time.
    ...
    public void findStreetOptions() {
        for (TraverseMode mode : Lists.newArrayList(TraverseMode.WALK, TraverseMode.BICYCLE,
                TraverseMode.CAR)) {
    ...
    /**
     * Find all patterns that stop close to a given vertex. Return a map from each pattern to the
     * closest stop on that pattern. We want a single stop object rather than an index within the
     * pattern because we want to consider stops that appear more than once in a pattern at every
     * index where they occur. Therefore this is not the right place to convert stop objects to stop
     * indexes.
     */
    public Map<TripPattern, StopAtDistance> findClosestPatterns(boolean back) {
        ...
        if (closest.size() > 50) {
            ...
            Multimap<StopAtDistance, TripPattern> busPatterns = TreeMultimap.create(
                    Ordering.natural(), Ordering.arbitrary());
            Multimap<StopAtDistance, TripPattern> otherPatterns = TreeMultimap.create(
                    Ordering.natural(), Ordering.arbitrary());
    ...
    /**
     * Perform an on-street search at the origin and destination points to find nearby stops, as
     * well as a path to the destination if possible.
     */
    private List<StopAtDistance> findClosestStops(boolean back) {
        ...
        // Note that the (forward) search is intentionally unlimited so it will reach the
        // destination
        // on-street, even though only transit boarding locations closer than req.streetDist will be
        // used.
