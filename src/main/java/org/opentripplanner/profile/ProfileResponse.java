...
public class ProfileResponse {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ProfileResponse.class);

	public List<Option> options = Lists.newArrayList();

	/**
	 * The constructed response will include all the options that do not use
	 * transit, as well as the top N options that do use transit.
	 *
	 * @param orderBy
	 *            specifies how the top N transit options will be chosen
	 * @param limit
	 *            the maximum number of transit options to include in the
	 *            response.
	 */
	public ProfileResponse(Collection<Option> options,
			Option.SortOrder orderBy, int limit) {
