...
public class DCFareCalculator {
	...
	private static final FareTable VRE = new FareTable(
			"org/opentripplanner/profile/fares/vre.csv");

	private static final String[] metroExpress = { "J7", "J9", "P17", "P19",
			"W13", "W19", "11Y", "17A", "17B", "17G", "17H", "17K", "17L",
			"17M", "18E", "18G", "18H", "18P", "29E", "29G", "29H", "29X" };

	// geographic fare areas for MTA (Maryland) commuter buses
	private static final FareArea MTA_DC = new FareArea(-77.066139, 38.868166,
			-76.986868, 38.926);
	private static final FareArea MTA_SILVER_SPRING = new FareArea(-76.943069,
			38.838617, -76.916848, 38.855997);
	private static final FareArea MTA_SUITLAND = new FareArea(-77.04039,
			38.985653, -77.02017, 39.002778);
	private static final FareArea MTA_KENT_ISLAND = new FareArea(-76.360503,
			38.939413, -76.170122, 39.012155);
	private static final FareArea MTA_DAVIDSONVILLE = new FareArea(-76.669367,
			38.952661, -76.637353, 38.970023);
	private static final FareArea MTA_ANNAPOLIS = new FareArea(-76.574267,
			38.960267, -76.469125, 39.001783);
	private static final FareArea MTA_OWINGS_NORTH_BEACH = new FareArea(
			-76.623115, 38.687874, -76.519002, 38.732147);
	private static final FareArea MTA_PINDELL = new FareArea(-76.772156,
			38.754121, -76.639891, 38.830921);
	private static final FareArea MTA_CHARLOTTE_HALL_WALDORF = new FareArea(
			-76.97628, 38.435241, -76.71225, 38.656712);
	private static final FareArea MTA_CALIFORNIA = new FareArea(-76.571502,
			38.273603, -76.483254, 38.3446);
	private static final FareArea MTA_DUNKIRK = new FareArea(-76.677069,
			38.692187, -76.635025, 38.742091);
	private static final FareArea MTA_SUNDERLAND = new FareArea(-76.627974,
			38.530956, -76.564987, 38.678062);
	private static final FareArea MTA_ST_LEONARD = new FareArea(-76.527123,
			38.458123, -76.48802, 38.495911);

	private static RideType classify(Route route) {
		// NOTE the agencyId string of the route's agencyAndId is not the same
		// as the one given by route.getAgency.
		// The former is the same for all routes in the feed. The latter is the
		// true agency of the feed.

		String agency = route.getAgency().getId();
		String agency_url = route.getAgency().getUrl(); // this is used in
														// single-agency feeds
														// so it should work
	...
	/**
	 * Should we have exactly one fare per ride, where some fares may have zero
	 * cost if they are transfers from the same operator? ...except that this
	 * doesn't work for MetroRail, where two legs combine into one.
	 */
	public static List<Fare> calculateFares(List<Ride> rides) {
		List<FareRide> fareRides = Lists.newArrayList();
		FareRide prev = null;
		for (Ride ride : rides) {
			// Calculate the fare for a ride based on only one of its patterns.
			// This is usually right as long as the
			// patterns are all the same mode and operator. That happens less
			// naturally now that stops are being grouped.
			...
			if (prev != null && prev.type == fareRide.type) {
				prev.to = fareRide.to;
				prev.calcFare(); // recalculate existing fare using new
									// destination
			} else {
	...
	public static Rectangle2D.Double createFareZone(double min_lon,
			double max_lat, double max_lon, double min_lat) {
		return new Rectangle2D.Double(min_lon, max_lat, max_lon - min_lon,
				max_lat - min_lat);
	}

	static class FareArea extends Rectangle2D.Double {

		public FareArea(double min_lon, double min_lat, double max_lon,
				double max_lat) {
	...
	static class FareRide {
		...
		public FareRide(PatternRide pRide, FareRide prevRide) {
			...
			to = pRide.getToStop();
			// Problem: Rides no longer have a single fare because they may be
			// on multiple routes.
			// TODO: make sure Patterns in Rides are all the same mode and
			// operator.
			// This seems to happen naturally because different operators
			// generally do not share stops.
		...
		private void setFare(double low, double peak, double senior,
				boolean transferReduction) {
			fare = new Fare(peak);
			fare.low = low;
			fare.senior = senior;
			fare.transferReduction = transferReduction;
		...
		// TODO store rule-based Fares in a table keyed on (type, prevtype)
		// instead of doing on the fly
		// automatically compose string using 'free' or 'discounted' and route
		// name
		private void calcFare() {
			...
			case METRO_RAIL:
				...
				if (prevType == RideType.METRO_BUS_LOCAL
						...
						|| prevType == RideType.MCRO_BUS_EXPRESS) { // TODO
																	// merge
																	// local and
																	// express
																	// categories
			...
			case PRTC_BUS:
				...
				if (prevType == RideType.VRE_RAIL) {
					...
					setFare(5.75, false);
				} else if (routeName.contains("omnilink")
						|| routeName.contains("connector")) {
					setFare(1.30, false);
			...
			case MTA_BUS_COMMUTER:
				...
				case "902":
					if (MTA_DUNKIRK.containsStop(from)
							|| MTA_DUNKIRK.containsStop(to)) {
						setFare(3.50, false);
				...
				case "904":
					if (MTA_PINDELL.containsStop(from)
							|| MTA_PINDELL.containsStop(to)) {
						setFare(3.50, false);
	...
	enum RideType {
		METRO_RAIL, METRO_BUS_LOCAL, METRO_BUS_EXPRESS, METRO_BUS_AIRPORT, DC_CIRCULATOR_BUS, ART_BUS, DASH_BUS, MARC_RAIL, MTA_BUS_LOCAL, MTA_BUS_EXPRESS, MTA_BUS_COMMUTER, VRE_RAIL, MCRO_BUS_LOCAL, MCRO_BUS_EXPRESS, FAIRFAX_CONNECTOR_BUS, PRTC_BUS
	}
