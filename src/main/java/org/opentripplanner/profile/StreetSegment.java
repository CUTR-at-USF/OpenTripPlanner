...
/**
 * A response object describing a non-transit part of an option (usually
 * access/egress).
 */
public class StreetSegment {

	private static final Logger LOG = LoggerFactory
			.getLogger(StreetSegment.class);
	...
	/**
	 * Build the walksteps from the final State of a path. The path may contain
	 * more than one leg, because you may need to walk the bike. Therefore
	 * accumulate the geometries into one long polyline, and accumulate the
	 * walksteps.
	 */
	public StreetSegment(State state) {
		...
		Geometry geom = GeometryUtils.getGeometryFactory().createLineString(
				coordinates);
	...
	/**
	 * A StreetSegment is very similar to a StopAtDistance but it's a response
	 * object so the State has to be rendered into walksteps.
	 */
	public StreetSegment(StopAtDistance sd) {
		this(sd.state);
		mode = sd.mode; // Intended mode is known more reliably in a
						// StopAtDistance than from a State.
	}
