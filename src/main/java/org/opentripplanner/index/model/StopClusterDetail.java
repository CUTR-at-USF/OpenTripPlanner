...
/**
 * A representation of a stop cluster for use in an index API response. Can
 * contain references to stops for more detail.
 */
public class StopClusterDetail {
	...
	public static List<StopClusterDetail> list(Collection<StopCluster> in,
			boolean detail) {
		List<StopClusterDetail> out = Lists.newArrayList();
		for (StopCluster cluster : in)
			out.add(new StopClusterDetail(cluster, detail));
		return out;
	}
