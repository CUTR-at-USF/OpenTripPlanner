...
// One @Produces annotation for all endpoints.
public class IndexAPI {
	...
	/*
	 * Needed to check whether query parameter map is empty, rather than
	 * chaining " && x == null"s
	 */
	@Context
	UriInfo uriInfo;
	...
	public Response getAgencies() {
		return Response.status(Status.OK).entity(index.agencyForId.values())
				.build();
	}
	...
	@GET
	@Path("/stops")
	public Response getStopsInRadius(@QueryParam("minLat") Double minLat,
			@QueryParam("minLon") Double minLon,
			@QueryParam("maxLat") Double maxLat,
			@QueryParam("maxLon") Double maxLon, @QueryParam("lat") Double lat,
			@QueryParam("lon") Double lon, @QueryParam("radius") Double radius) {

		/* When no parameters are supplied, return all stops. */
		if (uriInfo.getQueryParameters().isEmpty()) {
			Collection<Stop> stops = index.stopForId.values();
			return Response.status(Status.OK).entity(StopShort.list(stops))
					.build();
		}
		/*
		 * If any of the circle parameters are specified, expect a circle not a
		 * box.
		 */
		boolean expectCircle = (lat != null || lon != null || radius != null);
		if (expectCircle) {
			if (lat == null || lon == null || radius == null || radius < 0) {
				return Response.status(Status.BAD_REQUEST).entity(MSG_400)
						.build();
			}
			...
			for (TransitStop stopVertex : streetIndex.getNearbyTransitStops(
				...
				double distance = distanceLibrary.fastDistance(
						stopVertex.getCoordinate(), coord);
				if (distance < radius) {
			...
			List<StopShort> stops = Lists.newArrayList();
			Envelope envelope = new Envelope(new Coordinate(minLon, minLat),
					new Coordinate(maxLon, maxLat));
			for (TransitStop stopVertex : streetIndex
	...
	public Response getRoutesForStop(@PathParam("stopId") String stopId) {
		Stop stop = index.stopForId
				.get(GtfsLibrary.convertIdFromString(stopId));
		if (stop == null)
			return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
		...
		return Response.status(Status.OK).entity(RouteShort.list(routes))
				.build();
	}
	...
	public Response getPatternsForStop(@PathParam("stopId") String stopIdString) {
		...
		return Response.status(Status.OK).entity(PatternShort.list(patterns))
				.build();
	}
	...
	public Response getStoptimesForStop(@PathParam("stopId") String stopIdString) {
		Stop stop = index.stopForId.get(GtfsLibrary
				.convertIdFromString(stopIdString));
		if (stop == null)
			return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
		return Response.status(Status.OK).entity(index.stopTimesForStop(stop))
				.build();
	}
	...
	public Response getRoutes(@QueryParam("hasStop") List<String> stopIds) {
		...
		if (stopIds != null) {
			// Protective copy, we are going to calculate the intersection
			// destructively
			routes = Lists.newArrayList(routes);
			for (String stopId : stopIds) {
				Stop stop = index.stopForId.get(GtfsLibrary
						.convertIdFromString(stopId));
				if (stop == null)
					return Response.status(Status.NOT_FOUND).entity(MSG_404)
							.build();
	...
	public Response getPatternsForRoute(
			@PathParam("routeId") String routeIdString) {
		AgencyAndId routeId = GtfsLibrary.convertIdFromString(routeIdString);
		Route route = index.routeForId.get(routeId);
		if (route != null) {
			Collection<TripPattern> patterns = index.patternsForRoute
					.get(route);
			return Response.status(Status.OK)
					.entity(PatternShort.list(patterns)).build();
		} else {
			return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
		}
	}
	...
	public Response getTripsForRoute(@PathParam("routeId") String routeIdString) {
		...
		if (route != null) {
			...
			return Response.status(Status.OK).entity(TripShort.list(trips))
					.build();
		} else {
			return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
		}
	...
	public Response getSemanticHashForTrip(
	...
	public Response getStoptimesForTrip(@PathParam("tripId") String tripIdString) {
		...
		if (trip != null) {
			...
			return Response.status(Status.OK)
					.entity(TripTimeShort.fromTripTimes(table, trip)).build();
		} else {
			return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
		}
	}
	...
	public Response getPattern(@PathParam("patternId") String patternIdString) {
		...
		if (pattern != null) {
			return Response.status(Status.OK)
					.entity(new PatternDetail(pattern)).build();
		} else {
			return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
		}
	}
	...
	public Response getTripsForPattern(
	...
	public Response getStopsForPattern(
			@PathParam("patternId") String patternIdString) {
		// Pattern names are graph-unique because we made them that way (did not
		// read them from GTFS).
		TripPattern pattern = index.patternForId.get(patternIdString);
		if (pattern != null) {
	...
	public Response getSemanticHashForPattern(
			@PathParam("patternId") String patternIdString) {
		// Pattern names are graph-unique because we made them that way (did not
		// read them from GTFS).
		TripPattern pattern = index.patternForId.get(patternIdString);
	...
	/**
	 * List details about a specific service ID including which dates it runs
	 * on. Replaces the old /calendar.
	 */
	@GET
	...
	public Response getAllStopClusters() {
		...
		List<StopClusterDetail> scl = StopClusterDetail.list(
				index.stopClusterForId.values(), detail);
	...
	public Response getStopCluster(
			@PathParam("clusterId") String clusterIdString) {
		StopCluster cluster = index.stopClusterForId.get(clusterIdString);
		if (cluster != null) {
			return Response.status(Status.OK)
					.entity(new StopClusterDetail(cluster, true)).build();
		} else {
			return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
		}
	}
