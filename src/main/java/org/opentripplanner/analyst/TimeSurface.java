...
/**
 * A travel time surface. Timing information from the leaves of a
 * ShortestPathTree. In Portland, one timesurface takes roughly one MB of memory
 * and is also about that size as JSON. However it is proportionate to the graph
 * size not the time cutoff.
 */
public class TimeSurface implements Serializable {
	...
	public Map<String, String> params; // The query params sent by the user, for
										// reference only

	public SparseMatrixZSampleGrid<WTWD> sampleGrid; // another representation
														// on a regular grid
	...
	public TimeSurface(ShortestPathTree spt) {
		...
		// Here we use the key "default" unlike the graphservice which
		// substitutes in the default ID.
		// We don't want to keep that default in sync across two modules.
		this.routerId = routerId;
		long t0 = System.currentTimeMillis();
		times = new int[Vertex.getMaxIndex()]; // memory leak due to temp
												// vertices?
	...
	// TODO Lazy-initialize sample grid on demand so initial SPT finishes
	// faster, and only isolines lag behind.
	// however, the existing sampler needs an SPT, not general vertex-time
	// mappings.
	public void makeSampleGrid(ShortestPathTree spt) {
		long t0 = System.currentTimeMillis();
		final double gridSizeMeters = 300; // Todo: set dynamically and make
											// sure this matches isoline builder
											// params
		...
		final double D0 = 0.8 * gridSizeMeters; // offroad walk distance roughly
												// grid size
		...
		double dX = dY / cosLat;
		sampleGrid = new SparseMatrixZSampleGrid<WTWD>(16,
				spt.getVertexCount(), dX, dY, coordinateOrigin);
		SampleGridRenderer.sampleSPT(spt, sampleGrid, gridSizeMeters * 0.7,
				gridSizeMeters, V0, spt.getOptions().getMaxWalkDistance(),
				cosLat);
		long t1 = System.currentTimeMillis();
