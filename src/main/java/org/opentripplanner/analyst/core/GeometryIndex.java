...
/**
 * This index is used in Analyst and does not need to be instantiated if you are
 * not performing Analyst requests.
 */
public class GeometryIndex implements GeometryIndexService {

	private static final Logger LOG = LoggerFactory
			.getLogger(GeometryIndex.class);
	...
	public GeometryIndex(Graph graph) {
		...
		for (StreetVertex vertex : Iterables.filter(graph.getVertices(),
				StreetVertex.class)) {
			for (StreetEdge e : Iterables.filter(vertex.getOutgoing(),
					StreetEdge.class)) {
				LineString geom = e.getGeometry();
				if (e.getPermission().allows(
	...
	public BoundingBox getBoundingBox(CoordinateReferenceSystem crs) {
		try {
			...
			LOG.error(
					"error transforming graph bounding box to request CRS : {}",
					crs);
			return null;
		}
	}
