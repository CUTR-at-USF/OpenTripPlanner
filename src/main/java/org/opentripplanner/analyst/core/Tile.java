...
/**
 * Analyst 8-bit tile format: Seconds are converted to minutes. Minutes are
 * clamped to +-120 Unreachable pixels are set to Byte.MIN_VALUE (-128) Result
 * is stored in image pixel as a signed byte.
 * 
 * So: -119 to +119 are interpreted literally, +120 means >= +120, -120 means <=
 * -120, -128 means "unreachable".
 */
public abstract class Tile {
	...
	/**
	 * Creates an interpolated 8-bit color map from the supplied array of color
	 * values. Each row in the input array is a 5-element array consisting of:
	 * colorIndex, red, green, blue, alpha Color indexes must be in increasing
	 * order. Negative indexes will be stored as signed bytes, so -1 is 0xFF
	 * etc.
	 */
	...
	/*
	 * Pixels are travel times in minutes, stored as signed bytes. This allows
	 * us to represent times and time differences with absolute values up to 2
	 * hours.
	 */
	private static final IndexColorModel ICM_SMOOTH_COLOR_15 = interpolatedColorMap(new int[][] {
			{ 0, 0, 0, 0, 0 }, { 15, 100, 100, 100, 80 },
			{ 30, 0, 200, 0, 80 }, { 45, 0, 0, 200, 80 },
			{ 60, 200, 200, 0, 80 }, { 75, 200, 0, 0, 80 },
			{ 90, 200, 0, 200, 50 }, { 120, 200, 0, 200, 0 } });
	...
	private static final IndexColorModel ICM_STEP_COLOR_15 = interpolatedColorMap(new int[][] {
			...
			{ 0, 100, 100, 100, 0 }, { 15, 100, 100, 100, 90 },
			{ 15, 0, 140, 0, 10 }, { 30, 0, 140, 0, 90 },
			{ 30, 0, 0, 140, 10 }, { 45, 0, 0, 140, 90 },
			{ 45, 140, 140, 0, 10 }, { 60, 140, 140, 0, 90 },
			{ 60, 140, 0, 0, 10 }, { 75, 140, 0, 0, 90 },
			{ 75, 140, 0, 140, 10 }, { 90, 140, 0, 140, 90 },
			{ 90, 100, 100, 100, 50 }, { 121, 100, 100, 100, 200 } });

	private static final IndexColorModel ICM_DIFFERENCE_15 = interpolatedColorMap(new int[][] {
			{ -128, 0, 0, 0, 0 }, { -127, 150, 0, 0, 80 },
			{ -60, 150, 0, 0, 80 }, { -15, 150, 150, 0, 80 },
			{ 0, 150, 150, 0, 0 }, { 0, 0, 0, 0, 0 }, { 15, 0, 0, 150, 80 },
			{ 45, 0, 150, 0, 90 }, { 60, 100, 150, 100, 99 },
			{ 127, 50, 150, 50, 99 } });
	...
	private static final IndexColorModel ICM_SAMENESS_5 = interpolatedColorMap(new int[][] {
			{ -20, 80, 80, 80, 0 }, { -15, 100, 0, 100, 80 },
			{ -10, 0, 0, 150, 80 }, { -5, 0, 150, 0, 80 },
			{ 0, 0, 150, 0, 150 }, { 5, 0, 150, 0, 80 }, { 10, 0, 0, 150, 80 },
			{ 15, 100, 0, 100, 80 }, { 20, 80, 80, 80, 0 }, { -20, 0, 0, 0, 0 } // wrap
																				// around
																				// to
																				// hide
																				// inaccessible
	...
	Tile(TileRequest req) {
		...
		this.gg = new GridGeometry2D(gridEnv,
				(org.opengis.geometry.Envelope) (req.bbox));
	...
	private static IndexColorModel buildOldDefaultColorMap() {
		...
		for (int i = 0; i < 28; i++) {
			...
			palette[i + 00] = new Color(ALPHA
					& Color.HSBtoRGB(0.333f, i * 0.037f, 0.8f), true); // Green
			palette[i + 30] = new Color(ALPHA
					& Color.HSBtoRGB(0.666f, i * 0.037f, 0.8f), true); // Blue
			palette[i + 60] = new Color(ALPHA
					& Color.HSBtoRGB(0.144f, i * 0.037f, 0.8f), true); // Yellow
			palette[i + 90] = new Color(ALPHA
					& Color.HSBtoRGB(0.000f, i * 0.037f, 0.8f), true); // Red
			palette[i + 120] = new Color(ALPHA
					& Color.HSBtoRGB(0.000f, 0.000f, (29 - i) * 0.0172f), true); // Black
		}
		for (int i = 28; i < 30; i++) {
			palette[i + 00] = new Color(ALPHA
					& Color.HSBtoRGB(0.333f, (30 - i) * 0.333f, 0.8f), true); // Green
			palette[i + 30] = new Color(ALPHA
					& Color.HSBtoRGB(0.666f, (30 - i) * 0.333f, 0.8f), true); // Blue
			palette[i + 60] = new Color(ALPHA
					& Color.HSBtoRGB(0.144f, (30 - i) * 0.333f, 0.8f), true); // Yellow
			palette[i + 90] = new Color(ALPHA
					& Color.HSBtoRGB(0.000f, (30 - i) * 0.333f, 0.8f), true); // Red
			palette[i + 120] = new Color(ALPHA
					& Color.HSBtoRGB(0.000f, 0.000f, (29 - i) * 0.0172f), true); // Black
		}
	...
	public BufferedImage generateImage(TimeSurface surf,
			RenderRequest renderRequest) {
		long t0 = System.currentTimeMillis();
		BufferedImage image = getEmptyImage(renderRequest.style);
		byte[] imagePixelData = ((DataBufferByte) image.getRaster()
	...
	public BufferedImage linearCombination(double k1, TimeSurface surfA,
			double k2, TimeSurface surfB, double intercept,
			RenderRequest renderRequest) {
		long t0 = System.currentTimeMillis();
		BufferedImage image = getEmptyImage(renderRequest.style);
		byte[] imagePixelData = ((DataBufferByte) image.getRaster()
	...
	public GridCoverage2D getGridCoverage2D(BufferedImage image) {
		GridCoverage2D gridCoverage = new GridCoverageFactory().create(
				"isochrone", image, gg.getEnvelope2D());
		return gridCoverage;
	}
	...
	public static BufferedImage getLegend(Style style, int width, int height) {
		...
		for (int row = 0; row < height; row++)
			for (int col = 0; col < width; col++)
				pixels[row * width + col] = (byte) (startVal + col * bandsTotal
						/ width);
