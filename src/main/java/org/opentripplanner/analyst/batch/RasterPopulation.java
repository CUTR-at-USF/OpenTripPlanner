...
public class RasterPopulation extends BasicPopulation {
    ...
    @Setter
    int rows = 200, cols = 200; // these are the raster (gridEnvelope) dimensions

    @Setter
    double left, right, top, bottom; // bounding box values in CRS

    @Setter
    int band = 0; // raster band to read

    @Setter
    double unitySeconds = 0; // scale output values so unity=1. 0 to turn off.
    ...
    protected GridGeometry2D gridGeometry; // relationship between the grid envelope and the CRS
                                           // envelope
    ...
    public void writeGeotiff(String fileName, ResultSet results) {
        ...
        GridCoverage2D coverage = new GridCoverageFactory().create("OTPAnalyst", imagePixelData,
                refEnvelope);
        try {
    ...
    public void createIndividuals() {
        ...
        try {
            ...
            // because we may want to produce an empty raster rather than loading one, alongside the
            // coverage we
            // store the row/col dimensions and the referenced envelope in the original coordinate
            // reference system.
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                try {
                    ...
                    // TODO: we are performing 2 transforms here, it would probably be more
                    // efficient to compose
                    ...
                    // evaluate using grid coordinates, which should be more efficient than using
                    // world coordinates
                    if (coverage != null)
