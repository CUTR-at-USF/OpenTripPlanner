...
/**
 * Individuals should be in a random-access-friendly List implementation in
 * row-major order
 */
public class RasterPopulation extends BasicPopulation {

	private static final Logger LOG = LoggerFactory
			.getLogger(RasterPopulation.class);

	/* configuration fields */
	public int rows = 200, cols = 200; // these are the raster (gridEnvelope)
										// dimensions
	public double left, right, top, bottom; // bounding box values in CRS
	public int band = 0; // raster band to read
	public double unitySeconds = 0; // scale output values so unity=1. 0 to turn
									// off.

	/* derived fields */
	protected CoordinateReferenceSystem coverageCRS; // from input raster or
														// config string
	...
	protected GridGeometry2D gridGeometry; // relationship between the grid
											// envelope and the CRS envelope
	protected MathTransform gridToWGS84; // composed transform from the grid to
											// the CRS to WGS84
	protected GridCoverage2D coverage; // null if synthetic (no coverage to
										// load)
	...
	public void writeGeotiff(String fileName, ResultSet results) {
		...
		GridCoverage2D coverage = new GridCoverageFactory().create(
				"OTPAnalyst", imagePixelData, refEnvelope);
		try {
			GeoTiffWriteParams wp = new GeoTiffWriteParams();
			wp.setCompressionMode(GeoTiffWriteParams.MODE_EXPLICIT);
			wp.setCompressionType("LZW");
			ParameterValueGroup params = new GeoTiffFormat()
					.getWriteParameters();
			params.parameter(
	...
	public void createIndividuals() {
		...
		try {
			...
			// because we may want to produce an empty raster rather than
			// loading one, alongside the coverage we
			// store the row/col dimensions and the referenced envelope in the
			...
			throw new IllegalStateException(
					"Error loading population from raster file: ", ex);
		}
		LOG.info("Done loading raster from file.");
	}

	/**
	 * Shared internal createIndividuals method allowing synthetic subclass to
	 * reuse projection code
	 */
	protected void createIndividuals0() {
		...
		// grid coordinate object to be reused for reading each cell in the
		// raster
		...
		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < cols; col++) {
				...
				try {
					...
					// TODO: we are performing 2 transforms here, it would
					// probably be more efficient to compose
					// the grid-to-crs and crs-to-WGS84 transforms into
					// grid-to-WGS84.
					...
					// evaluate using grid coordinates, which should be more
					// efficient than using world coordinates
					if (coverage != null)
						coverage.evaluate(coord, val);
					// add this grid cell to the population
					String label = row + "_" + col;
					Individual individual = new Individual(label, lon, lat,
							val[band]);
