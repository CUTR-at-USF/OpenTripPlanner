...
public interface Population extends Iterable<Individual> {
    ...
    /**
     * @return a list of all Individuals in this Population, including those that have been marked
     *         as rejected by the filter chain.
     */
     ...
     * filtering them, but not sampling (linking them into the graph) because origin populations do
     * not need to be permanently linked.
     */
    public void setup();

    /**
     * Subclass-specific method to load the individuals from a file or create them based on other
     * properties. This method should fill in all fields of each individual except the sample, since
     * sampling will be carried out after the filter chain is applied.
     */
