...
public class BatchProcessor {
	 ...
	 * org.springframework.core.io.Resource appContextResource; if( args.length
	 * == 0) {
	 * LOG.warn("no configuration XML file specified; using example on classpath"
	 * ); appContextResource = new ClassPathResource(EXAMPLE_CONTEXT); } else {
	 * String configFile = args[0]; appContextResource = new
	 * FileSystemResource(configFile); } GenericApplicationContext ctx = new
	 * GenericApplicationContext(); XmlBeanDefinitionReader xmlReader = new
	 * XmlBeanDefinitionReader(ctx);
	 * xmlReader.loadBeanDefinitions(appContextResource); ctx.refresh();
	 * ctx.registerShutdownHook(); BatchProcessor processor =
	...
	private void run() {
		...
		if (aggregator != null) {
			...
			mode = Mode.AGGREGATE;
			aggregateResultSet = new ResultSet(origins); // results shaped like
															// origins
		} else if (accumulator != null) {
			/* accumulate data for each origin into all destinations */
			mode = Mode.ACCUMULATE;
			aggregateResultSet = new ResultSet(destinations); // results shaped
																// like
																// destinations
		} else {
	...
	private boolean checkpoint() {
		if (checkpointIntervalMinutes < 0 || aggregateResultSet == null)
			return false;
		long currentTime = System.currentTimeMillis();
		// not threadsafe, but the worst thing that will happen is a double
		// checkpoint
		// anyway, this is being called in the controller thread now
		if (currentTime > lastCheckpointTime + checkpointIntervalMinutes * 60
				* 1000) {
			...
			if (req != null) {
				...
				case ACCUMULATE:
					synchronized (aggregateResultSet) {
						accumulator.accumulate(oi.input, results,
								aggregateResultSet);
					}
					break;
				case AGGREGATE:
					aggregateResultSet.results[i] = aggregator
							.computeAggregate(results);
					break;
				default:
					String subName = outputPath.replace("{}",
							String.format("%d_%s", i, oi.label));
