...
public class CSVPopulation extends BasicPopulation {
	...
	public void createIndividuals() {
		try {
			CsvReader reader = new CsvReader(sourceFilename, ',',
			...
			if (crs != null) {
				...
				if (!destCrs.equals(sourceCrs)) {
					...
					mathTransform = CRS.findMathTransform(sourceCrs, destCrs,
							false);
				}
			}
			...
			while (reader.readRecord()) {
				...
				if (transform) {
					...
					// x: lat, y: lon. This seems backwards but is the way
					// Geotools does it.
					if (latLon) {
