...
public class SampleFactory implements SampleSource {
	...
	public void setSearchRadiusM(double radiusMeters) {
		...
		this.searchRadiusLat = SphericalDistanceLibrary
				.metersToDegrees(searchRadiusM);
	}
	...
	/**
	 * DistanceToPoint.computeDistance() uses a LineSegment, which has a
	 * closestPoint method. That finds the true distance every time rather than
	 * once the closest segment is known, and does not allow for
	 * equi-rectangular projection/scaling.
	 * 
	 * Here we want to compare squared distances to all line segments until we
	 * find the best one, then do the precise calculations.
	 * 
	 */
	public Sample findClosest(List<Edge> edges, Coordinate pt, double xscale) {
		...
		for (Edge edge : edges) {
			/*
			 * LineString.getCoordinates() uses
			 * PackedCoordinateSequence.toCoordinateArray() which necessarily
			 * builds new Coordinate objects.CoordinateSequence.getOrdinate()
			 * reads them directly.
			 */
			c.edge = edge;
			...
			for (int seg = 0; seg < numCoords - 1; seg++) {
				...
				c.frac = GeometryUtils.segmentFraction(x0, y0, x1, y1, pt.x,
						pt.y, xscale);
	...
	private static class Candidate {
		...
		public double distanceAlong() {
			CoordinateSequence cs = ((LineString) (edge.getGeometry()))
			...
			dist += distanceLibrary.fastDistance(y0, x0, y, x); // dist along
																// partial
																// segment
			return dist;
		}
		...
		public double distanceToEnd() {
			...
			double x0 = cs.getX(s);
			double y0 = cs.getY(s);
			double dist = distanceLibrary.fastDistance(y0, x0, y, x); // dist
																		// along
																		// partial
																		// segment
			int nc = cs.size();
