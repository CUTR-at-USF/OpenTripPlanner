...
public class Renderer {
	...
	public Response getResponse(TileRequest tileRequest, TimeSurface surfA,
			TimeSurface surfB, RenderRequest renderRequest) throws Exception {

		Tile tile = tileCache.get(tileRequest);
		BufferedImage image;
		switch (renderRequest.layer) {
		case DIFFERENCE:
			image = tile.linearCombination(1, surfA, -1, surfB, 0,
					renderRequest);
			break;
		case HAGERSTRAND:
			long elapsed = Math.abs(surfB.dateTime - surfA.dateTime);
			image = tile.linearCombination(-1, surfA, -1, surfB, elapsed / 60,
					renderRequest);
			break;
		case TRAVELTIME:
		default:
		...
		// of course this will make it useless as a raster for analysis, but
		// it's good for animations.
		if (renderRequest.timestamp) {
			DateFormat df = DateFormat.getDateTimeInstance();
			df.setTimeZone(TimeZone.getTimeZone("America/New_York"));
			String ds = df.format(new Date(surfA.dateTime * 1000));
			shadowWrite(image, ds,
					String.format("%f, %f", surfA.lat, surfA.lon));
	...
	public static Response generateStreamingImageResponse(
		...
		StreamingOutput streamingOutput = new StreamingOutput() {
			public void write(OutputStream outStream) {
				try {
					...
					LOG.error("exception while preparing image : {}",
							e.getMessage());
		...
		cc.setMaxAge(3600);
		cc.setNoCache(false);
		return Response.ok(streamingOutput).type(format.toString())
				.cacheControl(cc).build();
	}

	private static Response generateStreamingGeotiffResponse(
		...
		StreamingOutput streamingOutput = new StreamingOutput() {
			public void write(OutputStream outStream) {
				try {
					...
					new GeoTiffWriter(outStream).write(
							coverage,
							(GeneralParameterValue[]) params.values().toArray(
									new GeneralParameterValue[1]));
					// new GeoTiffWriter(outStream).write(coverage, null);
					// //wasn't this line writing twice and trashing compressed
					// version?
					long t1 = System.currentTimeMillis();
					LOG.debug("wrote geotiff in {}msec", t1 - t0);
				} catch (Exception e) {
					LOG.error("exception while preparing geotiff : {}",
							e.getMessage());
					throw new WebApplicationException(e);
				}
			}
		...
		cc.setMaxAge(3600);
		cc.setNoCache(false);
		return Response.ok(streamingOutput).type("image/geotiff")
				.cacheControl(cc).build();
	}
