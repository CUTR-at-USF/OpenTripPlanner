 ...
 * 
 * First compute a shortest-path-tree from the given routing request. It then
 * build the sample grid (a regular grid of samples covering the whole SPT area)
 * using an accumulative grid sampling process.
 * 
 * @see ZSampleGrid
...
public class SampleGridRenderer {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(SampleGridRenderer.class);
	 ...
	 * @return
	 */
	public ZSampleGrid<WTWD> getSampleGrid(SampleGridRequest spgRequest,
		...
		sptRequest.worstTime = (sptRequest.dateTime + (sptRequest.arriveBy ? -spgRequest.maxTimeSec
				- tOvershot
				: spgRequest.maxTimeSec + tOvershot));
		...
		SparseMatrixZSampleGrid<WTWD> sampleGrid = new SparseMatrixZSampleGrid<WTWD>(
				16, spt.getVertexCount(), dX, dY, coordinateOrigin);
		sampleSPT(spt, sampleGrid, gridSizeMeters * 0.7, gridSizeMeters, V0,
				sptRequest.getMaxWalkDistance(), cosLat);
		sptRequest.cleanup();

		long t2 = System.currentTimeMillis();
		LOG.info("Computed SPT in {}msec, {}msec for sampling ({} msec total)",
	 ...
	 * @return
	 */
	public static void sampleSPT(ShortestPathTree spt,
			ZSampleGrid<WTWD> sampleGrid, final double d0,
			final double gridSizeMeters, final double v0,
			final double maxWalkDistance, final double cosLat) {

		final DistanceLibrary distanceLibrary = SphericalDistanceLibrary
				.getInstance();

		// Below is a closure that makes use of the parameters to the enclosing
		// function.

		/**
		 * Any given sample is weighted according to the inverse of the squared
		 * normalized distance + 1 to the grid sample. We add to the sampling
		 * time a default off-road walk distance to account for off-road
		 * sampling.
		 */
		AccumulativeMetric<WTWD> accMetric = new AccumulativeMetric<WTWD>() {
			@Override
			public WTWD cumulateSample(Coordinate C0, Coordinate Cs, WTWD z,
					WTWD zS) {
			...
			/**
			 * A Generated closing sample take 1) as off-road distance, the
			 * minimum of the off-road distance of all enclosing samples, plus
			 * the grid size, and 2) as time the minimum time of all enclosing
			 * samples plus the grid size * off-road walk speed as additional
			 * time. All this are approximations.
			 *
			 * TODO Is there a better way of computing this? Here the
			 * computation will be different based on the order where we close
			 * the samples.
			 */
			@Override
			public WTWD closeSample(WTWD zUp, WTWD zDown, WTWD zRight,
					WTWD zLeft) {
				...
				z.w = 1.0;
				/*
				 * The computations below are approximation, but we are on the
				 * edge anyway and the current sample does not correspond to any
				 * computed value.
				 */
		...
		johnny.walk(new SPTVisitor() {
			...
			@Override
			public final void visit(Coordinate c, State s0, State s1,
					double d0, double d1) {
				double wd0 = s0.getWalkDistance() + d0;
				double wd1 = s0.getWalkDistance() + d1;
				double t0 = wd0 > maxWalkDistance ? Double.POSITIVE_INFINITY
						: s0.getActiveTime() + d0 / v0;
				double t1 = wd1 > maxWalkDistance ? Double.POSITIVE_INFINITY
						: s1.getActiveTime() + d1 / v0;
	...
	/**
	 * The default TZ data we keep for each sample: Weighted Time and Walk
	 * Distance
	 * 
	 * For now we keep all possible values in the vector; we may want to remove
	 * the values that will not be used in the process (for example # of
	 * boardings). Currently the filtering is done afterwards, it may be faster
	 * and surely less memory-intensive to do the filtering when processing.
	 * 
	 * @author laurent
	 */
	public static class WTWD {
		...
		public static class IsolineMetric implements
				IsolineBuilder.ZMetric<WTWD> {
			@Override
			public int cut(WTWD zA, SampleGridRenderer.WTWD zB, WTWD z0) {
				double t0 = z0.wTime / z0.w;
				double tA = zA.d > z0.d ? Double.POSITIVE_INFINITY : zA.wTime
						/ zA.w;
				double tB = zB.d > z0.d ? Double.POSITIVE_INFINITY : zB.wTime
						/ zB.w;
				if (tA < t0 && t0 <= tB)
					return 1;
				if (tB < t0 && t0 <= tA)
			...
			public double interpolate(WTWD zA, WTWD zB, WTWD z0) {
				if (zA.d > z0.d || zB.d > z0.d) {
					...
					// Interpolate on d
					double k = zA.d == zB.d ? 0.5 : (z0.d - zA.d)
							/ (zB.d - zA.d);
					return k;
				} else {
					// Interpolate on t
