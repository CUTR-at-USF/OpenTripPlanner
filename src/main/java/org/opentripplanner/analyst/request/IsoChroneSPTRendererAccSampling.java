...
/**
 * Compute isochrones out of a shortest path tree request (AccSampling isoline
 * algorithm).
 * 
 * @author laurent
 */
public class IsoChroneSPTRendererAccSampling implements IsoChroneSPTRenderer {
	...
	public IsoChroneSPTRendererAccSampling(GraphService graphService,
	...
	public List<IsochroneData> getIsochrones(IsoChroneRequest isoChroneRequest,
		...
		final double D0 = sampleGridRenderer
				.getOffRoadDistanceMeters(isoChroneRequest.precisionMeters);
		...
		LOG.info("Computed {} isochrones in {}msec", isochrones.size(),
				(int) (t1 - t0));
