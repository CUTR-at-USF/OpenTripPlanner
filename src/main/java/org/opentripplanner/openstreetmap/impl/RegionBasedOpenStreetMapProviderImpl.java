...
public class RegionBasedOpenStreetMapProviderImpl implements
		OpenStreetMapProvider {

	private static Logger LOG = LoggerFactory
			.getLogger(RegionBasedOpenStreetMapProviderImpl.class);
	...
	private static class DownloadHandler implements OSMDownloaderListener {
		...
		public void handleMapTile(String key, double lat, double lon,
			...
			try {
				...
				throw new IllegalStateException("error reading osm file "
						+ pathToMapTile, ex);
			} catch (SAXException ex) {
				throw new IllegalStateException("error parsing osm file "
						+ pathToMapTile, ex);
			}
		}
	};
	...
	public void checkInputs() {
		if (!_cacheDirectory.canWrite()) {
			throw new RuntimeException("Can't write to OSM cache: "
					+ _cacheDirectory);
		}
	}
}
