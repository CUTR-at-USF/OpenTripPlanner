...
/**
 * Parser for the OpenStreetMap PBF format. Parses files in three passes: First
 * the relations, then the ways, then the nodes are also loaded.
 *
 ...
 * @since 0.4
 */
public class BinaryFileBasedOpenStreetMapProviderImpl implements
	...
	public void readOSM(OpenStreetMapContentHandler handler) {
		try {
			BinaryOpenStreetMapParser parser = new BinaryOpenStreetMapParser(
					handler);
			...
			throw new IllegalStateException("error loading OSM from path "
					+ _path, ex);
		}
	}
