...
/**
 * Bike-rental station data source for the
 * "Communauté d'Agglomération de Bordeaux" (CUB) VCub (aka V^3) bike-rental
 * network.
 * 
 * URL:
 * http://data.lacub.fr/wfs?key=<your-API-key>&request=getfeature&service=wfs
 * &version=1.1.0& typename=CI_VCUB_P&srsname=epsg:4326
 * 
 * 
 * @author laurent
 */
public class VCubDataSource extends GenericXmlBikeRentalDataSource {
	...
	public BikeRentalStation makeStation(Map<String, String> attributes) {
		...
		String[] coordinates = attributes.get("ms:msGeometry").trim()
				.split(" ");
		...
		boolean connected = "CONNECTEE".equalsIgnoreCase(attributes
				.get("ms:ETAT"));
		...
		/*
		 * Please see http://www.vcub.fr/stations-vcub-1 for more information on
		 * rules of VCUB vs VCUB+. Apparently both network are compatible, VCUB+
		 * only offer more renting options which are not handled by OTP anyway.
		 */
