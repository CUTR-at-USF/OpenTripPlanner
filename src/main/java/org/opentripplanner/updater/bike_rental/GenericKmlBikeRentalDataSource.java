...
/**
 * Load bike rental stations from a KML placemarks. Use name as bike park name
 * and point coordinates. Rely on: 1) bike park to be KML Placemarks, 2)
 * geometry to be Point.
 */
public class GenericKmlBikeRentalDataSource extends
		GenericXmlBikeRentalDataSource {

	private static final Logger LOG = LoggerFactory
			.getLogger(GenericKmlBikeRentalDataSource.class);
	...
	/**
	 * @param namePrefix
	 *            A string to prefix all station names coming from this source
	 *            (for example: "OV-fietspunt "). Please add a space at the end
	 *            if needed.
	 */
	...
	/**
	 * @param networks
	 *            A network, or a comma-separated list of networks, to set to
	 *            all stations from the dataSource. Default to null (compatible
	 *            with all).
	 */
	...
	/**
	 * @param allowDropoff
	 *            True if the bike rental stations coming from this source
	 *            allows bike dropoff. True by default.
	 */
	...
	public BikeRentalStation makeStation(Map<String, String> attributes) {
		...
		brStation.id = String.format(Locale.US, "%s[%.3f-%.3f]",
				brStation.name.replace(" ", "_"), brStation.x, brStation.y);
