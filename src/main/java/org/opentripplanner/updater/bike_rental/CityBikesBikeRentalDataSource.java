...
public class CityBikesBikeRentalDataSource implements BikeRentalDataSource,
	...
	private static final Logger log = LoggerFactory
			.getLogger(BixiBikeRentalDataSource.class);
	...
	public boolean update() {
		try {
			...
			log.warn("Error parsing bike rental feed from " + url
					+ "(bad XML of some sort)", e);
			return false;
		}
		return true;
	}

	private void parseJson(String data) throws ParserConfigurationException,
		...
		for (JsonNode stationNode : mapper.readTree(data)) {
			...
			// We need string IDs but they are in JSON as numbers. Avoid null
			// from textValue(). See pull req #1450.
