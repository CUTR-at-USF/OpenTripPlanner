...
/**
 * Configure/decorate a graph upon loading through Preferences (Preference is
 * the new Java API replacing "Properties"). Usually preferences are loaded from
 * a .properties files, but could also come from the graph itself or any other
 * sources.
 * 
 * When a graph is loaded, client should call setupGraph() with the Preferences
 * setup.
 * 
 * When a graph is unloaded, one must ensure the shutdownGraph() method is
 * called to cleanup all resources that could have been created.
 * 
 * This class then creates "graph updaters" (usually real-time connector,
 * etc...) depending on the given configuration, and configure them using the
 * corresponding children Preferences node.
 * 
 * If an embedded configuration is present in the graph, we also try to use it.
 * In case of conflicts between two child nodes in both configs (two childs node
 * with the same name) the dynamic (ie provided) configuration takes complete
 * precedence over the embedded one: childrens properties are *not* merged.
 * 
 */
public class GraphUpdaterConfigurator {

	private static Logger LOG = LoggerFactory
			.getLogger(GraphUpdaterConfigurator.class);
	...
	public void setupGraph(Graph graph, Preferences mainConfig) {
		...
		LOG.info("Using configurations: "
				+ (mainConfig == null ? "" : "[main]") + " "
	...
	/**
	 * Apply a list of configs to a graph. Please note that the order of the
	 * config in the list *is important* as a child node already seen will not
	 * be overriden.
	 * 
	 * @param graph
	 * @param updaterManager
	 *            is the graph updater manager to which all updaters should be
	 *            added
	 * @param configs
	 *            is the list of configs.
					...
					try {
						...
						if (updater == null) {
							...
							if (updater instanceof PreferencesConfigurable) {
								((PreferencesConfigurable) updater).configure(
										graph, prefs);
							}
	...
	public void shutdownGraph(Graph graph) {
		...
		if (updaterManager != null) {
			LOG.info("Stopping updater manager with " + updaterManager.size()
					+ " updaters.");
			updaterManager.stop();
		}
	}
