 ...
 * Besides implementing the methods of the interface PollingGraphUpdater, the
 * updater also needs to be registered in the function
 * GraphUpdaterConfigurator.applyConfigurationToGraph.
 * 
 * This example is suited for polling updaters. For streaming updaters (aka push
 * updaters) it is better to use the GraphUpdater interface directly for this
 * purpose. The class ExampleGraphUpdater shows an example of how to implement
 * this.
 * 
 * Usage example ('polling-example' name is an example) in file
 * 'Graph.properties':
 * 
 * <pre>
...
public class ExamplePollingGraphUpdater extends PollingGraphUpdater {
	...
	private static Logger LOG = LoggerFactory
			.getLogger(ExamplePollingGraphUpdater.class);
	...
	// 'Graph.properties'.
	// The property frequencySec is already read and used by the abstract base
	// class.
	@Override
	protected void configurePolling(Graph graph, Preferences preferences)
		...
		LOG.info(
				"Configured example polling updater: frequencySec={} and url={}",
				frequencySec, url);
	}
	...
	protected void runPolling() {
		LOG.info("Run example polling updater with hashcode: {}",
				this.hashCode());
	...
	// This is a private GraphWriterRunnable that can be executed to modify the
	// graph
