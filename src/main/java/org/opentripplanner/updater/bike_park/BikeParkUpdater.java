 ...
 * 
 * Usage example ('fietsstalling' name is an example) in the file
 * 'Graph.properties':
 * 
 * <pre>
...
public class BikeParkUpdater extends PollingGraphUpdater {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(BikeParkUpdater.class);
	...
	protected void configurePolling(Graph graph, Preferences preferences)
		...
		if (source == null) {
			throw new IllegalArgumentException(
					"Unknown bike rental source type: " + sourceType);
		...
		this.graph = graph;
		this.source = source;
		LOG.info("Creating bike-park updater running every {} seconds : {}",
				frequencySec, source);
	}
	...
	public void setup() throws InterruptedException, ExecutionException {
		...
		updaterManager.executeBlocking(new GraphWriterRunnable() {
			...
			public void run(Graph graph) {
				bikeService = graph.getService(BikeRentalStationService.class,
						true);
			}
		});
	}
	...
	protected void runPolling() throws Exception {
		...
		BikeParkGraphWriterRunnable graphWriterRunnable = new BikeParkGraphWriterRunnable(
				bikeParks);
	...
	private class BikeParkGraphWriterRunnable implements GraphWriterRunnable {
		...
		public void run(Graph graph) {
			...
			for (Entry<BikePark, BikeParkVertex> entry : verticesByPark
					.entrySet()) {
