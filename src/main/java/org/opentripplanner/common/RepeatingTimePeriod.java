...
/**
 * Represents a repeating time period, used for opening hours &c. For instance:
 * Monday - Friday 8AM to 8PM, Satuday 10AM to 5PM, Sunday closed. For now it is
 * week-based so doesn't handle every possible case, but since it is
 * encapsulated that could conceivably be changed.
 * 
 * @author mattwigway
 *
 */
public class RepeatingTimePeriod implements Serializable {
	...
	/**
	 * This stores the time periods this is active/open, stored as seconds from
	 * noon (positive or negative) on the given day.
	 */
	private int[][] monday;
	private int[][] tuesday;
	...
	public static RepeatingTimePeriod parseFromOsmTurnRestriction(
			String day_on, String day_off, String hour_on, String hour_off) {
		// first, create the opening and closing times. This is easy because
		...
		for (int i = 0; i < parsedOn.length; i++) {
			onOff[i] = new int[] { parseHour(parsedOn[i]),
					parseHour(parsedOff[i]) };
		}
		...
		for (String today : new String[] { "monday", "tuesday", "wednesday",
	...
	private static int parseHour(String hour) {
		...
		// subtract 12 hours to make it noon-relative. This implicitly handles
		// DST.
		ret -= 12 * 3600;
	...
	public boolean active(long time) {
		...
		int timeOfDay = cal.get(Calendar.HOUR_OF_DAY) * 3600
				+ cal.get(Calendar.MINUTE) * 60 + cal.get(Calendar.SECOND) - 12
				* 3600;
