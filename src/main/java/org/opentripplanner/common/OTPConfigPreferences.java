...
/**
 * A read-only implementation of the Java Preferences API (hierarchically
 * structured configuration information). The preferences are loaded from a text
 * configuration file where every line is either a key-value pair (where the key
 * is separated from the value by whitespace characters), a child node name
 * followed by whitespace and an opening bracket, or a single closing bracket.
 * For example:
 *
 * <pre>
...
public class OTPConfigPreferences extends AbstractPreferences {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(OTPConfigPreferences.class);
	...
	private static void parseError(String msg, String filename, int line) {
		String longmsg = String.format(
				"parse error in config file '%s' at line %d:\n    %s",
				filename, line, msg);
		LOG.error(longmsg);
	}
	...
	public static OTPConfigPreferences fromFile(String filename) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(filename))) {
			...
			while ((line = br.readLine()) != null) {
				nline++;
				String[] tokens = line.trim().split("\\s+", 2); // split on one
																// or more
																// whitespace
																// characters
				...
				if (tokens.length > 2) {
					parseError(
							"line must contain a key-value pair, a child name and an opening bracket, "
									+ "or a single closing bracket.", filename,
							nline);
					return null;
				}
				...
				if (key.equals("}")) {
					if (tokens.length > 1) {
						parseError(
								"Closing bracket should appear alone on a line.",
								filename, nline);
						return null;
					}
					if (stack.isEmpty()) {
						parseError(
								"Bracket mismatch (too many closing brackets).",
								filename, nline);
						return null;
					}
					curr = stack.pop();
				...
				if (key.endsWith("{")) {
					parseError(
							"An opening bracket should be preceded by a child name and whitespace.",
							filename, nline);
					return null;
				}
				if (val.startsWith("{") && val.length() > 1) {
					parseError(
							"The opening bracket should be the last element on a line.",
							filename, nline);
					return null;
				}
				if ("{".equals(val)) {
					...
					if (child != null) {
						parseError(String.format(
								"Multiple definitions of child '%s'.", key),
								filename, nline);
						return null;
					}
					...
					if (curr.entries.get(key) != null) {
						parseError(String.format(
								"Multiple definitions of key '%s'.", key),
								filename, nline);
						return null;
					}
			...
			if (!stack.isEmpty()) {
				parseError("Bracket mismatch (not enough closing brackets).",
						filename, nline);
				return null;
			}
			return curr;
	...
	protected void putSpi(String s, String s2) {
		throw new UnsupportedOperationException(
				"read-only preferences implementation.");
	}
	...
	private String toString(int depth) {
		...
		for (Map.Entry<String, OTPConfigPreferences> child : children
				.entrySet()) {
			sb.append(indent);
