...
public class StreetUtils {
	...
	public static void pruneFloatingIslands(Graph graph, int maxIslandSize,
		...
		if (islandLogName != null && !islandLogName.isEmpty()) {
			try {
				...
				LOG.error("Failed to write islands log file due to {}",
						e.toString());
				e.printStackTrace();
			}
		}
		...
		// RoutingRequest options = new RoutingRequest(new
		// TraverseModeSet(TraverseMode.WALK, TraverseMode.TRANSIT));
		...
		for (Vertex gv : graph.getVertices()) {
			...
			Subgraph subgraph = computeConnectedSubgraph(neighborsForVertex,
					vertex);
			if (subgraph != null) {
				for (Iterator<Vertex> vIter = subgraph.streetIterator(); vIter
						.hasNext();) {
	...
	private static void depedestrianizeOrRemove(Graph graph, Subgraph island) {
		...
		LOG.debug(graph.addBuilderAnnotation(new GraphConnectivity(island
				.getRepresentativeVertex(), island.streetSize())));
	}
	...
	private static void WriteNodesInSubGraph(Subgraph subgraph,
			PrintWriter islandLog, boolean hadRemoved) {
		...
		islandLog.printf("%d\t%d\t%d\t%s\t%b\n", islandCounter,
				subgraph.stopSize(), subgraph.streetSize(), convexHullGeom,
				hadRemoved);
		islandCounter++;
	}
}