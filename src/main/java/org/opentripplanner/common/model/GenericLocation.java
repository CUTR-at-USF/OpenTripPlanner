...
/**
 * Class describing a location provided by clients of routing. Used to describe
 * end points (origin, destination) of a routing request as well as any
 * intermediate points that should be passed through.
 * 
 * Handles parsing of geospatial information from strings so that it need not be
 * littered through the routing code.
 * 
 * @author avi
 */
public class GenericLocation implements Cloneable {
	...
	/**
	 * The identifier of the place, if provided. May be a lat,lng string or a
	 * vertex ID.
	 */
	 ...
	 * true north.
	 * 
	 * 0 = heading true north. +/-180 = heading south.
	 */
	public Double heading;

	// Pattern for matching lat,lng strings, i.e. an optional '-' character
	// followed by
	...
	// We want to ignore any number of non-digit characters at the beginning of
	// the string, except
	// that signs are also non-digits. So ignore any number of non-(digit or
	// sign or decimal point).
	private static final Pattern _latLonPattern = Pattern
			.compile("[^[\\d&&[-|+|.]]]*(" + _doublePattern
					+ ")(\\s*,\\s*|\\s+)(" + _doublePattern + ")\\D*");

	private static final Pattern _headingPattern = Pattern
			.compile("\\D*heading=(" + _doublePattern + ")\\D*");

	private static final Pattern _edgeIdPattern = Pattern
			.compile("\\D*edgeId=(\\d+)\\D*");
	...
	/**
	 * Construct from a name, place pair. Parses latitude, longitude data,
	 * heading and numeric edge ID out of the place string. Note that if the
	 * place string does not appear to contain a lat/lon pair, heading, or edge
	 * ID the GenericLocation will be missing that information but will still
	 * retain the place string, which will be interpreted during routing context
	 * construction as a vertex label within the graph for the appropriate
	 * routerId (by StreetVertexIndexServiceImpl.getVertexForLocation()). TODO:
	 * Perhaps the interpretation as a vertex label should be done here for
	 * clarity.
	 */
	...
	/**
	 * Creates the GenericLocation by parsing a "name::place" string, where
	 * "place" is a latitude,longitude string or a vertex ID.
	 * 
	 * @param input
	 * @return
	 */
	...
	/**
	 * Represents the location as an old-style string for clients that relied on
	 * that behavior.
	 * 
	 * TODO(flamholz): clients should stop relying on these being strings and
	 * then we can return a string here that fully represents the contents of
	 * the object.
	 */
	@Override
	...
	/**
	 * Returns a descriptive string that has the information that I wish
	 * toString() returned.
	 */
	public String toDescriptiveString() {
		StringBuilder sb = new StringBuilder();
		sb.append("<GenericLocation lat,lng=").append(this.lat).append(",")
				.append(this.lng);
		if (this.hasHeading()) {
