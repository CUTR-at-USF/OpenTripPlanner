...
/**
 * Compute isoline based on a zFunc and a set of initial coverage points
 * P0={(x,y)} to seed the computation.
 * 
 * This assume we have a z = Fz(x,y) function which can gives a z value for any
 * given point (x,y), z can be undefined for some region.
 * 
 * There are many tricks to reduce to the minimum the numbers of Fz samplings,
 * explained in the code below.
 * 
 * It will compute an isoline for a given z0 value. The isoline is composed of a
 * list of n polygons, CW for normal polygons, CCW for "holes". The isoline
 * computation can be called multiple times on the same builder for different z0
 * values: this will reduce the number of Fz sampling as they are cached in the
 * builder. Please note that the initial covering points must touch all isolines
 * you want to cover.
 * 
 * @author laurent
 */
public class RecursiveGridIsolineBuilder {
	...
	private static class XYIndex {
		...
		public final boolean equals(Object other) {
			if (other instanceof XYIndex) {
				...
				return otherTileIndex.xIndex == this.xIndex
						&& otherTileIndex.yIndex == this.yIndex;
			}
			return false;
		}
	...
	private static class GridEdge {
		...
		private GridEdge(GridDot A, GridDot B, int size, boolean horizontal) {
			// We accept A and B in any order. They must be correctly placed
			// however
		...
		@Override
		public final String toString() {
			return "[" + (horizontal ? "H" : "V") + "-Edge" + A + "-" + B
					+ " L=" + size + "]";
		}
	}

	private static final Logger LOG = LoggerFactory
			.getLogger(RecursiveGridIsolineBuilder.class);

	/**
	 * Size (in index-unit) of initial (seed) sampling. This *MUST* be a power
	 * of 2. Good values are 4 (slower but miss less small islands) or 8 (faster
	 * but miss more small islands).
	 */
	...
	/**
	 * Create an object to compute isochrones. One may call several time
	 * isochronify on the same IsoChronificator object, this will re-use the z =
	 * f(x,y) sampling if possible, as they are kept in cache.
	 * 
	 * @param request
	 *            Parameters for the computation
	 * @param center
	 *            Center point (eg origin)
	 * @param fz
	 *            Function returning the z-value for a xy-coordinate
	 * @param p0List
	 *            Initial set of coverage points to seed the heuristics
	 */
	public RecursiveGridIsolineBuilder(double dX, double dY, Coordinate center,
			ZFunc fz, List<Coordinate> p0List) {

		this.dX = dX;
		this.dY = dY;
		/*
		 * Center position only purpose is to serve as a reference value to the
		 * XY integer indexing, so it only needs to be not too far off to
		 * prevent int indexes from overflowing.
		 */
		this.center = center;
		...
		/*
		 * Step 2. SEED (2). Create initial edges from initial dots. There will
		 * be slightly less edges than dots.
		 */
		...
		LOG.debug("Created {} dots and {} edges out of {} initial points.",
				initialDots.size(), initialEdges.size(), p0List.size());
	}
	...
	public Geometry computeIsoline(long z0) {
		...
		/*
		 * Step 3. DIVIDE. While there are cutting edges from size > 1 to
		 * divide, divide them in two. Note: edgesToExpand only contains cutting
		 * edges.
		 */
		...
		while (!edgesToDivide.isEmpty()) {
			...
			if (e.cut(z0) == 0)
				continue;
			int size2 = e.size / 2;
			XYIndex iC = e.horizontal ? e.A.index.translated(size2, 0)
					: e.A.index.translated(0, size2);
		...
		/*
		 * Step 4. EXPAND. While there are unprocessed cutting edges, expand
		 * them by looking around them for cutting edges.
		 */
		...
		while (!edgesToExpand.isEmpty()) {
			...
			/**
			 * Build the 6 remaining edges (e1..e6) around the 2 squares ABDC
			 * and DBFE touching e
			 * 
			 * <pre>
			 ...
			 * </pre>
			 */
			XYIndex iC = e.horizontal ? e.A.index.translated(0, 1) : e.A.index
					.translated(-1, 0);
			XYIndex iD = e.horizontal ? e.B.index.translated(0, 1) : e.B.index
					.translated(-1, 0);
			XYIndex iE = e.horizontal ? e.A.index.translated(0, -1) : e.A.index
					.translated(1, 0);
			XYIndex iF = e.horizontal ? e.B.index.translated(0, -1) : e.B.index
					.translated(1, 0);
			GridDot C = getOrCreateDot(iC);
			GridDot D = getOrCreateDot(iD);
			GridDot E = getOrCreateDot(iE);
			GridDot F = getOrCreateDot(iF);
			GridEdge[] e2List = new GridEdge[] {
					new GridEdge(e.A, C, 1, !e.horizontal),
					new GridEdge(C, D, 1, e.horizontal),
					new GridEdge(e.B, D, 1, !e.horizontal),
					new GridEdge(e.A, E, 1, !e.horizontal),
					new GridEdge(E, F, 1, e.horizontal),
		...
		/*
		 * Note: Here finalEdges only contains cutting edges, and should end up
		 * containing all cutting edges that are discoverable.
		 */
		...
		if (debugSeedGrid) {
			for (GridEdge e : initialEdges) {
				...
				debugGeom.add(geomFactory.createLineString(new Coordinate[] {
						A, B }));
			}
		...
		if (debugCrossingEdges) {
			for (GridEdge e : finalEdges) {
				...
				Coordinate C1 = new Coordinate(C.x + (B.y - A.y) * 0.1, C.y
						+ (B.x - A.x) * 0.1);
				Coordinate C2 = new Coordinate(C.x - (B.y - A.y) * 0.1, C.y
						- (B.x - A.x) * 0.1);
				debugGeom.add(geomFactory.createLineString(new Coordinate[] {
						A, C, C1, C2, C, B }));
			}
		...
		while (!finalEdges.isEmpty()) {
			...
			int cut = e0.cut(z0);
			Direction direction = e0.horizontal ? (cut > 0 ? Direction.UP
					: Direction.DOWN) : (cut > 0 ? Direction.LEFT
					: Direction.RIGHT);
			GridEdge e = e0;
			while (true) {
				...
				if (ok1 && ok2) {
					/*
					 * We can go either turn, pick the best one from e1 or e2 by
					 * looking if C lies closer to e.A or e.B. Please note this
					 * is approximate only, as we should take into account real
					 * interpolated position on e1 and e2 to compute segment
					 * lenght. But this gives good approximated results and is
					 * probably sufficient given the approximated solution
					 * anyway.
					 */
					double dA = Math.max(Math.abs(cA.x - cC.x),
							Math.abs(cA.y - cC.y));
					double dB = Math.max(Math.abs(cB.x - cC.x),
							Math.abs(cB.y - cC.y));
					if (dA <= dB) {
						// C closer to A
						e = e1;
		...
		LOG.info(
				"Isochrones: {}+{} Fz samples, {} cutting edges, {} non-cutting edges",
				allDots.size(), fzInterpolateCount, finalEdgesSize,
				finalNonCuttingEdgesSize);
		...
		return geomFactory.createGeometryCollection(retval
				.toArray(new Geometry[retval.size()]));
	}
	...
	private final Coordinate getCoordinate(XYIndex index) {
		return new Coordinate(index.xIndex * dX + center.x, index.yIndex * dY
				+ center.y);
	}
	...
	private final Coordinate interpolate(Coordinate A, Coordinate B, long zA,
			long zB, long z0) {
		int n = 0;
		...
		Coordinate C = new Coordinate(A.x * (1.0 - k) + B.x * k, A.y
				* (1.0 - k) + B.y * k);
		return C;
	}

	@SuppressWarnings("unchecked")
	private final List<Polygon> punchHoles(GeometryFactory geomFactory,
		...
		for (Polygon shell : shells) {
			List<LinearRing> shellHoles = ((List<LinearRing>) shell
					.getUserData());
			punched.add(geomFactory.createPolygon(
