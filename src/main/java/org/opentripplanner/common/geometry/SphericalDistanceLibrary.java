...
public class SphericalDistanceLibrary implements DistanceLibrary {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.opentripplanner.common.geometry.DistanceLibrary#distance(com.
	 ...
	 * 
	 * @see
	 * org.opentripplanner.common.geometry.DistanceLibrary#fastDistance(com.
	 * vividsolutions.jts.geom.Coordinate,
	 * com.vividsolutions.jts.geom.Coordinate)
	 */
	@Override
	...
	public final double fastDistance(Coordinate point, LineString lineString) {
		...
		Point point2 = GeometryUtils.getGeometryFactory().createPoint(
	...
	public final double fastLength(LineString lineString) {
		...
		return equirectangularProject(lineString, cosLat).getLength()
	 ...
	 */
	private LineString equirectangularProject(LineString lineString,
			double cosLat) {
	...
	/**
	 * @see org.opentripplanner.common.geometry.DistanceLibrary#distance(double,
	 *      double, double, double)
	 */
	@Override
	public final double distance(double lat1, double lon1, double lat2,
			double lon2) {
		return distance(lat1, lon1, lat2, lon2, RADIUS_OF_EARTH_IN_M);
	}

	/**
	 * @see org.opentripplanner.common.geometry.DistanceLibrary#fastDistance(double,
	 *      double, double, double)
	 */
	@Override
	public final double fastDistance(double lat1, double lon1, double lat2,
			double lon2) {
	...
	public final double distance(double lat1, double lon1, double lat2,
		...
		double y = sqrt(p2(cos(lat2) * sin(deltaLon))
				+ p2(cos(lat1) * sin(lat2) - sin(lat1) * cos(lat2)
						* cos(deltaLon)));
		double x = sin(lat1) * sin(lat2) + cos(lat1) * cos(lat2)
				* cos(deltaLon);
	...
	/**
	 * Approximated, fast and under-estimated equirectangular distance between
	 * two points. Works only for small delta lat/lon, fall-back on exact
	 * distance if not the case. See:
	 ...
	 *            Distance in meters.
	 * @return The number of degree for the given distance. For a latitude, this
	 *         is exact. For a longitude, this is an overestimate.
	 */
	 ...
	 *            Distance in meters.
	 * @param latDeg
	 *            Latitude of center point, in degree.
	 * @return The number of longitude degree for the given distance. This is a
	 *         slight overestimate as the number of degree of longitude for a
	 *         given distance depends on the exact latitude.
	 */
	public static double metersToLonDegrees(double distanceMeters, double latDeg) {
		double dLatDeg = 360 * distanceMeters
				/ (2 * Math.PI * RADIUS_OF_EARTH_IN_M);
		/*
		 * The computation below ensure that minCosLat is the minimum value of
		 * cos(lat) for lat in the range [lat-dLat, lat+dLat].
		 */
		double minCosLat;
		if (latDeg > 0) {
	...
	public final Envelope bounds(double lat, double lon, double latDistance,
		...
		return new Envelope(new Coordinate(lonFrom, latFrom), new Coordinate(
				lonTo, latTo));
	}
