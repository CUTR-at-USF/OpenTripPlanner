...
public final class SparseMatrixZSampleGrid<TZ> implements ZSampleGrid<TZ>,
	...
	private final class SparseMatrixSamplePoint implements ZSamplePoint<TZ>,
			DelaunayPoint<TZ> {
	...
	private final class GridDelaunayEdge implements DelaunayEdge<TZ> {
		...
		private GridDelaunayEdge(SparseMatrixSamplePoint A,
				SparseMatrixSamplePoint B, int type) {
			this.A = A;
			this.B = B;
			switch (type) {
		...
		public String toString() {
			return "<GridDelaunayEdge " + A.getCoordinates() + "->"
					+ B.getCoordinates() + ">";
		}
	 ...
	 * @param dX
	 *            X grid size, same units as center coordinates.
	 * @param dY
	 *            Y grid size, same units as center coordinates.
	 * @param center
	 *            Center position of the grid. Do not need to be precise.
	 */
	public SparseMatrixZSampleGrid(int chunkSize, int totalSize, double dX,
			double dY, Coordinate center) {
		this.center = center;
		this.dX = dX;
		this.dY = dY;
		this.chunkSize = chunkSize;
		allSamples = new SparseMatrix<SparseMatrixSamplePoint>(chunkSize,
				totalSize);
	}
	...
	public Iterator<ZSamplePoint<TZ>> iterator() {
		return new Iterator<ZSamplePoint<TZ>>() {
			...
			private Iterator<SparseMatrixSamplePoint> iterator = allSamples
					.iterator();
	...
	public Coordinate getCoordinates(ZSamplePoint<TZ> point) {
		...
		return new Coordinate(point.getX() * dX + center.x, point.getY() * dY
				+ center.y);
	}
	...
	/**
	 * The conversion from a grid of points to a Delaunay triangulation is
	 * trivial. Each square from the grid is cut through one diagonal in two
	 * triangles, the resulting output is a Delaunay triangulation.
	 */
	private void delaunify() {
		...
		for (SparseMatrixSamplePoint A : allSamples) {
			...
			SparseMatrixSamplePoint C = (SparseMatrixSamplePoint) (B != null ? B
					.up() : D != null ? D.right() : null);
			if (B != null)
				triangulation.add(new GridDelaunayEdge(A, B,
						GridDelaunayEdge.TYPE_HORIZONTAL));
			if (D != null)
				triangulation.add(new GridDelaunayEdge(A, D,
						GridDelaunayEdge.TYPE_VERTICAL));
			if (C != null)
				triangulation.add(new GridDelaunayEdge(A, C,
						GridDelaunayEdge.TYPE_DIAGONAL));
		}
		// 2. Link edges
