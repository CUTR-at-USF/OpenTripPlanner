...
/**
 * Variable-length integer encoding. This optimize integer storage when most of
 * the values are small, but few of them can be quite large (as in a geometry).
 * Adapted Dlugosz scheme to support signed int whose average are around 0.
 * 
 * See Dlugosz' variable-length integer encoding
	...
	public static int[] unpack(byte[] arr) {
		...
		while (i < arr.length) {
			...
			if ((v1 & 0x80) == 0x00) {
				...
				// 110 xxxx + 2x8 -> 21 bits value
				int sv = ((v1 & 0x1F) << 16) + ((arr[i] & 0xFF) << 8)
						+ (arr[i + 1] & 0xFF) - 1048576;
				i += 2;
				retval.add(sv);
			} else if ((v1 & 0xF8) == 0xE0) {
				// 1110 0xxx + 3x8 -> 27 bits value
				int sv = ((v1 & 0x1F) << 24) + ((arr[i] & 0xFF) << 16)
						+ ((arr[i + 1] & 0xFF) << 8) + (arr[i + 2] & 0xFF)
						- 67108864;
				i += 3;
				retval.add(sv);
			} else {
