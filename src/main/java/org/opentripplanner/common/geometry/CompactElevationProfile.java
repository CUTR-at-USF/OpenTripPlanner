 ...
 * </ul>
 * 
 * Performance hit should be low as we do not need the elevation profile itself
...
public final class CompactElevationProfile implements Serializable {
	...
	/**
	 * Multipler for fixed-float representation. In meters, the precision is 1
	 * cm (elevation and arc length).
	 */
	private static final double FIXED_FLOAT_MULT = 1.0e2;

	/**
	 * Compact an elevation profile onto a var-len int packed form (Dlugosz
	 * coding).
	 * 
	 * @param profile
	 *            The elevation profile to compact
	...
	/**
	 * Uncompact an ElevationProfile from a var-len int packed form (Dlugosz
	 * coding).
	 * 
	 ...
	 */
	public static PackedCoordinateSequence uncompactElevationProfile(
			byte[] packedCoords) {
