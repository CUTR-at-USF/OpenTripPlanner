 ...
 * 
 * Objects to index are placed in all grid bins touching the bounding envelope.
 * We *do not store* any bouding envelope for each object, so this imply that we
 * will return false positive when querying, and it's up to the client to filter
 * them out (with whatever knowledge it has on the location of the object).
 * 
 * Note: For performance reasons, write operation are not synchronized, it must
 * be taken care by the client. Read-only operation are multi-thread-safe
 * though.
 * 
 * @author laurent
 * 
 * @param <T>
 *            Type of objects to be spatial indexed.
 */
public class HashGridSpatialIndex<T> implements SpatialIndex, Serializable {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(HashGridSpatialIndex.class);
	...
	/*
	 * The map of all bins. Please see visit() and xKey/yKey for details on the
	 * key.
	 */
	...
	public final void insert(Envelope envelope, final Object item) {
		visit(envelope, true, new BinVisitor<T>() {
			...
			public boolean visit(List<T> bin, long mapKey) {
				/*
				 * Note: here we can end-up having several time the same object
				 * in the same bin, if the client insert multiple times the same
				 * object with different envelopes. However we do filter
				 * duplicated when querying, so apart for memory/performance
				 * reasons it should work. If this becomes a problem, we can use
	 ...
	 *            Self-descripting.
	 * @param createIfEmpty
	 *            Create a new bin if not existing.
	 * @param binVisitor
	 *            The callback to call for each visited bin.
	 */
	private void visit(Envelope envelope, boolean createIfEmpty,
			final BinVisitor<T> binVisitor) {
		...
		for (long xKey = minXKey; xKey <= maxXKey; xKey++) {
			for (long yKey = minYKey; yKey <= maxYKey; yKey++) {
				/*
				 * For all known use, the average absolute value of x/y keys
				 * will be rather small compared to Integer.MAX_VALUE. We need
				 * to swap the two words (MSB and LSB) of xKey in order to have
				 * a well-behaving long hash, fitting in an int, because the
				 * default implementation is: hashInt = (int)(value ^ (value >>>
				 * 32));
				 */
				long mapKey = (yKey << 32) | ((xKey & 0xFFFF) << 16)
						| ((xKey >> 16) & 0xFFFF);
	...
	public String toString() {
		return String
				.format("HashGridSpatialIndex %f x %f, %d bins allocated, %d objs, %d entries (avg %.2f entries/bin, %.2f entries/object)",
						this.xBinSize, this.yBinSize, this.nBins,
						this.nObjects, this.nEntries, this.nEntries * 1.0
								/ this.nBins, this.nEntries * 1.0
								/ this.nObjects);
	}
}
