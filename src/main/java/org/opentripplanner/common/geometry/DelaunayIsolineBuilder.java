 ...
 * value: this will reduce the number of Fz sampling as they are cached in the
 * builder, and reduce the number of time the Delaunay triangulation has to be
 * built.
 * 
 * The algorithm is rather simple: for each edges of the triangulation check if
 * the edge is "cutting" (ie crossing the z0 plane). Then start for each
 * unprocessed cutting edge using a walk algorithm, keeping high z0 always one
 * the same side, to build a set of closed polygons. Then process each polygons
 * to punch holes: a CW polygon is a hole in a larger CCW polygon, a CCW polygon
 * is an islan (shell).
 * 
 * @author laurent
 */
public class DelaunayIsolineBuilder<TZ> implements IsolineBuilder<TZ> {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(DelaunayIsolineBuilder.class);
	...
	/**
	 * Create an object to compute isolines. One may call several time
	 * computeIsoline on the same object, with different z0 values.
	 * 
	 * @param triangulation
	 *            The triangulation to process. Must be closed (no edge at the
	 *            border should intersect).
	 * @param zMetric
	 *            The Z metric (intersection detection and interpolation
	 *            method).
	 */
	public DelaunayIsolineBuilder(DelaunayTriangulation<TZ> triangulation,
			ZMetric<TZ> zMetric) {
	...
	public Geometry computeIsoline(TZ z0) {
		...
		while (!processQ.isEmpty()) {
			...
			while (true) {
				...
				double k = zMetric.interpolate(e.getA().getZ(),
						e.getB().getZ(), z0);
				Coordinate cC = new Coordinate(cA.x * (1.0 - k) + cB.x * k,
						cA.y * (1.0 - k) + cB.y * k);
				polyPoints.add(cC);
				e.setProcessed(true);
				DelaunayEdge<TZ> E1 = e.getEdge1(ccw);
				DelaunayEdge<TZ> E2 = e.getEdge2(ccw);
				int cut1 = E1 == null ? 0 : zMetric.cut(E1.getA().getZ(), E1
						.getB().getZ(), z0);
				int cut2 = E2 == null ? 0 : zMetric.cut(E2.getA().getZ(), E2
						.getB().getZ(), z0);
		...
		return geometryFactory.createGeometryCollection(retval
	...
	private final void generateDebugGeometry(TZ z0) {
		...
		for (DelaunayEdge<TZ> e : triangulation.edges()) {
			Coordinate cA = e.getA().getCoordinates();
			Coordinate cB = e.getB().getCoordinates();
			debugGeom.add(geometryFactory.createLineString(new Coordinate[] {
					cA, cB }));
	...
	public final Geometry getDebugGeometry() {
		return geometryFactory.createGeometryCollection(debugGeom
	...
	private final List<Polygon> punchHoles(List<LinearRing> rings) {
		...
		for (Polygon shell : shells) {
			List<LinearRing> shellHoles = ((List<LinearRing>) shell
					.getUserData());
			punched.add(geometryFactory.createPolygon(
