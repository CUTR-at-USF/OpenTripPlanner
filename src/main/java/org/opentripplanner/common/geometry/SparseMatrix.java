...
/**
 * A fast sparse 2D matrix holding elements of type T. The x and y indexes into the sparse matrix
 * are _signed_ 32-bit integers (negative indexes are allowed). Square sub-chunks of size chunkSize
 * x chunkSize are stored in a hashmap, keyed on a combination of the x and y coordinates. Does not
...
public class SparseMatrix<T> implements Iterable<T> {
    ...
    int size = 0; // The number of elements currently stored in this matrix (number of cells
                  // containing a T).
    ...
    /**
     * @param chunkSize Must be a power of two so chunk indexes can be determined by shifting off
     *        low order bits. Keep it small (8, 16, 32...). Chunks are square, with this many
     *        elements in each of two dimensions, so the number of elements in each chunk will be
     *        the square of this value.
     * @param totalSize Estimated total number of elements to be stored in the matrix (actual use,
     *        not capacity).
     */
    public SparseMatrix(int chunkSize, int totalSize) {
        ...
        /*
         * Find log_2 chunkSize, the number of low order bits to shift off an index to get its chunk
         * index.
         */
        while (chunkSize > 1) {
            if (chunkSize % 2 != 0)
     ...
     * (each of which is a flattened two-dimensional array). We need to publish the internal
     * structure to be able to efficiently export it through web-services.
     */
    public class SparseMatrixChunk {

        @Getter
        public int x0, y0; // the coordinates of the minimum corner of this chunk (i.e. with the low
                           // bits)
    ...
    /**
     * We were previously bit-shifting two 32 bit integers into a long. These are used as map keys,
     * so they had to be Long objects rather than primitive long ints. This purpose-built key object
     * should be roughly the same in terms of space and speed, and more readable.
     */
    static class Key {
        int x, y;

        public Key(int x, int y, int shift) {
            this.x = x >>> shift; // shift off low order bits (index within chunk) retaining only
                                  // the chunk number
