...
public class MavenVersion implements Serializable {
	...
	private static MavenVersion fromProperties() {
		...
		try {
			...
			InputStream in = MavenVersion.class.getClassLoader()
			...
			LOG.info("Parsed Maven artifact version: {}",
					version.toStringVerbose());
			return version;
		} catch (Exception e) {
			LOG.error("Error reading version from properties file: {}",
					e.getMessage());
	...
	private MavenVersion() {
		// JAXB Marshalling requires classes to have a 0-arg constructor and
		// mutable fields.
		// otherwise it throws a
		// com.sun.xml.bind.v2.runtime.IllegalAnnotationsException.
		// It is protecting you against yourself, since you might someday want
		// to
		// unmarshal that same object in Java.
		// The 'proper' way of handling this is to make a mutable equivalent of
		// your class,
		// plus another adapter class that converts between the mutable and
		// immutable
		// versions. It would be absurd to produce so much boilerplate and
		// verbosity in this
		// situation, so I am providing a 0-arg constructor with a totally
		// different role:
		// generating a default version when OTP encounters a problem parsing
		// the
		// maven-version.properties file.
		this("0.0.0-ParseFailure", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN");
	}

	public MavenVersion(String version, String commit, String describe,
			String commit_time, String build_time) {
	...
	public String toString() {
		return String.format("MavenVersion(%d, %d, %d, %s, %s)", major, minor,
				incremental, qualifier, commit);
	}

	public String toStringVerbose() {
		return String.format("%s => %s UID=%d", version, this.toString(),
				getUID());
	}
