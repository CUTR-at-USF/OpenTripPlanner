 ...
 * https://svn.apache
 * .org/repos/asf/lucene/dev/trunk/lucene/demo/src/java/org/apache
 * /lucene/demo/IndexFiles.java
 */
public class LuceneIndex {
	...
	private QueryParser parser = new QueryParser(Version.LUCENE_47, "name",
			analyzer);
	...
	/**
	 * Index stations, stops, intersections, streets, and addresses by name and
	 * location.
	 */
	private void index() {
		try {
			...
			for (StreetVertex sv : Iterables.filter(
					graphIndex.vertexForId.values(), StreetVertex.class)) {
				addCorner(writer, sv);
			}
			writer.close();
	...
	private void addStop(IndexWriter iwriter, Stop stop) throws IOException {
		...
		doc.add(new StringField("category", Category.STOP.name(),
				Field.Store.YES));
		iwriter.addDocument(doc);
	}

	private void addCorner(IndexWriter iwriter, StreetVertex sv)
			throws IOException {
		String mainStreet = null;
		String crossStreet = null;
		// TODO score based on OSM street type, using intersection nodes instead
		// of vertices.
		for (StreetEdge pse : Iterables.filter(sv.getOutgoing(),
				StreetEdge.class)) {
			if (mainStreet == null)
	...
	/**
	 * Return a list of results in in the format expected by GeocoderBuiltin.js
	 * in the OTP Leaflet client.
	 */
	public List<LuceneResult> query(String queryString) {
		/*
		 * Turn the query string into a Lucene query. Terms are fuzzy and should
		 * all be present.
		 */
		BooleanQuery query = new BooleanQuery();
		for (String term : queryString.split(" ")) {
			query.add(new FuzzyQuery(new Term("name", term)),
	...
	/**
	 * This class matches the structure of the Geocoder responses expected by
	 * the OTP client.
	 */
