...
/**
 * This is a JCommander-annotated class that holds parameters for OTP
 * stand-alone mode. These parameters can be parsed from the command line, or
 * provided in a file using Jcommander's at-symbol syntax (see
 * http://jcommander.org/#Syntax). When stand-alone OTP is started as a daemon,
 * parameters are loaded from such a file, located by default in
 * '/etc/opentripplanner.cfg'.
 * 
 * Note that JCommander-annotated parameters can be any type that can be
 * constructed from a string. This module also contains classes for validating
 * parameters. See: http://jcommander.org/#Parameter_validation
 * 
 * Some parameter fields are not initialized so when inferring other parameters,
 * we can check for null and see whether they were specified on the command
 * line.
 * 
 * @author abyrd
 */
public class CommandLineParameters implements Cloneable {
	...
	@Parameter(names = { "--bindAddress" }, description = "the address of the network interface to bind to. defaults to all interfaces.")
	...
	// --basePath (rather than --graphs). Maybe just eliminate most short
	// options.
	...
	@Parameter(validateWith = ReadableFile.class, // the remaining parameters in
													// one array
	description = "files")
	List<File> files = new ArrayList<File>();

	@Parameter(names = { "--insecure" }, description = "allow unauthenticated access to sensitive resources, e.g. /routers")
	...
	public static class ReadableFile implements IParameterValidator {
		...
		public void validate(String name, String value)
			...
			if (!file.isFile()) {
				String msg = String.format("%s: '%s' is not a file.", name,
						value);
				throw new ParameterException(msg);
			}
			if (!file.canRead()) {
				String msg = String.format("%s: file '%s' is not readable.",
						name, value);
				throw new ParameterException(msg);
			}
		}
	...
	public static class ReadableDirectory implements IParameterValidator {
		...
		public void validate(String name, String value)
			...
			File file = new File(value);
			if (!file.isDirectory()) {
				String msg = String.format("%s: '%s' is not a directory.",
						name, value);
				throw new ParameterException(msg);
			}
			if (!file.canRead()) {
				String msg = String.format(
						"%s: directory '%s' is not readable.", name, value);
				throw new ParameterException(msg);
			}
		}
	}
	...
	public static class ReadWriteDirectory implements IParameterValidator {
		...
		public void validate(String name, String value)
			...
			if (!file.canWrite()) {
				String msg = String.format(
						"%s: directory '%s' is not writable.", name, value);
				throw new ParameterException(msg);
			}
		}
	}
	...
	public static class PositiveInteger implements IParameterValidator {
		...
		public void validate(String name, String value)
			...
			if (i <= 0) {
				String msg = String.format("%s must be a positive integer.",
						name);
				throw new ParameterException(msg);
			}
		}
	...
	public static class RouterId implements IParameterValidator {
		...
		public void validate(String name, String value)
			...
			if (!m.matches()) {
				String msg = String.format(
						"%s: '%s' is not a valid router ID.", name, value);
				throw new ParameterException(msg);
			}
		}
	}
