...
public class OTPConfigurator {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(OTPConfigurator.class);
	...
	/**
	 * We could even do this at Configurator construct time (rather than lazy
	 * initializing), using the inMemory param to create the right kind of
	 * GraphService ahead of time. However that would create indexes even when
	 * only a build was going to happen.
	 */
	...
	/**
	 * Create a cached GraphService that will be shared between all OTP
	 * components.
	 */
	public void makeGraphService(Graph graph) {
		...
		if (graph != null && (params.inMemory || params.preFlight)) {
			...
			try {
				FileInputStream graphConfiguration = new FileInputStream(
						params.graphConfigFile);
				Preferences config = new PropertiesPreferences(
						graphConfiguration);
				this.graphService.registerGraph("", new MemoryGraphSource("",
						graph, config));
			} catch (Exception e) {
		...
		if (params.routerIds.size() > 0 || params.autoScan) {
			/*
			 * Auto-register pre-existing graph on disk, with optional
			 * auto-scan.
			 */
	...
	public GraphBuilderTask builderFromParameters() {
		...
		if (hasOSM) {
			...
			osmBuilder
					.setDefaultWayPropertySetSource(defaultWayPropertySetSource);
		...
		if (hasGTFS) {
			...
			for (File gtfsFile : gtfsFiles) {
				...
				gtfsBundle
						.setTransfersTxtDefinesStationPaths(params.useTransfersTxt);
			...
			// When using the long distance path service, or when there is no
			// street data,
			// link stops to each other based on distance only, unless user has
			// requested linking
			...
			if ((!hasOSM) || params.longDistance) {
				if (!params.useTransfersTxt) {
					if (!hasOSM || !params.useStreetsForLinking) {
						graphBuilder
								.addGraphBuilder(new StreetlessStopLinker());
					}
				}
			}
			if (hasOSM) {
				graphBuilder
						.addGraphBuilder(new TransitToTaggedStopsGraphBuilderImpl());
				graphBuilder
						.addGraphBuilder(new TransitToStreetNetworkGraphBuilderImpl());
				// The stops can be linked to each other once they have links to
				// the street network.
				if (params.longDistance && params.useStreetsForLinking
						&& !params.useTransfersTxt) {
		...
		if (params.elevation) {
			...
			ElevationGridCoverageFactory gcf = new NEDGridCoverageFactoryImpl(
					cacheDirectory);
