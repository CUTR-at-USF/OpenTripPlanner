...
public class OTPServer {
	...
	public RoutingRequest routingRequest; // the prototype routing request which
											// establishes default parameter
											// values
	...
	public OTPServer(CommandLineParameters params, GraphService gs) {
		...
		// Choose a PathService to wrap the SPTService, depending on expected
		// maximum path lengths
		if (params.longDistance) {
			LongDistancePathService pathService = new LongDistancePathService(
					graphService, sptServiceFactory);
		...
		if (params.analyst) {
			...
			sampleGridRenderer = new SampleGridRenderer(graphService,
					sptServiceFactory);
			isoChroneSPTRenderer = new IsoChroneSPTRendererAccSampling(
					graphService, sptServiceFactory, sampleGridRenderer);
			surfaceCache = new SurfaceCache(30);
			pointSetCache = new DiskBackedPointSetCache(100, new File(
					params.pointSetDirectory));
		}

	}

	/**
	 * Return an HK2 Binder that injects this specific OTPServer instance into
	 * Jersey web resources. This should be registered in the ResourceConfig
	 * (Jersey) or Application (JAX-RS) as a singleton. More on custom injection
	 * in Jersey 2:
	 * http://jersey.576304.n2.nabble.com/Custom-providers-in-Jersey
	 * -2-tp7580699p7580715.html
	 */
