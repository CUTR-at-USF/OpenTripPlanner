...
/**
 * A sparse bit set capable of handling 64-bit int indexes (like OSM IDs). Each
 * block is 64 64-bit longs. At 512 bytes long, it tracks 4096 IDs. Node numbers
 * in OSM tend to be contiguous so maybe the blocks should be bigger. Disabled
 * debug statements have no measurable effect on speed (verified).
 */
public class NodeTracker {

	private static final Logger LOG = LoggerFactory
			.getLogger(NodeTracker.class);
	...
	/**
	 * The create parameter allows us to examine a block for a set bit without
	 * creating an empty block as a side effect.
	 */
	private long[] block(long x, boolean create) {
		...
		if (block == null && create) {
			...
			LOG.debug("Number of blocks is now {}, total {} MiB",
					blocks.size(), blocks.size() * 64 * 8 / 1024.0 / 1024.0);
		}
		LOG.debug("   block {}", key);
		return block;
	}
