...
/**
 * OTP representation of a subset of OpenStreetMap. One or more PBF files can be
 * loaded into this object, which serves as a simplistic database for fetching
 * and iterating over OSM elements.
 */
public class OSM {
	...
	public OSM(boolean diskBacked) {
		...
		if (diskBacked) {
			...
			DB db = DBMaker.newTempFileDB().transactionDisable()
					.asyncWriteEnable().compressionEnable().make();
	...
	public static OSM fromPBF(String pbfFile, Envelope env) {
		LOG.info("Reading PBF file '{}' filtering with envelope {}", pbfFile,
				env);
	...
	/**
	 * Find nodes referenced by more than one way. NodeTracker intersections
	 * will be null until this is called. MapDB TreeSets are much faster than
	 * MapDB HashSets, but in-memory NodeTrackers are much faster than MapDB
	 * TreeSets.
	 */
