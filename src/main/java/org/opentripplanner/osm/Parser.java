...
/**
 * Parser for the OpenStreetMap PBF Format. Implements callbacks for the
 * crosby.binary OSMPBF library. It loads OSM into the OTP model classes, then
 * defers to implementations of handleNode, handleWay, and handleRelation. Each
 * block of a PBF file can be of a different type, so if we want to examine
 * nodes, then ways we must parse the entire file several times. This is just
 * the nature of OSM PBF.
 *
 * Subclasses of Parser that wish to skip certain OSM element types should
 * override parseWays, parseDense, etc. rather than the corresponding handle*
 * methods to avoid ever converting the low-level PBF objects into objects using
 * OTP's internal OSM model.
 */
public abstract class Parser extends BinaryParser {
	...
	OSM osm;
	// no need to internalize strings. they will be serialized out to disk
	// anyway.
	// private Map<String, String> stringTable = new HashMap<String, String>();
	int nodeCount = 0;
	int wayCount = 0;

	private static final String[] retainKeys = new String[] { "highway",
			"parking", "bicycle" };
	...
	/**
	 * Note that in many PBF files this never gets called because nodes are
	 * dense.
	 */
	@Override
	...
	public void parse(Osmformat.HeaderBlock block) {
		for (String s : block.getRequiredFeaturesList()) {
			...
			throw new IllegalStateException("File requires unknown feature: "
					+ s);
		}
	}
	...
	/**
	 * Override this method to tell the parser what to do to with each way, once
	 * it has been parsed into OTP's internal OSM model.
	 */
