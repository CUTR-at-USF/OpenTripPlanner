...
/**
 * A Comparator implementation that will sort a 1D list of Vertex objects such
 * that 2D spatial locality is mostly preserved. It implicitly compares the
 * Morton code for the vertex's coordinates in a roughly area-preserving
...
public class MortonVertexComparator implements Comparator<Vertex> {
	...
	/*
	 * Specify the reference latitude for the projection. It is tempting to
	 * scale the x units using avg latitude of the two points during each
	 * comparison, but this will cause a vertex's projected x coordinate to vary
	 * slightly depending on which other vertex it is compared to. This noise
