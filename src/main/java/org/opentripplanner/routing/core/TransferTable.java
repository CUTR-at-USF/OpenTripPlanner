...
/**
 * This class represents all transfer information in the graph. Transfers are
 * grouped by stop-to-stop pairs. Each transfer may consist of multiple specific
 * transfers. See
 * https://developers.google.com/transit/gtfs/reference#transfers_fields and
 * https://support.google.com/transitpartners/answer/2450962 (heading
 * Route-to-route and trip-to-trip transfers) for more details about the
 * specifications.
 * 
...
public class TransferTable implements Serializable {
	...
	/**
	 * Preferred transfers (or timed transfers, which are preferred as well) are
	 * present if true
	 */
	...
	/**
	 * Get the transfer time that should be used when transferring from a trip
	 * to another trip. Note that this function does not check whether another
	 * specific transfer exists with the same specificity, what is forbidden by
	 * the specifications.
	 * 
	 * @param fromStop
	 *            is the arriving stop
	 * @param toStop
	 *            is the departing stop
	 * @param fromTrip
	 *            is the arriving trip
	 * @param toTrip
	 *            is the departing trip
	 * @param forwardInTime
	 *            is true when moving forward in time; false when moving
	 *            backwards in time (usually this will be the variable
	 *            "boarding")
	 * @return the transfer time in seconds. May contain special (negative)
	 *         values which meaning can be found in the StopTransfer.*_TRANSFER
	 *         constants. If no transfer is found, StopTransfer.UNKNOWN_TRANSFER
	 *         is returned.
	 */
	public int getTransferTime(Stop fromStop, Stop toStop, Trip fromTrip,
			Trip toTrip, boolean forwardInTime) {
		...
		int transferTime = getTransferTime(fromStop.getId(), toStop.getId(),
				fromTrip, toTrip);
		...
		if (transferTime == StopTransfer.UNKNOWN_TRANSFER) {
			...
			if (fromStop.getParentStation() != null
				...
				fromStopParentId = new AgencyAndId(fromStop.getId()
						.getAgencyId(), fromStop.getParentStation());
			}
			if (toStop.getParentStation() != null
					&& !toStop.getParentStation().isEmpty()) {
				// To stop has a parent
				toStopParentId = new AgencyAndId(toStop.getId().getAgencyId(),
						toStop.getParentStation());
			}

			// Check parent of from stop if no transfer was found
			if (fromStopParentId != null) {
				transferTime = getTransferTime(fromStopParentId,
						toStop.getId(), fromTrip, toTrip);
			}

			// Check parent of to stop if still no transfer was found
			if (transferTime == StopTransfer.UNKNOWN_TRANSFER
					&& toStopParentId != null) {
				transferTime = getTransferTime(fromStop.getId(),
						toStopParentId, fromTrip, toTrip);
			}

			// Check parents of both stops if still no transfer was found
			if (transferTime == StopTransfer.UNKNOWN_TRANSFER
					&& fromStopParentId != null && toStopParentId != null) {
				transferTime = getTransferTime(fromStopParentId,
						toStopParentId, fromTrip, toTrip);
			}
		}
	 ...
	 * @param toStopId
	 *            is the id of the departing stop
	 * @param fromTrip
	 *            is the arriving trip
	 * @param toTrip
	 *            is the departing trip
	 ...
	 *         is returned.
	 */
	private int getTransferTime(AgencyAndId fromStopId, AgencyAndId toStopId,
			Trip fromTrip, Trip toTrip) {
		...
		StopTransfer stopTransfer = table.get(new P2<AgencyAndId>(fromStopId,
				toStopId));
	 ...
	 *            is the departing stop
	 * @param fromRoute
	 *            is the arriving route; is allowed to be null
	 * @param toRoute
	 *            is the departing route; is allowed to be null
	 * @param fromTrip
	 *            is the arriving trip; is allowed to be null
	 * @param toTrip
	 *            is the departing trip; is allowed to be null
	 * @param transferTime
	 *            is the transfer time in seconds. May contain special
	 *            (negative) values which meaning can be found in the
	 *            StopTransfer.*_TRANSFER constants. If no transfer is found,
	 *            StopTransfer.UNKNOWN_TRANSFER is returned.
	 */
	public void addTransferTime(Stop fromStop, Stop toStop, Route fromRoute,
			Route toRoute, Trip fromTrip, Trip toTrip, int transferTime) {
		checkNotNull(fromStop);
		checkNotNull(toStop);
		...
		P2<AgencyAndId> stopIdPair = new P2<AgencyAndId>(fromStop.getId(),
				toStop.getId());
		...
		SpecificTransfer specificTransfer = new SpecificTransfer(fromRoute,
				toRoute, fromTrip, toTrip, transferTime);
		stopTransfer.addSpecificTransfer(specificTransfer);
	}

	/**
	 * Determines the transfer penalty given a transfer time and a penalty for
	 * non-preferred transfers.
	 * 
	 * @param transferTime
	 *            is the transfer time
	 * @param nonpreferredTransferPenalty
	 *            is the penalty for non-preferred transfers
	 * @return the transfer penalty
	 */
	public int determineTransferPenalty(int transferTime,
			int nonpreferredTransferPenalty) {
		int transferPenalty = 0;

		if (hasPreferredTransfers()) {
			// Only penalize transfers if there are some that will be
			// depenalized
	...
	public static class Transfer {
		...
		public Transfer(AgencyAndId fromStopId, AgencyAndId toStopId,
				int seconds) {
	...
	/**
	 * Public function for testing purposes only. Returns only the first
	 * specific transfers.
	 * 
	 * @see TransferGraphLinker
	 */
