...
public class State implements Cloneable {
	...
	// TODO(flamholz): this is a very confusing name as it actually applies to
	// all non-transit modes.
	// we should DEFINITELY rename this variable and the associated methods.
	public double walkDistance;

	// The time traveled pre-transit, for park and ride or kiss and ride
	// searches
	int preTransitTime;
	...
	/**
	 * Create an initial state representing the beginning of a search for the
	 * given routing context. Initial "parent-less" states can only be created
	 * at the beginning of a trip. elsewhere, all states must be created from a
	 * parent and associated with an edge.
	 */
	public State(RoutingRequest opt) {
		this(opt.rctx.origin, opt.rctx.originBackEdge, opt
				.getSecondsSinceEpoch(), opt);
	}

	/**
	 * Create an initial state, forcing vertex to the specified value. Useful
	 ...
	 * Useful for reusing a RoutingContext in TransitIndex, tests, etc.
	 */
	...
	/**
	 * Create an initial state, forcing vertex, back edge and time to the
	 * specified values. Useful for reusing a RoutingContext in TransitIndex,
	 * tests, etc.
	 */
	public State(Vertex vertex, Edge backEdge, long timeSeconds,
		...
		// note that here we are breaking the circular reference between rctx
		// and options
		// this should be harmless since reversed clones are only used when
		// routing has finished
		this.stateData.opt = options;
		this.stateData.startTime = timeSeconds;
		this.stateData.usingRentedBike = false;
		/*
		 * If the itinerary is to begin with a car that is left for transit, the
		 * initial state of arriveBy searches is with the car already "parked"
		 * and in WALK mode. Otherwise, we are in CAR mode and "unparked".
		 */
	...
	/**
	 * Create a state editor to produce a child of this state, which will be the
	 * result of traversing the given edge.
	 * 
	 * @param e
	 * @return
	 */
	...
	/*
	 * FIELD ACCESSOR METHODS States are immutable, so they have only get
	 * methods. The corresponding set methods are in StateEditor.
	 */
	 ...
	 * 
	 * @param key
	 *            - An Object that is a key in this State's extension map
	...
	public String toString() {
		return "<State " + new Date(getTimeInMillis()) + " [" + weight + "] "
				...
				+ (isCarParked() ? "CAR_PARKED " : "") + vertex + ">";
	}

	public String toStringVerbose() {
		return "<State " + new Date(getTimeInMillis()) + " w="
				+ this.getWeight() + " t=" + this.getElapsedTimeSeconds()
				+ " d=" + this.getWalkDistance() + " p="
				+ this.getPreTransitTime() + " b=" + this.getNumBoardings()
				+ " br=" + this.isBikeRenting() + " pr=" + this.isCarParked()
				+ ">";
	}
	...
	/**
	 * Returns the length of the trip in seconds up to this time, not including
	 * the initial wait. It subtracts out the initial wait, up to a clamp value
	 * specified in the request. If the clamp value is set to -1, no clamping
	 * will occur. If the clamp value is set to 0, the initial wait time will
	 * not be subtracted out (i.e. it will be clamped to zero). This is used in
	 * lieu of reverse optimization in Analyst.
	 */
	...
	/**
	 * Whether this path has ever previously boarded (or alighted from, in a
	 * reverse search) a transit vehicle
	 */
	...
	public boolean isFinal() {
		// When drive-to-transit is enabled, we need to check whether the car
		// has been parked (or whether it has been picked up in reverse).
		boolean parkAndRide = stateData.opt.parkAndRide
				|| stateData.opt.kissAndRide;
	...
	public int getTimeDeltaSeconds() {
		return backState != null ? (int) (getTimeSeconds() - backState
				.getTimeSeconds()) : 0;
	}
	...
	public void checkNegativeWeight() {
		...
		if (dw < 0) {
			throw new NegativeWeightException(String.valueOf(dw) + " on edge "
					+ backEdge);
		}
	}
	...
	/**
	 * Get the name of the direction used to get to this state. For transit, it
	 * is the headsign, while for other things it is what you would expect.
	 */
	public String getBackDirection() {
		// This can happen when stop_headsign says different things at two trips
		// on the same
		// pattern and at the same stop.
		if (backEdge instanceof TablePatternEdge) {
			return stateData.tripTimes
					.getHeadsign(((TablePatternEdge) backEdge).getStopIndex());
		} else {
	...
	/**
	 * Get the back trip of the given state. For time dependent transit, State
	 ...
	 * 
	 * @return the next additional result from an edge traversal, or null if no
	 *         more results
	 */
	...
	/**
	 * Extend an exiting result chain by appending this result to the existing
	 * chain. The usage model looks like this:
	 * 
	 * <code>
	 ...
	 *            the tail of an existing result chain, or null if the chain has
	 *            not been started
	 * @return
	 */
	public State addToExistingResultChain(State existingResultChain) {
		if (this.getNextResult() != null)
			throw new IllegalStateException(
					"this result already has a next result set");
	...
	/**
	 * This method is on State rather than RoutingRequest because we care
	 * whether the user is in possession of a rented bike.
	 * 
	 * @return BICYCLE if routing with an owned bicycle, or if at this state the
	 *         user is holding on to a rented bicycle.
	 */
	public TraverseMode getNonTransitMode() {
		return stateData.nonTransitMode;
	}

	// TODO: There is no documentation about what this means. No one knows
	// precisely.
	...
	public State reversedClone() {
		...
		// It is distributed symmetrically over all preboard and prealight
		// edges.
		State newState = new State(this.vertex, getTimeSeconds(),
	...
	public boolean multipleOptionsBefore() {
		...
		for (Edge out : backState.vertex.getOutgoing()) {
			...
			// this section handles the case of an option which is only an
			// option if you walk your
			// bike. It is complicated because you will not need to walk your
			// bike until one
			...
			for (Edge out2 : tov.getOutgoing()) {
				...
				if (outState2 != null
						&& !outState2.getBackMode().equals(requestedMode)) {
					// walking a bike, so, not really an exit
					continue;
				}
				found = true;
	...
	/**
	 * @return the last TripPattern used in this path (which is set when leaving
	 *         the vehicle).
	 */
	...
	/**
	 * Reverse the path implicit in the given state, re-traversing all edges in
	 * the opposite direction so as to remove any unnecessary waiting in the
	 * resulting itinerary. This produces a path that passes through all the
	 * same edges, but which may have a shorter overall duration due to
	 * different weights on time-dependent (e.g. transit boarding) edges. If the
	 * optimize parameter is false, the path will be reversed but will have the
	 * same duration. This is the result of combining the functions from
	 * GraphPath optimize and reverse.
	 * 
	 * @param optimize
	 *            Should this path be optimized or just reversed?
	 * @param forward
	 *            Is this an on-the-fly reverse search in the midst of a forward
	 *            search?
	 * @returns a state at the other end (or this end, in the case of a forward
	 *          search) of a reversed, optimized path
	 */
	public State optimizeOrReverse(boolean optimize, boolean forward) {
		...
		while (orig.getBackState() != null) {
			...
			if (optimize) {
				// first board/last alight: figure in wait time in on the fly
				// optimization
				if (edge instanceof TransitBoardAlight
						...
						(((TransitBoardAlight) edge).boarding && !stateData.opt.arriveBy) ||
						// alighting in a reverse main search
						(!((TransitBoardAlight) edge).boarding && stateData.opt.arriveBy))) {

					ret = ((TransitBoardAlight) edge).traverse(ret, orig
							.getBackState().getTimeSeconds());
					newInitialWaitTime = ret.stateData.initialWaitTime;
				} else {
					ret = edge.traverse(ret);
				}

				if (ret != null && ret.getBackMode() != null
						&& orig.getBackMode() != null
						&& ret.getBackMode() != orig.getBackMode()) {
					ret = ret.next; // Keep the mode the same as on the original
									// graph path (in K+R)
				}

				if (ret == null) {
					LOG.warn("Cannot reverse path at edge: "
							+ edge
				...
				editor.setNumBoardings(getNumBoardings()
						- orig.getNumBoardings());

				ret = editor.makeState();

				// EdgeNarrative origNarrative = orig.getBackEdgeNarrative();
				// EdgeNarrative retNarrative = ret.getBackEdgeNarrative();
				// copyExistingNarrativeToNewNarrativeAsAppropriate(origNarrative,
				// retNarrative);
			}
		...
		if (forward) {
			...
			if (getWeight() <= reversed.getWeight())
				LOG.warn("Optimization did not decrease weight: before "
						+ this.getWeight() + " after " + reversed.getWeight());
			if (getElapsedTimeSeconds() != reversed.getElapsedTimeSeconds())
				LOG.warn("Optimization changed time: before "
						+ this.getElapsedTimeSeconds() + " after "
						+ reversed.getElapsedTimeSeconds());
			if (getActiveTime() <= reversed.getActiveTime())
				// NOTE: this can happen and it isn't always bad (i.e. it
				// doesn't always mean that
				// reverse-opt got called when it shouldn't have). Imagine three
				// lines A, B and C
				// A trip takes line A at 7:00 and arrives at the first transit
				// center at 7:30, where line
				// B is boarded at 7:40 to another transit center with an
				// arrival at 8:00. At 8:30, line C
				// is boarded. Suppose line B runs every ten minutes and the
				// other two run every hour. The
				// optimizer will optimize the B->C connection, moving the trip
				// on line B forward
				// ten minutes. However, it will not be able to move the trip on
				// Line A forward because
				// there is not another possible trip. The waiting time will get
				// pushed towards the
				// the beginning, but not all the way.
				LOG.warn("Optimization did not decrease active time: before "
						+ this.getActiveTime() + " after "
						+ reversed.getActiveTime() + ", boardings: "
						+ this.getNumBoardings());
			if (reversed.getWeight() < this.getBackState().getWeight())
				// This is possible; imagine a trip involving three lines, line
				// A, line B and
				// line C. Lines A and C run hourly while Line B runs every ten
				// minute starting
				// at 8:55. The user boards line A at 7:00 and gets off at the
				// first transfer point
				// (point u) at 8:00. The user then boards the first run of line
				// B at 8:55, an optimal
				// transfer since there is no later trip on line A that could
				// have been taken. The user
				// deboards line B at point v at 10:00, and boards line C at
				// 10:15. This is a
				// non-optimal transfer; the trip on line B can be moved forward
				// 10 minutes. When
				// that happens, the first transfer becomes non-optimal (8:00 to
				// 9:05) and the trip
				// on line A can be moved forward an hour, thus moving 55
				// minutes of waiting time
				// from a previous state to the beginning of the trip where it
				// is significantly
				// cheaper.
				LOG.warn("Weight has been reduced enough to make it run backwards, now:"
						+ reversed.getWeight()
						+ " backState "
						+ getBackState().getWeight()
						+ ", "
			...
			if (Math.abs(getWeight() - reversed.getWeight()) > 1
				...
				LOG.warn("Weight is changed (before: " + getWeight()
						+ ", after: " + reversed.getWeight()
						+ "), initial wait times " + "constant at "
						+ newInitialWaitTime);
			if (newInitialWaitTime != reversed.stateData.initialWaitTime)
				LOG.warn("Initial wait time not propagated: is "
						+ reversed.stateData.initialWaitTime + ", should be "
						+ newInitialWaitTime);

			// copy the path parser states so this path is not thrown out going
			// forward
			// reversed.pathParserStates =
			// Arrays.copyOf(this.pathParserStates,
			// this.pathParserStates.length, newLength);
	...
	/**
	 * After reverse-optimizing, many things are not set. Set them from the
	 * unoptimized state.
	 * 
	 * @param o
	 *            The other state to initialize things from.
	 */
	private void initializeFieldsFrom(State o) {
		...
		// this will get re-set on the next alight (or board in a reverse
		// search)
