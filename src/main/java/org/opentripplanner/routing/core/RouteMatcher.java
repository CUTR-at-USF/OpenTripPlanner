...
/**
 * A RouteMatcher is a collection of routes based on IDs, short name and/or
 * agency IDs.
 * 
 * We currently support route full IDs (agency ID + route ID), agency ID + route
 * name, or route name only. Support for other matching expression can be easily
 * added later on.
 */
public class RouteMatcher implements Cloneable, Serializable {
	 ...
	 *            [agencyId]_[routeName]_[routeId] Please note that this format
	 *            is not really intuitive as it does not follow the OBA-gtfslib
	 *            AgencyAndId standard ('agencyID_routeId'). This was kept for
	 *            backward-compatibility purposes. If the original routeName
	 *            contains some "_" each *must* be replaced by a space. If the
	 *            agency or route ID contains a "_" they must be escaped using a
	 *            backslash. TODO why do we want to accept route name strings
	 *            when we have IDs? Break backward compatibility. FIXME this is
	 *            the only place we are still using underscores as scope
	 *            separators. Rethink this from scratch.
	...
	public static RouteMatcher parse(String routeSpecList) {
		...
		for (String element : routeSpecList.split(",")) {
			...
			if (routeSpec.length != 2 && routeSpec.length != 3) {
				throw new IllegalArgumentException("Wrong route spec format: "
						+ element);
			}
			...
			if (agencyId != null && routeId != null && routeName == null) {
				...
				retval.agencyAndRouteIds
						.add(new AgencyAndId(agencyId, routeId));
			} else if (agencyId != null && routeName != null && routeId == null) {
				// Case 2: specified agency ID and route name but no route ID
				retval.agencyIdAndRouteNames.add(new T2<String, String>(
						agencyId, routeName));
			} else if (agencyId == null && routeName != null && routeId == null) {
	...
	public boolean matches(Route route) {
		...
		if (agencyIdAndRouteNames.contains(new T2<String, String>(route.getId()
				.getAgencyId(), routeName)))
			return true;
	...
	public String asString() {
		...
		for (AgencyAndId agencyAndId : agencyAndRouteIds) {
			builder.append(agencyAndId.getAgencyId() + "__"
					+ agencyAndId.getId());
			builder.append(",");
		}
		for (T2<String, String> agencyIdAndRouteName : agencyIdAndRouteNames) {
			builder.append(agencyIdAndRouteName.first + "_"
					+ agencyIdAndRouteName.second);
			builder.append(",");
		}
	...
	public String toString() {
		return String
				.format("RouteMatcher<agencyAndRouteIds=%s agencyIdAndRouteNames=%s routeNames=%s>",
	...
	public boolean equals(Object another) {
		...
		return agencyAndRouteIds.equals(anotherMatcher.agencyAndRouteIds)
				&& agencyIdAndRouteNames
						.equals(anotherMatcher.agencyIdAndRouteNames)
