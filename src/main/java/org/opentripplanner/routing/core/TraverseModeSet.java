...
/**
 * A set of traverse modes -- typically, one non-transit mode (walking, biking,
 * car) and zero or more transit modes (bus, tram, etc). This class allows
 * efficiently adding or removing modes from a set.
 * 
 * @author novalis
 *
 */
public class TraverseModeSet implements Cloneable, Serializable {
	...
	private static final int MODE_TRAINISH = MODE_TRAM | MODE_RAIL
			| MODE_SUBWAY | MODE_FUNICULAR | MODE_GONDOLA;

	private static final int MODE_BUSISH = MODE_CABLE_CAR | MODE_BUS;

	private static final int MODE_TRANSIT = MODE_TRAINISH | MODE_BUSISH
			| MODE_FERRY;

	private static final int MODE_DRIVING = MODE_CAR
			| MODE_CUSTOM_MOTOR_VEHICLE;

	private static final int MODE_ALL = MODE_TRANSIT | MODE_DRIVING | MODE_WALK
			| MODE_BICYCLE;
	...
	/**
	 * Returns true if any the trip may use some train-like (train, subway,
	 * tram) mode
	 */
