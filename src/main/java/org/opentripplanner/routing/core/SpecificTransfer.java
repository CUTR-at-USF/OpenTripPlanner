...
/**
 * SpecificTransfer class used by Transfer. Represents a specific transfer
 * between two stops. See the links described at TransferTable for more details
...
public class SpecificTransfer implements Serializable {
	...
	/**
	 * Constant containing the minimum specificity that is allowed by the
	 * specifications
	 */
	public static final int MIN_SPECIFICITY = 0;

	/**
	 * Constant containing the maximum specificity that is allowed by the
	 * specifications
	 */
	public static final int MAX_SPECIFICITY = 4;

	/**
	 * Route id of arriving trip. Is allowed to be null. Is ignored when
	 * fromTripId is not null.
	 */
	final private AgencyAndId fromRouteId;

	/**
	 * Route id of departing trip. Is allowed to be null. Is ignored when
	 * toTripId is not null.
	 */
	...
	/**
	 * Value indicating the minimum transfer time in seconds. May contain
	 * special (negative) values which meaning can be found in the
	 * Transfer.*_TRANSFER constants.
	 */
	final int transferTime;

	public SpecificTransfer(AgencyAndId fromRouteId, AgencyAndId toRouteId,
			AgencyAndId fromTripId, AgencyAndId toTripId, int transferTime) {
	...
	public SpecificTransfer(Route fromRoute, Route toRoute, Trip fromTrip,
			Trip toTrip, int transferTime) {
		if (fromRoute != null) {
	...
	/**
	 * Returns whether this specific transfer is applicable to a transfer
	 * between two trips.
	 * 
	 * @param fromTrip
	 ...
	 * @return true if this specific transfer is applicable to a transfer
	 *         between two trips.
	 */
