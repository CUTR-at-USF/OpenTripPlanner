...
/**
 * This class is a wrapper around a new State that provides it with setter and
 * increment methods, allowing it to be modified before being put to use.
 * 
 * By virtue of being in the same package as States, it can modify their package
 * private fields.
 * 
 * @author andrewbyrd
 */
public class StateEditor {
	...
	/**
	 * Why can a state editor only be used once? If you modify some component of
	 * state with and editor, use the editor to create a new state, and then
	 * make more modifications, these modifications will be applied to the
	 * previously created state. Reusing the state editor to make several states
	 * would modify an existing state somewhere earlier in the search, messing
	...
	public State makeState() {
		...
		if (spawned)
			throw new IllegalStateException(
					"A StateEditor can only be used once.");
	...
	/**
	 * Put a new value into the State extensions map. This will always clone the
	 * extensions map before it is modified the first time, making sure that
	 * other references to the map in earlier States are unaffected.
	 */
	...
	public void setExtension(Object key, Object value) {
		...
		if (!extensionsModified) {
			...
			if (child.stateData.extensions == null)
				...
				newExtensions = (HashMap<Object, Object>) child.stateData.extensions
						.clone();
	 ...
	 * what other editing has been done. This allows graph patches to block
	 * traversals.
	 */
	...
	/**
	 * Advance or rewind the time of the new state by the given non-negative
	 * amount. Direction of time is inferred from the direction of traversal.
	 * This is the only element of state that runs backward when traversing
	 * backward.
	 */
	...
	/**
	 * Initial wait time is recorded so it can be subtracted out of paths in
	 * lieu of "reverse optimization". This happens in Analyst.
	 */
	...
	/**
	 * The lastNextArrivalDelta is the amount of time between the arrival of the
	 * last trip the planner used and the arrival of the trip after that.
	 */
	...
	public void setRoute(AgencyAndId routeId) {
		...
		if (routeId != null) {
			...
			child.stateData.routeSequence = Arrays.copyOf(oldRouteSequence,
					oldLength + 1);
			child.stateData.routeSequence[oldLength] = routeId;
			// LOG.debug("new route seq {}",
			// Arrays.asList(child.stateData.routeSequence)); // array will be
	...
	/**
	 * This has two effects: marks the car as parked, and switches the current
	 * mode. Marking the car parked is important for allowing co-dominance of
	 * walking and driving states.
	 */
	...
	/**
	 * Set non-incremental state values (ex. {@link State#getRoute()}) from an
	 * existing state. Incremental values (ex. {@link State#getNumBoardings()})
	...
	/*
	 * Allow patches to see the State being edited, so they can decide whether
	 * to apply their transformations to the traversal result or not.
	 */
	...
	/**
	 * To be called before modifying anything in the child's StateData. Makes
	 * sure that changes are applied to a copy of StateData rather than the same
	 * one that is still referenced in existing, older states.
	 */
	private void cloneStateDataAsNeeded() {
		if (child.backState != null
				&& child.stateData == child.backState.stateData)
			child.stateData = child.stateData.clone();
	}
	...
	public boolean parsePath(State state) {
		...
		for (PathParser parser : parsers) {
			...
			if (newState != oldState) {
				if (!modified) {
					// clone the state array so only the new state will see
					// modifications
