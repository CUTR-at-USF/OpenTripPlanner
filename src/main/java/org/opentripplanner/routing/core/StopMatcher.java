 ...
 * 
 * We currently only support full stop IDs (agencyId:stopId). Support for other
 * matching expression (or other types of stop banning) can be easily added
 * later on.
 */
public class StopMatcher implements Cloneable, Serializable {
	 ...
	 * 
	 * @param stopList
	 *            is a comma-separated list of stops, each of the format
	 *            [agencyId]:[stopId]
	...
	public static StopMatcher parse(String stopList) {
		...
		for (String stopString : stopList.split(",")) {
			...
			try {
				AgencyAndId stopId = GtfsLibrary
						.convertIdFromString(stopString);
				retval.agencyAndStopIds.add(stopId);
			} catch (IllegalArgumentException e) {
				throw new IllegalArgumentException("Wrong stop spec format: "
						+ stopString);
			}
		}
		if (n == 0) {
	 ...
	 * When a stop has a parent stop, it is also matched when its parent stop is
	 * matched.
	 * 
	 * @param stop
	 *            is the stop to match using its ID
	...
	public boolean matches(Stop stop) {
		...
		if (this.isEmpty()) {
			...
			// Check whether parent stop is matched
				...
				AgencyAndId parentId = new AgencyAndId(stop.getId()
	...
	/**
	 * Function to determine whether this StopMatcher matches a particular stop
	 * id. Warning: this function does not check for parent stops.
	 * 
	 * @param stopId
	 *            is the stop id
	...
	public String toString() {
		return String.format("StopMatcher<agencyAndStopIds=%s>",
				agencyAndStopIds);
	}
