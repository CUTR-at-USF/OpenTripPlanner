...
/**
 * A trip planning request. Some parameters may not be honored by the trip
 * planner for some or all itineraries. For example, maxWalkDistance may be
 * relaxed if the alternative is to not provide a route.
 * 
 * NOTE this is the result of merging what used to be called a REQUEST and a
 * TRAVERSEOPTIONS. The Lombok Getter/Setter annotations serve to create getter
 * and setter methods on all fields, so defaults can be supplied in a
 * PrototypeRoutingRequest bean via Spring.
 */
public class RoutingRequest implements Cloneable, Serializable {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(RoutingRequest.class);
	 ...
	 * 
	 * TODO(flamholz): this is a weird place to inject this model. We do it here
	 * because, for historical reasons, this is the most reasonable place to
	 * inject it.
	 */
	...
	/*
	 * TODO no defaults should be set here, they should all be handled in one
	 * place (searchresource)
	 */
	...
	/**
	 * The worst possible time (latest for depart-by and earliest for arrive-by)
	 * to accept
	 */
	 ...
	 * | TRANSIT.
	 */
	public TraverseModeSet modes = new TraverseModeSet("TRANSIT,WALK"); // defaults
																		// in
																		// constructor
	...
	/**
	 * The epoch date/time that the trip should depart (or arrive, for requests
	...
	public double maxSlope = 0.0833333333333; // ADA max wheelchair ramp slope
												// is a good default.
	 ...
	 * one). Not to be confused with bikeBoardCost and walkBoardCost, which are
	 * the cost of boarding a vehicle with and without a bicycle. The boardCosts
	 * are used to model the 'usual' perceived cost of using a transit vehicle,
	 * and the transferPenalty is used when a user requests even less transfers.
	 * In the latter case, we don't actually optimize for fewest transfers, as
	 * this can lead to absurd results. Consider a trip in New York from Grand
	 * Army Plaza (the one in Brooklyn) to Kalustyan's at noon. The true lowest
	 * transfers route is to wait until midnight, when the 4 train runs local
	 * the whole way. The actual fastest route is the 2/3 to the 4/5 at Nevins
	 * to the 6 at Union Square, which takes half an hour. Even someone
	 * optimizing for fewest transfers doesn't want to wait until midnight.
	 * Maybe they would be willing to walk to 7th Ave and take the Q to Union
	 * Square, then transfer to the 6. If this takes less than
	 * optimize_transfer_penalty seconds, then that's what we'll return.
	 */
	public int transferPenalty = 0;

	/**
	 * A multiplier for how bad walking is, compared to being in transit for
	 * equal lengths of time. Defaults to 2. Empirically, values between 10 and
	 * 20 seem to correspond well to the concept of not wanting to walk too much
	 * without asking for totally ridiculous itineraries, but this observation
	 * should in no way be taken as scientific or definitive. Your mileage may
	 * vary.
	 */
	...
	/**
	 * How long does it take to get an elevator, on average (actually, it
	 * probably should be a bit *more* than average, to prevent optimistic
	 * trips)? Setting it to "seems like forever," while accurate, will probably
	 * prevent OTP from working correctly.
	 */
	...
	/**
	 * Cost of renting a bike. The cost is a bit more than actual time to model
	...
	/**
	 * Time to park a car in a park and ride, w/o taking into account driving
	 * and walking cost (time to park, switch off, pick your stuff, lock the
	 * car, etc...)
	 */
	 ...
	 * vehicle, as a multiplier. The default value treats wait and on-vehicle
	 * time as the same.
	 *
	 * It may be tempting to set this higher than walkReluctance (as studies
	 * often find this kind of preferences among riders) but the planner will
	 * take this literally and walk down a transit line to avoid waiting at a
	 * stop. This used to be set less than 1 (0.95) which would make waiting
	 * offboard preferable to waiting onboard in an interlined trip. That is
	 * also undesirable.
	 *
	 * If we only tried the shortest possible transfer at each stop to
	...
	/**
	 * This prevents unnecessary transfers by adding a cost for boarding a
	 * vehicle.
	 */
	public int walkBoardCost = 60 * 10;

	/**
	 * Separate cost for boarding a vehicle with a bicycle, which is more
	 * difficult than on foot.
	 */
	...
	/**
	 * Do not use certain stops. See for more information the bannedStops
	 * property in the RoutingResource class.
	 */
	public StopMatcher bannedStops = StopMatcher.emptyMatcher();

	/**
	 * Do not use certain stops. See for more information the bannedStopsHard
	 * property in the RoutingResource class.
	 */
	 ...
	 * route as preferred. We return number of seconds that we are willing to
	 * wait for preferred route.
	 */
	...
	/**
	 * Penalty added for using every unpreferred route. We return number of
	 * seconds that we are willing to wait for preferred route.
	 */
	 ...
	 */
	// initialize to zero so this does not inadvertently affect tests, and let
	// Planner handle defaults
	...
	/**
	 * Extensions to the trip planner will require additional traversal options
	 * beyond the default set. We provide an extension point for adding
	...
	/**
	 * This is true when a GraphPath is being traversed in reverse for
	 * optimization purposes.
	 */
	public boolean reverseOptimizing = false;

	/**
	 * when true, do not use goal direction or stop at the target, build a full
	 * SPT
	 */
	public boolean batch = false;

	/**
	 * Whether or not bike rental availability information will be used to plan
	 * bike rental trips
	 */
	 ...
	 * Only effective in Analyst.
	 */
	public long clampInitialWait;

	/**
	 * When true, reverse optimize this search on the fly whenever needed,
	 * rather than reverse-optimizing the entire path when it's done.
	 */
	public boolean reverseOptimizeOnTheFly = false;

	/**
	 * If true, cost turns as they would be in a country where driving occurs on
	 * the right; otherwise, cost them as they would be in a country where
	...
	/**
	 * The routing context used to actually carry out this search. It is
	 * important to build States from TraverseOptions rather than
	 * RoutingContexts, and just keep a reference to the context in the
	 * TraverseOptions, rather than using RoutingContexts for everything because
	 * in some testing and graph building situations we need to build a bunch of
	 * initial states with different times and vertices from a single
	 * TraverseOptions, without setting all the transit context or building
	 * temporary vertices (with all the exception-throwing checks that entails).
	 * 
	 * While they are conceptually separate, TraverseOptions does maintain a
	 * reference to its accompanying RoutingContext (and vice versa) so that
	 * both do not need to be passed/injected separately into tight inner loops
	 * within routing algorithms. These references should be set to null when
	 * the request scope is torn down -- the routing context becomes irrelevant
	 * at that point, since temporary graph elements have been removed and the
	 * graph may have been reloaded.
	 */
	...
	public double softWalkPenalty = 60.0; // a jump in cost when stepping over
											// the walking limit
	public double softWalkOverageRate = 5.0; // a jump in cost for every meter
												// over the walking limit

	public double preTransitPenalty = 300.0; // a jump in cost when stepping
												// over the pre-transit time
												// limit
	public double preTransitOverageRate = 10.0; // a jump in cost for every
												// second over the pre-transit
												// time limit

	/*
	 * Additional flags affecting mode transitions. This is a temporary
	 * solution, as it only covers parking and rental at the beginning of the
	 * trip.
	 */
	...
	public RoutingRequest() {
		...
		carSpeed = 40; // 40 m/s, 144 km/h, above the maximum (finite) driving
						// speed limit worldwide
	...
	public void setModes(TraverseModeSet modes) {
		...
		if (modes.getBicycle()) {
			...
			bikeWalkingOptions.walkSpeed = walkSpeed * 0.8; // walking bikes is
															// slow
			bikeWalkingOptions.walkReluctance = walkReluctance * 2.7; // and
																		// painful
	...
	public void freezeTraverseMode() {
		...
		bikeWalkingOptions.bikeWalkingOptions = new RoutingRequest(
				new TraverseModeSet());
	}

	/**
	 * Add an extension parameter with the specified key. Extensions allow you
	 * to add arbitrary traversal options.
	 */
	...
	/**
	 * Determine if a particular extension parameter is present for the
	 * specified key.
	 */
	...
	public void setUnpreferredAgencies(String s) {
		if (s != null && !s.equals(""))
			unpreferredAgencies = new HashSet<String>(Arrays.asList(s
					.split(",")));
	}
	...
	public String toString(String sep) {
		return from + sep + to + sep + getMaxWalkDistance() + sep
				+ getDateTime() + sep + arriveBy + sep + optimize + sep
				+ modes.getAsStr() + sep + getNumItineraries();
	}
	...
	/**
	 * Sets intermediatePlaces by parsing GenericLocations from a list of
	 * string.
	 */
	...
	public boolean hasIntermediatePlaces() {
		return this.intermediatePlaces != null
				&& this.intermediatePlaces.size() > 0;
	}

	/**
	 * Adds a GenericLocation to the end of the intermediatePlaces list. Will
	 * initialize intermediatePlaces if it is null.
	 */
	...
	/**
	 * Returns true if intermediate places are marked ordered, or there is only
	 * one of them.
	 */
	public boolean intermediatesEffectivelyOrdered() {
		boolean exactlyOneIntermediate = (this.intermediatePlaces != null && this.intermediatePlaces
				.size() == 1);
	...
	public RoutingRequest clone() {
		try {
			...
			clone.bannedTrips = (HashMap<AgencyAndId, BannedStopSet>) bannedTrips
					.clone();
	...
	public RoutingRequest reversedClone() {
		...
		ret.reverseOptimizing = !ret.reverseOptimizing; // this is not strictly
														// correct
	...
	public void setRoutingContext(Graph graph) {
		if (rctx == null) {
			...
			// check after back reference is established, to allow temp edge
	...
	/**
	 * For use in tests. Force RoutingContext to specific vertices rather than
	 * making temp edges.
	 */
	public void setRoutingContext(Graph graph, Edge fromBackEdge, Vertex from,
			Vertex to) {
		// normally you would want to tear down the routing context...
		// but this method is mostly used in tests, and teardown interferes with
		// testHalfEdges
		// FIXME here, or in test, and/or in other places like TSP that use this
		// method
		// if (rctx != null)
		// this.rctx.destroy();
	...
	public void setRoutingContext(Graph graph, String from, String to) {
		this.setRoutingContext(graph, graph.getVertex(from),
				graph.getVertex(to));
	}

	/**
	 * Used in internals API. Make a RoutingContext with no origin or
	 * destination vertices specified.
	 */
	...
	/**
	 * Equality does not mean that the fields of the two RoutingRequests are
	 * identical, but that they will produce the same SPT. This is particularly
	 * important when the batch field is set to 'true'. Does not consider the
	 * RoutingContext, to allow SPT caching. Intermediate places are also not
	 * included because the TSP solver will factor a single intermediate places
	 * routing request into several routing requests without intermediates
	 * before searching.
	 */
	@Override
	public boolean equals(Object o) {
		...
		if (this.batch) {
			...
			endpointsMatch = ((from == null && other.from == null) || from
					.equals(other.from))
		...
		return endpointsMatch
				...
				&& numItineraries == other.numItineraries // should only apply
															// in non-batch?
				...
				&& Objects.equal(startingTransitTripId,
						other.startingTransitTripId);
	}

	/**
	 * Equality and hashCode should not consider the routing context, to allow
	 * SPT caching. When adding fields to the hash code, pick a random large
	 * prime number that's not yet in use.
	 */
	@Override
	public int hashCode() {
		int hashCode = new Double(walkSpeed).hashCode()
				...
				+ new Double(transferPenalty).hashCode()
				...
				+ new Boolean(disableRemainingWeightHeuristic).hashCode()
				* 193939;
		if (batch) {
			hashCode *= -1;
	...
	public void cleanup() {
		if (this.rctx == null)
			...
			LOG.debug("routing context destroyed ({} temporary edges removed)",
					nRemoved);
		}
	}
	...
	public void setWalkReluctance(double walkReluctance) {
		if (walkReluctance > 0) {
			...
			// Do not set bikeWalkingOptions.walkReluctance here, because that
	...
	/**
	 * tripIsBanned is a misnomer: this checks whether the agency or route are
	 * banned. banning of individual trips is actually performed inside the trip
	 * search, in TripTimes.tripAcceptable.
	 */
	...
	public long preferencesPenaltyForRoute(Route route) {
		...
		if ((preferredRoutes != null && !preferredRoutes.equals(RouteMatcher
				.emptyMatcher()))
				|| (preferredAgencies != null && !preferredAgencies.isEmpty())) {
			boolean isPreferedRoute = preferredRoutes != null
					&& preferredRoutes.matches(route);
		...
		boolean isUnpreferedRoute = unpreferredRoutes != null
				&& unpreferredRoutes.matches(route);
	...
	/**
	 * Get the maximum expected speed over all transit modes. TODO derive actual
	 * speeds from GTFS feeds. On the other hand, that's what the bidirectional
	 * heuristic does on the fly.
	 */
	public double getTransitSpeedUpperBound() {
		...
		// Considering that buses can travel on highways, return the same max
		// speed for all other transit.
