...
/**
 * A notes source of static notes, usually created at graph building stage and
...
public class StaticStreetNotesSource implements StreetNotesSource, Serializable {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(StaticStreetNotesSource.class);

	/**
	 * Notes for street edges. No need to synchronize access to the map as they
	 * will not be concurrent write access (no notes for temporary edges, we use
	 * notes from parent).
	 */
	...
	/**
	 * Set of unique matchers, kept during building phase, used for interning
	 * (lots of note/matchers are identical).
	 */
	...
	void addNote(Edge edge, Alert note, NoteMatcher matcher) {
		if (LOG.isDebugEnabled())
			LOG.debug("Adding note {} to {} with matcher {}", note, edge,
					matcher);
	...
	/**
	 * Remove all notes attached to this edge. NOTE: this should only be called
	 * within a graph building context (or unit testing).
	 * 
	 * @param edge
	 */
	...
	/**
	 * Create a MatcherAndAlert, interning it if the note and matcher pair is
	 * already created. Note: we use the default Object.equals() for matchers,
	 * as they are mostly already singleton instances.
	 * 
	 * @param noteMatcher
	 * @param note
	 * @return
	 */
	private MatcherAndAlert buildMatcherAndAlert(NoteMatcher noteMatcher,
			Alert note) {
