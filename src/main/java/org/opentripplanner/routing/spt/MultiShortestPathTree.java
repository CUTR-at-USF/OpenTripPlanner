...
public class MultiShortestPathTree extends AbstractShortestPathTree {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(MultiShortestPathTree.class);
	...
	public void dump() {
		...
		for (Integer nState : nStates) {
			LOG.info(nState + " states: " + histogram.count(nState)
					+ " vertices.");
		}
	}
	...
	public boolean add(State newState) {
		...
		// if the vertex has any states that dominate the new state, don't add
		// the state
	...
	public static boolean dominates(State thisState, State other) {
		...
		if (thisState.backEdge != other.getBackEdge()
				&& ((thisState.backEdge instanceof StreetEdge) && (!graph
						.getTurnRestrictions(thisState.backEdge).isEmpty())))
			return false;

		if (thisState.routeSequenceSubset(other)) {
			// TODO subset is not really the right idea
			return thisState.weight <= other.weight
					&& thisState.getElapsedTimeSeconds() <= other
							.getElapsedTimeSeconds();
			// && this.getNumBoardings() <= other.getNumBoardings();
		}

		// If returning more than one result from GenericAStar, the search can
		// be very slow
		// unless you replace the following code with:
		// return false;

		boolean walkDistanceIsHopeful = thisState.walkDistance
				/ other.getWalkDistance() < 1 + WALK_DIST_EPSILON;

		double weightRatio = thisState.weight / other.weight;
		boolean weightIsHopeful = (weightRatio < 1 + WEIGHT_EPSILON && thisState.weight
				- other.weight < WEIGHT_DIFF_MARGIN);

		double t1 = (double) thisState.getElapsedTimeSeconds();
		double t2 = (double) other.getElapsedTimeSeconds();
		double timeRatio = t1 / t2;
		boolean timeIsHopeful = (timeRatio < 1 + TIME_EPSILON)
				&& (t1 - t2 <= TIME_DIFF_MARGIN);
	...
	/**
	 * Check that a state coming out of the queue is still in the Pareto-optimal
	 * set for this vertex, which indicates that it has not been ruled out as a
	 * state on an optimal path. Many shortest path algorithms will decrease the
	 * key of an entry in the priority queue when it is updated, or remove it
	 * when it is dominated.
	 * 
	 * When the Fibonacci heap was replaced with a binary heap, the decrease-key
	 * operation was removed for the same reason: both improve theoretical run
	 * time complexity, at the cost of high constant factors and more complex
	 * code.
	 * 
	 * So there can be dominated (useless) states in the queue. When they come
	 * out we want to ignore them rather than spend time branching out from
	 * them.
	 */
	@Override
