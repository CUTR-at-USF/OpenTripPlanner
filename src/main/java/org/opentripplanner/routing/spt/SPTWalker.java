...
/**
 * Walk over a SPT tree to geometrically visit all nodes and edge geometry. For
 * each geometry longer than the provided base length d0, split it in several
 * steps of equal length and shorter than d0. For each walk step call the
 * visitor callback.
 * 
 * @author laurent
 */
public class SPTWalker {
	...
	public static interface SPTVisitor {
		...
		/**
		 * Note: The same state can be visited several times (from different
		 * edges).
		 * 
		 * @param c
		 * @param s0
		 * @param s1
		 * @param d0
		 *            Curvilinear coordinate of c on [s0-s1], in meters
		 * @param d1
		 *            Curvilinear coordinate of c on [s1-s0], in meters
		 */
	...
	public void walk(SPTVisitor visitor, double d0) {
		...
		for (State s0 : allStates) {
			for (Edge e : s0.getVertex().getIncoming()) {
				...
				if (e != null && visitor.accept(e)) {
					...
					double lineStringLen = distanceLibrary
							.fastLength(lineString);
					visitor.visit(vx0.getCoordinate(), s0, s1, 0.0,
							lineStringLen);
					visitor.visit(vx1.getCoordinate(), s0, s1, lineStringLen,
							0.0);
					nTotal += 2;
					...
					if (lineStringLen > d0) {
						int nSteps = (int) Math.floor(lineStringLen / d0) + 1; // Number
																				// of
																				// steps
						double stepLen = lineStringLen / nSteps; // Length of
																	// step
						...
						for (int i = 0; i < pList.length - 1; i++) {
							...
							double segLen = distanceLibrary
									.fastDistance(p0, p1);
							while (curLen - startLen < segLen) {
								double k = (curLen - startLen) / segLen;
								Coordinate p = new Coordinate(p0.x * (1 - k)
										+ p1.x * k, p0.y * (1 - k) + p1.y * k);
								visitor.visit(p, reverse ? s1 : s0,
										reverse ? s0 : s1, curLen,
		...
		LOG.info(
				"SPTWalker: Generated {} points ({} dup edges, {} no geometry) from {} states.",
