...
public class GraphPath {
	...
	/**
	 * Construct a GraphPath based on the given state by following back-edge
	 * fields all the way back to the origin of the search. This constructs a
	 * proper Java list of states (allowing random access etc.) from the
	 * predecessor information left in states by the search algorithm.
	 * 
	 * Optionally re-traverses all edges backward in order to remove excess
	 * waiting time from the final itinerary presented to the user. When
	 * planning with departure time, the edges will then be re-traversed once
	 * more in order to move the waiting time forward in time, towards the end.
	 * 
	 * @param s
	...
	public GraphPath(State s, boolean optimize) {
		...
		/*
		 * Starting from latest (time-wise) state, copy states to the head of a
		 * list in reverse chronological order. List indices will thus increase
		 * forward in time, and backEdges will be chronologically 'back'
		...
		for (State cur = lastState; cur != null; cur = cur.getBackState()) {
			...
			// Record the edge if it exists and this is not the first state in
			// the path.
	...
	/**
	 * @return A list containing one AgencyAndId (trip_id) for each vehicle
	 *         boarded in this path, in the chronological order they are
	 *         boarded.
	 */
	...
	public void dumpPathParser() {
		...
		System.out.println(this.toString());
		for (State s : states)
			System.out.println(s.getPathParserStates() + s + " via "
					+ s.getBackEdge());
		System.out.println(" --- END GRAPHPATH DUMP ---");
	}
