...
/**
 * This class contains all the transient indexes of graph elements -- those that
 * are not serialized with the graph. Caching these maps is essentially an
 * optimization, but a big one. The index is bootstrapped from the graph's list
 * of edges.
 */
public class GraphIndex {
	...
	public final Multimap<Agency, TripPattern> patternsForAgency = ArrayListMultimap
			.create();
	public final Multimap<Route, TripPattern> patternsForRoute = ArrayListMultimap
			.create();
	public final Multimap<Stop, TripPattern> patternsForStop = ArrayListMultimap
			.create();
	public final Multimap<String, Stop> stopsForParentStation = ArrayListMultimap
			.create();
	...
	/**
	 * Used for finding first/last trip of the day. This is the time at which
	 * service ends for the day.
	 */
	public final int overnightBreak = 60 * 60 * 2; // FIXME not being set, this
													// was done in transitIndex
	...
	public GraphIndex(Graph graph) {
		...
		/*
		 * We will keep a separate set of all vertices in case some have the
		 * same label. Maybe we should just guarantee unique labels.
		 */
		...
		// Copy these two service indexes from the graph until we have better
		// ones.
	...
	private void analyzeServices() {
		// This is a mess because CalendarService, CalendarServiceData, etc. are
		// all in OBA.
	...
	private Set<TripPattern> patternsForStopCluster(StopCluster sc) {
		...
		for (Stop stop : sc.children)
			tripPatterns.addAll(patternsForStop.get(stop));
		return tripPatterns;
	}

	/**
	 * Initialize transfer data needed for profile routing. Find the best
	 * transfers between each pair of patterns that pass near one another.
	 */
	public void initializeProfileTransfers() {
		...
		Map<P2<TripPattern>, ProfileTransfer.GoodTransferList> transfers = Maps
				.newHashMap();
		...
		for (StopCluster sc0 : stopClusterForId.values()) {
			...
			// Accounts for area-like (rather than point-like) nature of
			// clusters
			Map<StopCluster, Double> nearbyStopClusters = findNearbyStopClusters(
					sc0, TRANSFER_RADIUS);
			for (StopCluster sc1 : nearbyStopClusters.keySet()) {
				...
				for (TripPattern tp0 : tripPatterns0) {
					for (TripPattern tp1 : tripPatterns1) {
						...
						ProfileTransfer.GoodTransferList list = transfers
								.get(pair);
						if (list == null) {
							list = new ProfileTransfer.GoodTransferList();
							transfers.put(pair, list);
						}
						list.add(new ProfileTransfer(tp0, tp1, sc0, sc1,
								(int) distance));
					}
				}
			}
		...
		/*
		 * Now filter the transfers down to eliminate long series of transfers
		 * in shared trunks.
		 */
		...
		for (P2<TripPattern> pair : transfers.keySet()) {
			...
			TripPattern fromPattern = pair.first; // TODO consider using second
													// (think of express-local
													// transfers in NYC)
			Map<StopCluster, ProfileTransfer> transfersByFromCluster = Maps
					.newHashMap();
			...
			boolean inSeries = false; // true whenever a transfer existed for
										// the last stop in the stop pattern
			for (Stop stop : fromPattern.stopPattern.stops) {
				...
				if (inSeries)
					continue;
				// Keep this transfer: it's not preceded by another stop with a
				// transfer in this stop pattern
				retainedTransfers.add(transfer);
				inSeries = true;
			}
			// LOG.info("patterns {}, {} transfers", pair,
			// retainedTransfers.size());
		 ...
		 * System.out.println("STOP " + stop); for (Transfer transfer :
		 * transfersForStop.get(stop)) { System.out.println("    " +
		 * transfer.toString()); } }
		 */
	...
	/**
	 * Find transfer candidates for profile routing. TODO replace with an
	 * on-street search using the existing profile router functions.
	 */
	public Map<StopCluster, Double> findNearbyStopClusters(StopCluster sc,
			double radius) {
		...
		for (StopCluster cluster : stopClusterSpatialIndex.query(env)) {
			// TODO this should account for area-like nature of clusters. Use
			// size of bounding boxes.
			double distance = distlib.distance(sc.lat, sc.lon, cluster.lat,
					cluster.lon);
			if (distance < radius)
	...
	/**
	 * An OBA Service Date is a local date without timezone, only year month and
	 * day.
	 */
	...
	public BitSet servicesRunning(LocalDate date) {
		return servicesRunning(new ServiceDate(date.getYear(),
	 ...
	 * process because all of the departure search functions currently assume
	 * the existence of a State and a routing context. It would be nice to have
	 * another function that gives all departures within a time window at a
	 * stop, being careful to get a mix of all patterns passing through that
	 * stop. In fact, such a function could replace the current boarding logic
	 * if we want to allow boarding more than one trip on the same route at once
	 * (return more than one state). The current implementation is a sketch and
	 * does not adequately
	 */
	public Collection<StopTimesInPattern> stopTimesForStop(Stop stop) {
		...
		for (TripPattern pattern : patternsForStop.get(stop)) {
			...
			// A Stop may occur more than once in a pattern, so iterate over all
			// Stops.
			int sidx = 0;
	...
	/**
	 * FIXME OBA parentStation field is a string, not an AgencyAndId, so it has
	 * no agency/feed scope But the DC regional graph has no parent stations
	 * pre-defined, so no use dealing with them for now. However Trimet stops
	 * have "landmark" or Transit Center parent stations, so we don't use the
	 * parent stop field.
	 *
	 * Ideally in the future stop clusters will replicate and/or share
	 * implementation with GTFS parent stations.
	 *
	 * We can't use a similarity comparison, we need exact matches. This is
	 * because many street names differ by only one letter or number, e.g. 34th
	 * and 35th or Avenue A and Avenue B. Therefore normalizing the names before
	 * the comparison is essential. The agency must provide either parent
	 * station information or a well thought out stop naming scheme to cluster
	...
	public void clusterStops() {
		...
		// Each stop without a cluster will greedily claim other stops without
		// clusters.
		for (Stop s0 : stopForId.values()) {
			if (stopClusterForStop.containsKey(s0))
				continue; // skip stops that have already been claimed by a
							// cluster
			String s0normalizedName = StopNameNormalizer
					.normalize(s0.getName());
			StopCluster cluster = new StopCluster(String.format("C%03d",
					psIdx++), s0normalizedName);
			// LOG.info("stop {}", s0normalizedName);
			// No need to explicitly add s0 to the cluster. It will be found in
			// the spatial index query below.
			Envelope env = new Envelope(
					new Coordinate(s0.getLon(), s0.getLat()));
			env.expandBy(SphericalDistanceLibrary.metersToLonDegrees(
					CLUSTER_RADIUS, s0.getLat()), SphericalDistanceLibrary
					.metersToDegrees(CLUSTER_RADIUS));
			for (TransitStop ts1 : stopSpatialIndex.query(env)) {
				Stop s1 = ts1.getStop();
				double geoDistance = SphericalDistanceLibrary.getInstance()
						.fastDistance(s0.getLat(), s0.getLon(), s1.getLat(),
								s1.getLon());
				if (geoDistance < CLUSTER_RADIUS) {
					String s1normalizedName = StopNameNormalizer.normalize(s1
							.getName());
					// LOG.info("   --> {}", s1normalizedName);
					// LOG.info("       geodist {} stringdist {}", geoDistance,
					// stringDistance);
					if (s1normalizedName.equals(s0normalizedName)) {
						// Create a bidirectional relationship between the stop
						// and its cluster
