...
/**
 * Compute full SPT for earliest arrival problem. Always builds a full shortest
 * path tree ("batch mode").
 * 
 * Note that walk limiting must be turned off -- resource limiting is not
 * algorithmically correct.
 */
public class EarliestArrivalSPTService implements SPTService {

	private static final Logger LOG = LoggerFactory
			.getLogger(EarliestArrivalSPTService.class);
	...
	public ShortestPathTree getShortestPathTree(RoutingRequest options,
			double relTimeout, SearchTerminationStrategy terminationStrategy) {

		// clone options before modifying, otherwise disabling resource limiting
		// will cause
		...
		options.worstTime = options.dateTime
				+ (options.arriveBy ? -maxt : maxt);
		...
		while (!pq.empty()) {
			...
			if (!spt.visit(u))
				continue;
			Collection<Edge> edges = options.arriveBy ? u_vertex.getIncoming()
