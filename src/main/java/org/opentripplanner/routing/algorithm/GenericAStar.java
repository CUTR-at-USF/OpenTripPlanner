 ...
 */
public class GenericAStar implements SPTService { // maybe this should be
													// wrapped in a component
													// SPT service

	private static final Logger LOG = LoggerFactory
			.getLogger(GenericAStar.class);
	private static final MonitoringStore store = MonitoringStoreFactory
			.getStore();
	...
	public void startSearch(RoutingRequest options,
		...
		// null checks on origin and destination vertices are already performed
		// in setRoutingContext
		// options.rctx.check();

		runState.spt = new MultiShortestPathTree(runState.options);
		// runState.spt = new EarliestArrivalShortestPathTree(options);
		// //MultiShortestPathTree(runState.options);

		runState.heuristic = options.batch ? new TrivialRemainingWeightHeuristic()
				: runState.rctx.remainingWeightHeuristic;

		// Since initial states can be multiple, heuristic cannot depend on the
		// initial state.
		runState.heuristic.initialize(runState.options, runState.rctx.origin,
				runState.rctx.target, abortTime);
		...
		initialSize = (int) Math
				.ceil(2 * (Math.sqrt((double) initialSize + 1)));
		runState.pq = new BinHeap<State>(initialSize);
		runState.pq.insert(initialState, 0);

		// options = options.clone();
		// /** max walk distance cannot be less than distances to nearest
		// transit stops */
	...
	boolean iterate() {
		...
		Collection<Edge> edges = runState.options.arriveBy ? runState.u_vertex
		...
		for (Edge edge : edges) {
			...
			// multiple trips.
			for (State v = edge.traverse(runState.u); v != null; v = v
					.getNextResult()) {
				...
				double remaining_w = computeRemainingWeight(runState.heuristic,
						v, runState.rctx.target, runState.options);
				if (remaining_w < 0 || Double.isInfinite(remaining_w)) {
					continue;
				}
				double estimate = v.getWeight() + remaining_w
						* runState.options.heuristicWeight;

				if (verbose) {
					System.out.println("      edge " + edge);
					System.out.println("      " + runState.u.getWeight()
							+ " -> " + v.getWeight() + "(w) + " + remaining_w
							+ "(heur) = " + estimate + " vert = "
							+ v.getVertex());
				}
				...
				if (isWorstTimeExceeded(v, runState.options)) {
					...
					if (verbose)
						System.out
								.println("         too much time to reach, not enqueued. time = "
										+ v.getTimeSeconds());
					continue;
				}
	...
	void runSearch(long abortTime) {
		...
		while (!runState.pq.empty()) { // Until the priority queue is empty:
			...
			if (abortTime < Long.MAX_VALUE
					&& System.currentTimeMillis() > abortTime) {
				LOG.warn("Search timeout. origin={} target={}",
						runState.rctx.origin, runState.rctx.target);
				// Rather than returning null to indicate that the search was
				// aborted/timed out,
				// we instead set a flag in the routing context and return the
				// SPT anyway. This
				// allows returning a partial list results even when a timeout
				// occurs.
				runState.options.rctx.aborted = true; // signal search
														// cancellation up to
														// higher stack frames
			...
			if (runState.foundPathWeight != null
					&& runState.u.getWeight() > runState.foundPathWeight
							* OVERSEARCH_MULTIPLIER) {
				break;
			}
			if (runState.terminationStrategy != null) {
				if (runState.terminationStrategy.shouldSearchTerminate(
						runState.rctx.origin, runState.rctx.target, runState.u,
						runState.spt, runState.options)) {
					break;
				}
				// TODO AMB: Replace isFinal with bicycle conditions in
				// BasicPathParser
			} else if (!runState.options.batch
					&& runState.u_vertex == runState.rctx.target
					&& runState.u.isFinal()
				...
				if (runState.targetAcceptedStates.size() >= runState.options
						.getNumItineraries()) {
	...
	public ShortestPathTree getShortestPathTree(RoutingRequest options,
	...
	private double computeRemainingWeight(
			final RemainingWeightHeuristic heuristic, State v, Vertex target,
			RoutingRequest options) {
		// actually, the heuristic could figure this out from the
		// TraverseOptions.
		// set private member back=options.isArriveBy() on initial weight
		// computation.
		if (options.arriveBy) {
