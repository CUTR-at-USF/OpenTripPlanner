...
public class GenericAStar implements SPTService { // maybe this should be wrapped in a component SPT
    ...
    @Setter
    private int nPaths = 3; // TODO this should really be set based on the routing request
    ...
    boolean iterate(RunState runState) {
        ...
        Collection<Edge> edges = runState.options.isArriveBy() ? runState.u_vertex.getIncoming()
                : runState.u_vertex.getOutgoing();
        for (Edge edge : edges) {
            ...
            for (State v = edge.traverse(runState.u); v != null; v = v.getNextResult()) {
                ...
                double remaining_w = computeRemainingWeight(runState.heuristic, v,
                        runState.rctx.target, runState.options);

                if (remaining_w < 0 || Double.isInfinite(remaining_w)) {
                    continue;
                }
                double estimate = v.getWeight() + remaining_w
                        * runState.options.getHeuristicWeight();
    ...
    void runSearch(RunState runState, long abortTime) {
        ...
        while (!runState.pq.empty()) { // Until the priority queue is empty:
            ...
            // runState.options.rctx.aborted = true; // signal search cancellation up to higher
            // stack frames
            // runState.options.rctx.debugOutput.timedOut = true; // signal timeout in debug output
            // object
            //
            // break;
            // }
             ...
             * 
             * Note that termination is checked after adjacent states are added. This presents the
             * small inefficiency that adjacent states are generated for a state which could be the
             * last one you need to check. The advantage of this is that the algorithm is always
             * left in a restartable state, which is useful for debugging or potential future
             * variations.
             */
            if (!iterate(runState)) {
                continue;
            ...
            if (runState.terminationStrategy != null) {
                if (!runState.terminationStrategy.shouldSearchContinue(runState.rctx.origin,
                        runState.rctx.target, runState.u, runState.spt, runState.options))

                    break;
                // TODO AMB: Replace isFinal with bicycle conditions in BasicPathParser
            } else if (!runState.options.batch && runState.u_vertex == runState.rctx.target
                    && runState.u.isFinal() && runState.u.allPathParsersAccept()) {
