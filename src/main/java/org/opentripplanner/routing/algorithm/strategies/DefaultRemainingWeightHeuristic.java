...
/**
 * A Euclidean remaining weight strategy that takes into account transit
 * boarding costs where applicable.
 * 
 */
public class DefaultRemainingWeightHeuristic implements
		RemainingWeightHeuristic {

	private static final long serialVersionUID = -5172878150967231550L;
	private static Logger LOG = LoggerFactory
			.getLogger(DefaultRemainingWeightHeuristic.class);
	...
	/**
	 * On a non-transit trip, the remaining weight is simply distance / street
	 * speed. On a transit trip, there are two cases: (1) we're not on a transit
	 * vehicle. In this case, there are two possible ways to compute the
	 * remaining distance, and we take whichever is smaller: (a) walking
	 * distance / walking speed (b) boarding cost + transit distance / transit
	 * speed (this is complicated a bit when we know that there is some walking
	 * portion of the trip). (2) we are on a transit vehicle, in which case the
	 * remaining weight is simply transit distance / transit speed (no need for
	 * boarding cost), again considering any mandatory walking.
	 */
	@Override
	public double computeForwardWeight(State s, Vertex target) {
		Vertex sv = s.getVertex();
		double euclideanDistance = distanceLibrary.fastDistance(sv.getY(),
				sv.getX(), targetY, targetX);
		if (transit) {
			if (euclideanDistance < requiredWalkDistance) {
				return walkReluctance * euclideanDistance / maxStreetSpeed;
			}
			/*
			 * Due to the above conditional, the following value is known to be
			 * positive.
			 */
			double transitWeight = (euclideanDistance - requiredWalkDistance)
					/ maxTransitSpeed;
			double streetWeight = walkReluctance
					* (requiredWalkDistance / maxStreetSpeed);
	...
	/**
	 * Figure out the minimum amount of walking to reach the destination from
	 * transit. This is done by doing a Dijkstra search for the first reachable
	 * transit stop.
	 */
	private double determineRequiredWalkDistance(RoutingRequest req) {
		if (!req.modes.isTransit())
			return 0; // required walk distance will be unused.
		...
		gd.searchTerminationStrategy = new SearchTerminationStrategy() {
			@Override
			public boolean shouldSearchTerminate(Vertex origin, Vertex target,
	...
	static class ClosestStopTraverseVisitor implements TraverseVisitor {
		...
		public void visitVertex(State state) {
			if (state.getVertex() instanceof TransitStop) {
				...
				LOG.info("Found closest stop to search target: {} at {}m",
	 ...
	 * computeReverseWeight did not have the localStreetService clause). They
	 * have been merged.
	 */
	@Override
