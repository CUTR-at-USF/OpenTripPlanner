...
public class InterleavedBidirectionalHeuristic implements RemainingWeightHeuristic {
    ...
    /*
     * Implementation observations: 1. filling weights array with inf is expensive (~70 msec in PDX)
     * 2. keeping a separate closed bitset is ugly (and was not threadsafe) 3. street movement is
     * "almost Euclidean" in cost 4. precomputing 20km origin/destination circles takes ~30 msec
     * each 1 or 2km takes only about 5msec each. 20km using hashmap weights takes ~150 msec each
     * AND search gets really slow due to unlimited walking
     * 
     * 5. we could get the and calculate origin circle on the fly using distance 6. we could use a
     * HashMap for weights 0. Raising walk distance severely affects bidi heuristic performance
    ...
    public void doSomeWork() {
        ...
        for (int i = 0; i < HEURISTIC_STEPS_PER_MAIN_STEP; ++i) {
            ...
            // // Ignore vertices that could be rekeyed (but are not rekeyed in this
            // implementation).
     ...
     * 
     * All on-street vertices must be explored by the heuristic before the main search starts. This
     * allows us to completely skip walking outside a certain radius of the origin/destination.
     */
    @Override
    public double computeReverseWeight(State s, Vertex target) {
        ...
        if (v instanceof StreetVertex)
            ...
            double dist = distanceLibrary.fastDistance(v.getY(), v.getX(), target.getY(),
                    target.getX());
    ...
    /*
     * 
     * Main search always proceeds from the origin to the target (arriveBy or departAfter) heuristic
     * search always proceeds outward from the target (arriveBy or departAfter)
     * 
     * when main search is departAfter: it gets outgoing edges and traverses them with
     * arriveBy=false heuristic search gets incoming edges and traverses them with arriveBy=true
     * heuristic destination street search also gets incoming edges and traverses them with
     * arriveBy=true heuristic origin street search gets outgoing edges and traverses them with
     * arriveBy=false
     * 
     * when main search is arriveBy: it gets incoming edges and traverses them with arriveBy=true
     * heuristic search gets outgoing edges and traverses them with arriveBy=false heuristic
     ...
     * 
     * We traverse using the real traverse method rather than the lower bound traverse method
     ...
     * only walk within that distance. This would avoid needing to call the main traversal
     * functions.
     * 
     ...
     * 
     * The other ultra-simple option is to just set the heuristic value to 0 for all on-street
     * locations within walking distance of the origin and destination.
     * 
