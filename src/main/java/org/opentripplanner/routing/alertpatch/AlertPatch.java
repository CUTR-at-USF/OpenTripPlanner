...
/**
 * This adds a note to all boardings of a given route or stop (optionally, in a
 * given direction)
 *
 * @author novalis
 *
...
public class AlertPatch implements Serializable {
	...
	public void apply(Graph graph) {
		Agency agency = this.agency != null ? graph.index.agencyForId
				.get(this.agency) : null;
		Route route = this.route != null ? graph.index.routeForId
				.get(this.route) : null;
		Stop stop = this.stop != null ? graph.index.stopForId.get(this.stop)
				: null;
		Trip trip = this.trip != null ? graph.index.tripForId.get(this.trip)
				: null;
		...
		if (route != null || trip != null || agency != null) {
			...
			for (TripPattern tripPattern : tripPatterns) {
				if (direction != null
						&& !direction.equals(tripPattern.getDirection())) {
					continue;
				}
				for (int i = 0; i < tripPattern.stopPattern.stops.length; i++) {
					if (stop == null
							|| stop.equals(tripPattern.stopPattern.stops[i])) {
	...
	public int hashCode() {
		return ((direction == null ? 0 : direction.hashCode())
				...
				+ (route == null ? 0 : route.hashCode()) + (alert == null ? 0
					: alert.hashCode()));
	}
}
