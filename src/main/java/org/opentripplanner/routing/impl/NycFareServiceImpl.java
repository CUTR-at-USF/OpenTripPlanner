...
public class NycFareServiceImpl implements FareService, Serializable {
    ...
    public Fare getCost(GraphPath path) {
        ...
        final List<AgencyAndId> SIR_BONUS_STOPS = makeMtaStopList("140", "420", "419", "418",
                "M22", "M23", "R27", "R26");
        ...
        for (State state : states) {
            ...
            if (routeId == null) {
                ...
                if (newRide == null || !routeId.equals(newRide.route)) {
                    ...
                    } else if (shortName.startsWith("X") || shortName.startsWith("BxM")
                            || shortName.startsWith("QM") || shortName.startsWith("BM")) {
        ...
        for (Ride ride : rides) {
            ...
            case SUBWAY_PRE_TRANSFER_WALKED:
                if (ride.classifier.equals(SUBWAY)) {
                    ...
                    // lex and 59/63
                    if (!(lexFreeTransfer && SUBWAY_FREE_TRANSFER_STOPS.contains(ride.firstStop
                            .getId()))) {
                        totalFare += ORDINARY_FARE;
                    }
            ...
            case SUBWAY_POST_TRANSFER:
                if (ride.classifier.equals(WALK)) {
                    if (!canarsieFreeTransfer) {
                        /*
                         * note: if we end up walking to another subway after alighting at Canarsie,
                         * we will mistakenly not be charged, but nobody would ever do this
                         */
                        state = NycFareState.INIT;
                    }
                } else if (ride.classifier.equals(SIR)) {
