...
/**
 * A set of edges on a single route, with associated information for calculating
 * fares
 */
class Ride {
...
/**
 * This fare service impl handles the cases that GTFS handles within a single
 * feed. It cannot necessarily handle multi-feed graphs, because a rule-less
 * fare attribute might be applied to rides on routes in another feed, for
 * example. For more interesting fare structures like New York's MTA, or cities
 * with multiple feeds and inter-feed transfer rules, you get to implement your
...
public class DefaultFareServiceImpl implements FareService, Serializable {
	...
	public Fare getCost(GraphPath path) {
		...
		if (fareAttributes.size() > 0) {
			currency = Currency.getInstance(fareAttributes.values().iterator()
					.next().getCurrencyType());
		...
		if (lowestCost != Float.POSITIVE_INFINITY) {
			...
			int cents = (int) Math.round(lowestCost
	...
	public float getLowestCost(List<Ride> rides) {
		...
		// Cell [i,j] holds the best (lowest) cost for a trip from rides[i] to
		// rides[j]
	...
	protected float calculateCost(List<Ride> rides) {
		...
		// stops don't really have an agency id, they have the per-feed default
		// id
		...
		for (AgencyAndId fareId : fareAttributes.keySet()) {
			// fares also don't really have an agency id, they will have the
			// per-feed default id
			if (!fareId.getAgencyId().equals(feedId))
				continue;
			FareRuleSet ruleSet = fareRules.get(fareId);
			if (ruleSet == null
					|| ruleSet.matches(startZone, endZone, zones, routes)) {
