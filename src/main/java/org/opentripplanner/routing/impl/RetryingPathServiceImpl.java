...
public class RetryingPathServiceImpl implements PathService {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(RetryingPathServiceImpl.class);
	...
	/**
	 * Give up on searching for the first itinerary after this many seconds have
	 * elapsed. A negative or zero value means search forever.
	 */
	...
	/**
	 * Stop searching for additional itineraries (beyond the first one) after
	 * this many seconds have elapsed, relative to the beginning of the search
	 * for the first itinerary. A negative or zero value means search forever.
	 * Setting this lower than the firstPathTimeout will avoid searching for
	 * additional itineraries when finding the first itinerary takes a long
	 * time. This helps keep overall response time down while assuring that the
	 * end user will get at least one response.
	 */
	...
	public List<GraphPath> getPaths(RoutingRequest options) {
		...
		// The list of options specifying various modes, banned routes, etc to
		// try for multiple
		// itineraries
		...
		while (paths.size() < options.numItineraries) {
			...
			double timeout = paths.isEmpty() ? firstPathTimeout
					: multiPathTimeout;
			...
			ShortestPathTree spt = sptService.getShortestPathTree(currOptions,
					timeout);
			if (spt == null) {
				// Serious failure, no paths provided. This could be signaled
				// with an exception.
				LOG.warn(
						"Aborting search. {} paths found, elapsed time {} sec",
						paths.size(),
						(System.currentTimeMillis() - searchBeginTime) / 1000.0);
				break;
			}
			List<GraphPath> somePaths = spt.getPaths(); // somePaths may be
														// empty, but is never
														// null.
			...
			/*
			 * First, accumulate any new paths found into the list of
			 * itineraries.
			 */
			for (GraphPath path : somePaths) {
				if (!paths.contains(path)) {
					...
					// unless is is a non-transit trip (in which case this would
					// cause a useless retry)
			...
			LOG.debug("{} / {} itineraries", paths.size(),
					currOptions.numItineraries);
			...
			if (maxWeight == Double.MAX_VALUE && maxWalk == Double.MAX_VALUE) {
				/*
				 * the worst trip we are willing to accept is at most twice as
				 * bad or twice as long
				 */
				...
				maxTime = path.getStartTime() + MAX_TIME_FACTOR
						* (currOptions.arriveBy ? -duration : duration);
			...
			if (somePaths.isEmpty()) {
				...
				if (maxWalk > initialMaxWalk * MAX_WALK_MULTIPLE
						|| maxWalk >= Double.MAX_VALUE)
					break;
				maxWalk *= 2;
