...
public class CandidateEdge {
	...
	// Massive preference for streets that allow cars, also applied to platforms
	// for vehicles of
	// the specified mode.
	...
	public static class CandidateEdgeScoreComparator implements
			Comparator<CandidateEdge> {
		@Override
	...
	/**
	 * Construct CandidateEdge based on a GenericLocation. The edge's score is
	 * calculated as the final step of construction.
	 */
	public CandidateEdge(StreetEdge e, GenericLocation loc, double pref,
	...
	/**
	 * Get the platform mask for the given mode. This is compatible with the bit
	 * flags from StreetEdge.getStreetClass().
	 */
	...
	/**
	 * Internal calculator for the score. Assumes that edge, platform and
	 * distance are initialized.
	 */
	private double calcScore(TraverseModeSet modes) {
		...
		// apply strong preference to car edges and to platforms for the
		// specified modes
		// only apply for car modes -- TODO Check this.
		if (modes.getDriving()
				&& edge.getPermission().allows(StreetTraversalPermission.CAR)
