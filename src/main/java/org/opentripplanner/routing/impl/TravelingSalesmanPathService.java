...
public class TravelingSalesmanPathService implements PathService {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(TravelingSalesmanPathService.class);

	public GraphService graphService;

	public TravelingSalesmanPathService(GraphService graphService,
			PathService chainedPathService) {
	...
	public List<GraphPath> getPaths(RoutingRequest options) {
		...
		if (options.intermediatesEffectivelyOrdered()) {
			...
			for (Vertex v : vertices) {
				...
				List<GraphPath> partialPaths = chainedPathService
						.getPaths(options);
		...
		// Difficult case: intermediate places can occur in any order (Traveling
		// Salesman Problem)
		...
		List<Vertex> intermediates = (List<Vertex>) options.rctx.intermediateVertices
				.clone();
		for (Vertex v : intermediates) {

			/*
			 * Find initial paths from the source vertex to the intermediate
			 * vertex
			 */
			options.dateTime = time;
			...
			/*
			 * Find paths from this intermediate vertex to each other
			 * intermediate
			 */
			...
			for (Vertex tv : intermediates) {
				/*
				 * We don't need to compute paths where the source and target
				 * vertex are the same
				 */
				if (v == tv)
					continue;
				...
				List<GraphPath> morePaths = chainedPathService
						.getPaths(options);
