...
/**
 * This PathService is intended to provide faster response times when routing
 * over longer distances (e.g. across the entire Netherlands). It only uses the
 * street network at the first and last legs of the trip, and all other
 * transfers between transit vehicles will occur via SimpleTransfer edges which
 * must be created by the graph builder.
 * 
...
public class LongDistancePathService implements PathService {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(LongDistancePathService.class);
	...
	public List<GraphPath> getPaths(RoutingRequest options) {
		...
		LOG.debug("END SEARCH ({} msec)", System.currentTimeMillis()
				- searchBeginTime);
	...
	public static class Parser extends PathParser {
		...
		static {
			...
			/*
			 * A TransitLeg is a ride on transit, including preboard and
			 * prealight edges at its ends. It begins and ends at a TransitStop
			 * vertex. Note that these are STATION* rather than STATION+ because
			 * some transfer edges (timed transfer edges) connect arrival and
			 * depart vertices. Requiring a STATION edge would prevent them from
			 * being traversed.
			 */
			Nonterminal transitLeg = seq(star(STATION), plus(ONBOARD),
					star(STATION));

			/*
			 * A beginning gets us from the path's initial vertex to the first
			 * transit stop it passes through (its first board location). We may
			 * want to transfer at the beginning of an itinerary that begins at
			 * a station or stop, and does not use streets.
			 */
			Nonterminal beginning = choice(seq(optional(streetLeg), LINK),
					seq(optional(STATION_STOP), optional(TRANSFER)));

			/*
			 * Begin on board transit, ending up at another stop where the
			 * "middle" can take over.
			 */
			Nonterminal onboardBeginning = seq(plus(ONBOARD), plus(STATION),
					optional(TRANSFER));

			/*
			 * Ride transit at least one time, chaining transit legs together
			 * with single transfer edges.
			 */
			Nonterminal middle = seq(transitLeg,
					star(optional(TRANSFER), transitLeg));

			/*
			 * And end gets us from the last stop to the final vertex. It is the
			 * same as a beginning, but with the sub-sequences reversed. This
			 * must cover 6 different cases: 1. leave the station and optionally
			 * walk, 2. stay at the stop where we are, 3. stay at the stop where
			 * we are but go to its parent station, 4. transfer and stay at the
			 * target stop, 5. transfer and move to the target stop's parent
			 * station.
			 */
			...
			/*
			 * An itinerary that includes a ride on public transit. It might
			 * begin on- or offboard. if it begins onboard, it doesn't
			 * necessarily have subsequent transit legs.
			 */
			Nonterminal transitItinerary = choice(seq(beginning, middle, end),
					seq(onboardBeginning, optional(middle), end));

			/*
			 * A streets-only itinerary, which might begin or end at a stop or
			 * its station, but does not actually ride transit.
			 */
			Nonterminal streetItinerary = choice(
					TRANSFER,
					seq(optional(STATION_STOP), optional(LINK), streetLeg,
		...
		/**
		 * The terminal is normally based exclusively on the backEdge, i.e. each
		 * terminal represents exactly one edge in the path. In case of
		 * @link{StationStopEdge}, however, the type of the current vertex also
		 * determines what kind of terminal this is.
		 */
		@Override
		public int terminalFor(State state) {
			...
			if (e instanceof StationStopEdge) {
				return state.getVertex() instanceof TransitStop ? STATION_STOP
						: STOP_STATION;
			}
			// There should perhaps be a shared superclass of all transfer edges
			// to simplify this.
