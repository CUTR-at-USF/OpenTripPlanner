 ...
 *
 * TODO we need a way to make sure all temporary edges are recorded as such and
 * assigned a routingcontext when they are created. That list should probably be
 * in the routingContext itself instead of the created StreetLocation.
 */
public class PartialStreetEdge extends StreetWithElevationEdge {
	...
	public PartialStreetEdge(StreetEdge parentEdge, StreetVertex v1,
	...
	public PartialStreetEdge(StreetEdge parentEdge, StreetVertex v1,
			StreetVertex v2, LineString geometry, String name, double length) {
		this(parentEdge, v1, v2, geometry, name, length, parentEdge
				.getPermission(), false);
	}
	...
	/**
	 * This implementation makes it so that TurnRestrictions on the parent edge
	 * are applied to this edge as well.
	 */
	@Override
	...
	public boolean isReverseOf(Edge e) {
		...
		// TODO(flamholz): is there a case where a partial edge has a reverse of
		// its own?
	...
	/**
	 * Returns true if this edge is trivial - beginning and ending at the same
	 * point.
	 */
	...
	public String toString() {
		return "PartialPlainStreetEdge(" + this.getName() + ", "
				+ this.getFromVertex() + " -> " + this.getToVertex()
				+ " length=" + this.getDistance() + " carSpeed="
				+ this.getCarSpeed() + " parentEdge=" + parentEdge + ")";
	}

	@Override
	public State traverse(State s0) {
		// Split edges should only be usable by the routing context that created
		// them.
		// This should alleviate the concurrency problem in issue 1025.
		// In the window of time before the visibleTo field is set, traversal
		// will also fail (which is what we want).
