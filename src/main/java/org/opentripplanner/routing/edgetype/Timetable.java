...
/**
 * Timetables provide most of the TripPattern functionality. Each TripPattern
 * may possess more than one Timetable when stop time updates are being applied:
 * one for the scheduled stop times, one for each snapshot of updated stop
 * times, another for a working buffer of updated stop times, etc.
 */
public class Timetable implements Serializable {
	...
	/**
	 * A circular reference between TripPatterns and their scheduled
	 * (non-updated) timetables.
	 */
	public final TripPattern pattern;

	/**
	 * Contains one TripTimes object for each scheduled trip (even cancelled
	 * ones) and possibly additional TripTimes objects for unscheduled trips.
	 * Frequency entries are stored separately.
	 */
	public final List<TripTimes> tripTimes = Lists.newArrayList();

	/**
	 * Contains one FrequencyEntry object for each block of frequency-based
	 * trips.
	 */
	...
	/**
	 * For each hop, the best running time. This serves to provide lower bounds
	 * on traversal time.
	 */
	private transient int minRunningTimes[];

	/**
	 * For each stop, the best dwell time. This serves to provide lower bounds
	 * on traversal time.
	 */
	private transient int minDwellTimes[];

	/**
	 * Helps determine whether a particular pattern is worth searching for
	 * departures at a given time.
	 */
	...
	/**
	 * Copy constructor: create an un-indexed Timetable with the same TripTimes
	 * as the specified timetable.
	 */
	...
	/**
	 * Before performing the relatively expensive iteration over all the trips
	 * in this pattern, check whether it's even possible to board any of them
	 * given the time at which we are searching, and whether it's possible that
	 * any of them could improve on the best known time. This is only an
	 * optimization, but a significant one. When we search for departures, we
	 * look at three separate days: yesterday, today, and tomorrow. Many
	 * patterns do not have service at all hours of the day or past midnight.
	 * This optimization can cut the search time for each pattern by 66 to 100
	 * percent.
	 *
	 * @param bestWait
	 *            -1 means there is not yet any best known time.
	 */
	public boolean temporallyViable(ServiceDay sd, long searchTime,
			int bestWait, boolean boarding) {
		// Check whether any services are running at all on this pattern.
		if (!sd.anyServiceRunning(this.pattern.services))
			return false;
		...
		if (boarding ? (searchTime > this.maxTime)
				: (searchTime < this.minTime))
			return false;
		// Check whether any trip can improve on the best time yet found
		if (bestWait >= 0) {
			long bestTime = boarding ? (searchTime + bestWait)
					: (searchTime - bestWait);
			if (boarding ? (bestTime < this.minTime)
					: (bestTime > this.maxTime))
				return false;
		}
		return true;
	}
	...
	/**
	 * Get the next (previous) trip that departs (arrives) from the specified
	 * stop at or after (before) the specified time.
	 * 
	 * @return the TripTimes object representing the (possibly updated) best
	 *         trip, or null if no trip matches both the time and other
	 *         criteria.
	 */
	public TripTimes getNextTrip(State s0, ServiceDay serviceDay,
			int stopIndex, boolean boarding) {
		/*
		 * Search at the state's time, but relative to midnight on the given
		 * service day.
		 */
		...
		// NOTE the time is sometimes negative here. That is fine, we search for
		...
		// 1. we allow combining trips from different service IDs on the same
		// tripPattern.
		// 2. We mix frequency-based and one-off TripTimes together on
		// tripPatterns.
		// 3. Stoptimes may change with realtime updates, and we cannot count on
		// them being sorted.
		// The complexity of keeping sorted indexes up to date does not appear
		// to be worth the
		// apparently minor speed improvement.
		int bestTime = boarding ? Integer.MAX_VALUE : Integer.MIN_VALUE;
		// Hoping JVM JIT will distribute the loop over the if clauses as
		// needed.
		// We could invert this and skip some service days based on schedule
		// overlap as in RRRR.
		for (TripTimes tt : tripTimes) {
			...
			int adjustedTime = adjustTimeForTransfer(s0, currentStop, tt.trip,
					boarding, serviceDay, time);
			if (adjustedTime == -1)
				continue;
			if (boarding) {
		...
		if (!rtUpdatesAvailable) {
			...
			for (FrequencyEntry freq : frequencyEntries) {
				...
				if (!tt.tripAcceptable(s0, stopIndex))
					continue;
				int adjustedTime = adjustTimeForTransfer(s0, currentStop,
						tt.trip, boarding, serviceDay, time);
				if (adjustedTime == -1)
					continue;
				LOG.debug("  running freq {}", freq);
				if (boarding) {
					int depTime = freq.nextDepartureTime(stopIndex,
							adjustedTime); // min transfer time included in
											// search
					if (depTime < 0)
						continue;
					if (depTime >= adjustedTime && depTime < bestTime) {
					...
					int arvTime = freq.prevArrivalTime(stopIndex, adjustedTime); // min
																					// transfer
																					// time
																					// included
																					// in
			...
			if (bestFreq != null) {
				...
				bestTrip = bestFreq.tripTimes.timeShift(stopIndex, bestTime,
						boarding);
			}
		}
		return bestTrip;
	 ...
	 * return the boarding time t0 adjusted for this particular trip's minimum
	 * transfer time, or -1 if boarding this trip is not allowed. FIXME
	 * adjustedTime can legitimately be -1! But negative times might as well be
	 * zero.
	 */
	private int adjustTimeForTransfer(State state, Stop currentStop, Trip trip,
			boolean boarding, ServiceDay serviceDay, int t0) {
		...
		int transferTime = transferTable.getTransferTime(
				state.getPreviousStop(), currentStop, state.getPreviousTrip(),
				trip, boarding);
		...
		// There is a minimum transfer time to make this transfer. Ensure that
		// it is respected.
		int minTime = serviceDay.secondsSinceMidnight(state
	...
	/**
	 * Finish off a Timetable once all TripTimes have been added to it. This
	 * involves caching lower bounds on the running times and dwell times at
	 * each stop, and may perform other actions to compact the data structure
	 * such as trimming and deduplicating arrays.
	 */
	public void finish() {
		...
		/*
		 * Find the time range over which this timetable is active. Allows
		 * departure search optimizations.
		 */
		...
		// Again it seems reasonable to have a shared interface between
		// FrequencyEntries and normal TripTimes.
		for (FrequencyEntry freq : frequencyEntries) {
	...
	/**
	 * @return the index of TripTimes for this trip ID in this particular
	 *         Timetable
	 */
	public int getTripIndex(AgencyAndId tripId) {
		...
		int ret = 0;
		for (TripTimes tt : tripTimes) {
			// could replace linear search with indexing in stoptime updater,
			// but not necessary
	...
	/**
	 * @return the index of TripTimes for this trip ID in this particular
	 *         Timetable, ignoring AgencyIds.
	 */
	public int getTripIndex(String tripId) {
		int ret = 0;
		for (TripTimes tt : tripTimes) {
			if (tt.trip.getId().getId().equals(tripId))
				return ret;
			ret += 1;
		}
		return -1;
	 ...
	 * The existing TripTimes must not be modified directly because they may be
	 * shared with the underlying scheduledTimetable, or other updated
	 * Timetables. The StoptimeUpdater performs the protective copying of this
	 * Timetable. It is not done in this update method to avoid repeatedly
	 * cloning the same Timetable when several updates are applied to it at
	 * once. We assume here that all trips in a timetable are from the same
	 * feed, which should always be the case.
	 *
	 * @return whether or not the timetable actually changed as a result of this
	 *         operation (maybe it should do the cloning and return the new
	 *         timetable to enforce copy-on-write?)
	 */
	public boolean update(TripUpdate tripUpdate, TimeZone timeZone,
		...
		if (tripUpdate == null) {
			...
			try {
				// Though all timetables have the same trip ordering, some may
				// have extra trips due to
				// the dynamic addition of unscheduled trips.
				// However, we want to apply trip updates on top of *scheduled*
				// times
				...
				if (tripIndex == -1) {
					...
					LOG.trace(
							"tripId {} found at index {} in scheduled timetable.",
							tripId, tripIndex);
				}
				...
				if (tripDescriptor.hasScheduleRelationship()
					...
					// The GTFS-RT reference specifies that StopTimeUpdates are
					// sorted by stop_sequence.
					Iterator<StopTimeUpdate> updates = tripUpdate
							.getStopTimeUpdateList().iterator();
					if (!updates.hasNext()) {
						LOG.warn(
								"Won't apply zero-length trip update to trip {}.",
								tripId);
						return false;
					}
					...
					for (int i = 0; i < numStops; i++) {
						...
						if (update != null) {
							if (update.hasStopSequence()) {
								match = update.getStopSequence() == newTimes
										.getStopSequence(i);
						...
						if (match) {
							StopTimeUpdate.ScheduleRelationship scheduleRelationship = update
									.hasScheduleRelationship() ? update
							...
							if (scheduleRelationship == StopTimeUpdate.ScheduleRelationship.SKIPPED) {
								...
								long today = updateServiceDate.getAsDate(
										timeZone).getTime() / 1000;
								...
								if (update.hasArrival()) {
									...
									if (arrival.hasDelay()) {
										...
										if (arrival.hasTime()) {
											newTimes.updateArrivalTime(
													i,
													(int) (arrival.getTime() - today));
										} else {
											newTimes.updateArrivalDelay(i,
													delay);
										}
										...
										LOG.error(
												"Arrival time at index {} is erroneous.",
												i);
										return false;
									}
								} else {
								...
								if (update.hasDeparture()) {
									StopTimeEvent departure = update
											.getDeparture();
									if (departure.hasDelay()) {
										...
										if (departure.hasTime()) {
											...
											newTimes.updateDepartureDelay(i,
													delay);
										}
										...
										LOG.error(
												"Departure time at index {} is erroneous.",
												i);
										return false;
									}
								} else {
				...
				// Update succeeded, save the new TripTimes back into this
				// Timetable.
	...
	/**
	 * 
	 * Apply the TripUpdate of FrequnecyBased trips (exact_time = 0) to the
	 * appropriate TripTimes from this Timetable. TODO:Handel FrequencyBased
	 * trips with "exact_time = 1"
	 * 
	 * @return
	 */
		...
		if (tripUpdate == null) {
			...
			try {
				...
				if (tripDescriptor.hasScheduleRelationship()
					...
					// estimate arrival time for head of queue, eg:[0, 0, t3,
					// t4]--> [t1, t2, t3,
					// t4]
	...
	/**
	 * Add a trip to this Timetable. The Timetable must be analyzed, compacted,
	 * and indexed any time trips are added, but this is not done automatically
	 * because it is time consuming and should only be done once after an entire
	 * batch of trips are added. Note that the trip is not added to the
	 * enclosing pattern here, but in the pattern's wrapper function. Here we
	 * don't know if it's a scheduled trip or a realtime-added trip.
	 */
	...
	/**
	 * Add a frequency entry to this Timetable. See addTripTimes method. Maybe
	 * Frequency Entries should just be TripTimes for simplicity.
	 */
	...
	/**
	 * Check that all dwell times at the given stop are zero, which allows
	 * removing the dwell edge. TODO we should probably just eliminate
	 * dwell-deletion. It won't be important if we get rid of transit edges.
	 */
	...
	/**
	 * Find and cache service codes. Duplicates information in trip.getServiceId
	 * for optimization.
	 */
