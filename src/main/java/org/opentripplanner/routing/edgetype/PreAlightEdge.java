 ...
 * as well as transfer limits, timed and preferred transfer rules, transfer
 * penalties, and boarding costs. This avoids applying these costs/rules
 * repeatedly in (Pattern)Board edges. These are single station or
...
public class PreAlightEdge extends FreeEdge implements StationEdge {
	...
	public PreAlightEdge(TransitStopArrive from, TransitStop to) {
		...
		if (!(to instanceof TransitStop))
			throw new IllegalStateException(
					"Pre-alight edges must lead to a transit stop.");
	}
	...
	public State traverse(State s0) {
		...
		// they are currently only different because the
		// StateEditor.incrementTime methods are not
		// used.
		...
		if (options.arriveBy) {
			...
			// Return null here rather than in StreetTransitLink so that
			// walk-only
			// options can be used to find transit stops without boarding
			// vehicles.
			...
			/*
			 * look in the global transfer table for the rules from the previous
			 * stop to this stop.
			 */
			...
			if (s0.isEverBoarded()) {
				// this is not the first boarding, therefore we must have
				// "transferred" -- whether
