...
public abstract class RentABikeAbstractEdge extends Edge {
	...
	protected State traverseDropoff(State s0) {
		 ...
		 */
		if (!s0.isBikeRenting()
				|| !hasCompatibleNetworks(networks, s0.getBikeRentalNetworks()))
			return null;
		BikeRentalStationVertex pickup = (BikeRentalStationVertex) tov;
		if (options.useBikeRentalAvailabilityInformation
				&& pickup.getSpacesAvailable() == 0) {
			return null;
		}
	...
	/**
	 * @param stationNetworks
	 *            The station where we want to drop the bike off.
	 * @param rentedNetworks
	 *            The set of networks of the station we rented the bike from.
	 * @return true if the bike can be dropped off here, false if not.
	 */
	private boolean hasCompatibleNetworks(Set<String> stationNetworks,
