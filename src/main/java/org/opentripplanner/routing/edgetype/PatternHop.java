 ...
 * TripPattern.
 */
public class PatternHop extends TablePatternEdge implements OnboardEdge,
		HopEdge {
	...
	public PatternHop(PatternStopVertex from, PatternStopVertex to, Stop begin,
			Stop end, int stopIndex) {
		super(from, to);
		this.begin = begin;
		this.end = end;
	...
	public double getDistance() {
		return SphericalDistanceLibrary.getInstance().distance(begin.getLat(),
				begin.getLon(), end.getLat(), end.getLon());
	}
	...
	public State optimisticTraverse(State state0) {
		...
		if (!options.bannedStopsHard.isEmpty()) {
			if (options.bannedStopsHard.matches(((PatternStopVertex) fromv)
					.getStop())
					|| options.bannedStopsHard
							.matches(((PatternStopVertex) tov).getStop())) {
				return null;
			}
		}

		int runningTime = getPattern().scheduledTimetable
