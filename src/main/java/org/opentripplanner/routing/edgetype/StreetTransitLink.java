...
public class StreetTransitLink extends Edge {
	...
	public State traverse(State s0) {
		...
		// Do not check here whether any transit modes are selected. A check for
		// the presence of
		// transit modes will instead be done in the following PreBoard edge.
		// This allows searching for nearby transit stops using walk-only
		// options.
		StateEditor s1 = s0.edit(this);

		/*
		 * Only enter stations in CAR mode if parking is not required (kiss and
		 * ride)
		 */
		/*
		 * Note that in arriveBy searches this is double-traversing link edges
		 * to fork the state into both WALK and CAR mode. This is an insane
		 * hack.
		 */
		...
		s1.incrementTimeInSeconds(transitStop.getStreetToStopTime()
				+ STL_TRAVERSE_COST);
		s1.incrementWeight(STL_TRAVERSE_COST
				+ transitStop.getStreetToStopTime());
	...
	// anecdotally, the lower bound search is about 2x faster when you don't
	// reach stops
