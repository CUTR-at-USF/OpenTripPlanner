...
public class StreetEdge extends Edge implements Cloneable {
	...
	/**
	 * Length is stored internally as 32-bit fixed-point (millimeters). This
	 * allows edges of up to ~2100km. Distances used in calculations and exposed
	 * outside this class are still in double-precision floating point meters.
	 * Someday we might want to convert everything to fixed point
	 * representations.
	 */
	private int length_mm;

	/**
	 * bicycleSafetyWeight = length * bicycleSafetyFactor. For example, a 100m
	 * street with a safety factor of 2.0 will be considered in term of safety
	 * cost as the same as a 150m street with a safety factor of 1.0.
	 */
	...
	/**
	 * The speed (meters / sec) at which an automobile can traverse this street
	 * segment.
	 */
	private float carSpeed;

	/**
	 * The angle at the start of the edge geometry. Internal representation is
	 * -180 to +179 integer degrees mapped to -128 to +127 (brads)
	 */
	private byte inAngle;

	/**
	 * The angle at the start of the edge geometry. Internal representation like
	 * that of inAngle.
	 */
	...
	public StreetEdge(StreetVertex v1, StreetVertex v2, LineString geometry,
		...
		this.length_mm = (int) (length * 1000); // CONVERT FROM FLOAT METERS TO
												// FIXED MILLIMETERS
		...
		if (geometry != null) {
			try {
				...
				// Conversion from radians to internal representation as a
				// single signed byte.
				// We also reorient the angles since OTP seems to use South as a
				// reference
				// while the azimuth functions use North.
				// FIXME Use only North as a reference, not a mix of North and
				// South!
				// Range restriction happens automatically due to Java signed
				// overflow behavior.
				// 180 degrees exists as a negative rather than a positive due
				// to the integer range.
				double angleRadians = DirectionUtils.getLastAngle(geometry);
				outAngle = (byte) Math
						.round(angleRadians * 128 / Math.PI + 128);
	...
	public double getDistance() {
		return length_mm / 1000.0; // CONVERT FROM FIXED MILLIMETERS TO FLOAT
									// METERS
	}
	...
	public State traverse(State s0) {
		...
		/*
		 * Kiss and ride support. Mode transitions occur without the explicit
		 * loop edges used in park-and-ride.
		 */
		if (options.kissAndRide) {
			if (options.arriveBy) {
				// Branch search to "unparked" CAR mode ASAP after transit has
				// been used.
				// Final WALK check prevents infinite recursion.
				if (s0.isCarParked() && s0.isEverBoarded()
						&& currMode == TraverseMode.WALK) {
					editor = doTraverse(s0, options, TraverseMode.CAR);
					if (editor != null) {
						editor.setCarParked(false); // Also has the effect of
													// switching to CAR
						State forkState = editor.makeState();
						if (forkState != null) {
							forkState.addToExistingResultChain(state);
							return forkState; // return both parked and unparked
												// states
						}
					}
				}
				...
				// means being dropped off in this case.
				...
				if (!s0.isCarParked()
					...
					if (editor != null) {
						editor.setCarParked(true); // has the effect of
													// switching to WALK and
													// preventing further car
													// use
						return editor.makeState(); // return only the "parked"
													// walking state
					}
	...
	/**
	 * return a StateEditor rather than a State so that we can make parking/mode
	 * switch modifications for kiss-and-ride.
	 */
	private StateEditor doTraverse(State s0, RoutingRequest options,
		...
		if (backEdge != null) {
			// No illegal U-turns.
			// NOTE(flamholz): we check both directions because both edges get a
			// chance to decide
			// if they are the reverse of the other. Also, because it doesn't
			// matter which direction
			// we are searching in - these traversals are always disallowed
			// (they are U-turns in one direction
			// or the other).
		...
		/*
		 * Check whether this street allows the current mode. If not and we are
		 * biking, attempt to walk the bike.
		 */
		...
		double weight;
		// TODO(flamholz): factor out this bike, wheelchair and walking specific
		// logic to somewhere central.
		if (options.wheelchairAccessible) {
			...
			if (traverseMode.equals(TraverseMode.WALK)) {
				...
				double costs = ElevationUtils.getWalkCostsForSlope(
						getDistance(), getMaxSlope());
				// as the cost walkspeed is assumed to be for 4.8km/h (= 1.333
				// m/sec) we need to adjust
				// for the walkspeed set by the user
				double elevationUtilsSpeed = 4.0 / 3.0;
				weight = costs * (elevationUtilsSpeed / speed);
				time = weight; // treat cost as time, as in the current model it
								// actually is the same (this can be checked for
								// maxSlope == 0)
				/*
				 * // debug code if(weight > 100){ double timeflat = length /
				 * speed; System.out.format(
				 * "line length: %.1f m, slope: %.3f ---> slope costs: %.1f , weight: %.1f , time (flat):  %.1f %n"
				 * , length, elevationProfile.getMaxSlope(), costs, weight,
				 * timeflat); }
				 */
			}
		}
		...
		if (backEdge != null && backEdge instanceof StreetEdge) {
			...
			RoutingRequest backOptions = backWalkingBike ? s0.getOptions().bikeWalkingOptions
					: s0.getOptions();
			...
			/*
			 * This is a subtle piece of code. Turn costs are evaluated
			 * differently during forward and reverse traversal. During forward
			 * traversal of an edge, the turn *into* that edge is used, while
			 * during reverse traversal, the turn *out of* the edge is used.
			 * 
			 * However, over a set of edges, the turn costs must add up the same
			 * (for general correctness and specifically for reverse
			 * optimization). This means that during reverse traversal, we must
			 * also use the speed for the mode of the backEdge, rather than of
			 * the current edge.
			 */
			if (options.arriveBy && tov instanceof IntersectionVertex) { // arrive-by
																			// search
				IntersectionVertex traversedVertex = ((IntersectionVertex) tov);

				realTurnCost = backOptions.getIntersectionTraversalCostModel()
						.computeTraversalCost(traversedVertex, this, backPSE,
								backMode, backOptions, (float) speed,
								(float) backSpeed);
				...
				realTurnCost = options.getIntersectionTraversalCostModel()
						.computeTraversalCost(traversedVertex, backPSE, this,
								traverseMode, options, (float) backSpeed,
								(float) speed);
			} else {
				// In case this is a temporary edge not connected to an
				// IntersectionVertex
			...
			if (!traverseMode.isDriving()) {
				s1.incrementWalkDistance(realTurnCost / 100); // just a
																// tie-breaker
			}
		...
		/*
		 * On the pre-kiss/pre-park leg, limit both walking and driving, either
		 * soft or hard.
		 */
		...
		/*
		 * Apply a strategy for avoiding walking too far, either soft (weight
		 * increases) or hard limiting (pruning).
		 */
		if (s1.weHaveWalkedTooFar(options)) {
			...
			if (options.softWalkLimiting) {
				...
				weight += calculateOverageWeight(s0.getWalkDistance(),
						s1.getWalkDistance(), options.getMaxWalkDistance(),
	...
	private double calculateOverageWeight(double firstValue,
			double secondValue, double maxValue, double softPenalty,
			double overageRate) {
		...
		return (overageRate * overageValue)
				+ (applyPenalty ? softPenalty : 0.0);
	}
	...
	/**
	 * Calculate the speed appropriately given the RoutingRequest and
	 * traverseMode.
	 */
	private double calculateSpeed(RoutingRequest options,
			TraverseMode traverseMode) {
	...
	public String toString() {
		return "PlainStreetEdge(" + getId() + ", " + name + ", " + fromv
				+ " -> " + tov + " length=" + this.getDistance() + " carSpeed="
				+ this.getCarSpeed() + " permission=" + this.getPermission()
				+ ")";
	}
	...
	public boolean canTurnOnto(Edge e, State state, TraverseMode mode) {
		...
		for (TurnRestriction restriction : graph.getTurnRestrictions(this)) {
			/*
			 * FIXME: This is wrong for trips that end in the middle of
			 * restriction.to
			 */

			// NOTE(flamholz): edge to be traversed decides equivalence. This is
			// important since
			// it might be a temporary edge that is equivalent to some graph
			// edge.
			if (restriction.type == TurnRestrictionType.ONLY_TURN) {
				if (!e.isEquivalentTo(restriction.to)
						&& restriction.modes.contains(mode)
		...
		if (fromv != null) {
			for (Edge e : fromv.getIncoming()) {
				if (e instanceof StreetEdge) {
					for (TurnRestriction restriction : graph
							.getTurnRestrictions(e)) {
	...
	public LineString getGeometry() {
		return CompactLineString.uncompactLineString(fromv.getLon(),
				fromv.getLat(), tov.getLon(), tov.getLat(), compactGeometry,
				isBack());
	}

	private void setGeometry(LineString geometry) {
		this.compactGeometry = CompactLineString
				.compactLineString(fromv.getLon(), fromv.getLat(),
						tov.getLon(), tov.getLat(),
	...
	public void shareData(StreetEdge reversedEdge) {
		if (Arrays.equals(compactGeometry, reversedEdge.compactGeometry)) {
			...
			LOG.warn("Can't share geometry between {} and {}", this,
					reversedEdge);
		}
	}
	...
	/**
	 * Return the azimuth of the first segment in this edge in integer degrees
	 * clockwise from South. TODO change everything to clockwise from North
	 */
	...
	/**
	 * Return the azimuth of the last segment in this edge in integer degrees
