...
public enum StreetTraversalPermission {
	NONE(0), PEDESTRIAN(1), BICYCLE(2), PEDESTRIAN_AND_BICYCLE(2 | 1), CAR(4), PEDESTRIAN_AND_CAR(
			4 | 1), BICYCLE_AND_CAR(4 | 2), PEDESTRIAN_AND_BICYCLE_AND_CAR(
			4 | 2 | 1),
	...
	CUSTOM_MOTOR_VEHICLE(8), PEDESTRIAN_AND_CUSTOM_MOTOR_VEHICLE(8 | 1), BICYCLE_AND_CUSTOM_MOTOR_VEHICLE(
			...
			8 | 4 | 1), BICYCLE_AND_DRIVING(8 | 4 | 2), ALL(8 | 4 | 2 | 1), CROSSHATCHED(
			16); // this street exists in both Beszel and Ul Qoma; traffic
					// direction may depend on which city you're in.

	private static final Map<Integer, StreetTraversalPermission> lookup = new HashMap<Integer, StreetTraversalPermission>();

	static {
		for (StreetTraversalPermission s : EnumSet
				.allOf(StreetTraversalPermission.class))
			lookup.put(s.code, s);
	}
	...
	public StreetTraversalPermission modify(boolean permissive,
	...
	public boolean allows(TraverseMode mode) {
		if (mode == TraverseMode.WALK
				&& allows(StreetTraversalPermission.PEDESTRIAN)) {
			return true;
		} else if (mode == TraverseMode.BICYCLE
				&& allows(StreetTraversalPermission.BICYCLE)) {
			return true;
		} else if (mode == TraverseMode.CAR
				&& allows(StreetTraversalPermission.CAR)) {
			return true;
