...
/**
 * Represents a group of trips that all call at the same sequence of stops. For
 * each stop, there is a list of departure times, running times, arrival times,
 * dwell times, and wheelchair accessibility information (one of each of these
 * per trip per stop). Trips are assumed to be non-overtaking, so that an
 * earlier trip never arrives after a later trip.
 *
 * This is called a JOURNEY_PATTERN in the Transmodel vocabulary. However, GTFS
 * calls a Transmodel JOURNEY a "trip", thus TripPattern.
 */
public class TripPattern implements Serializable {

	private static final Logger LOG = LoggerFactory
			.getLogger(TripPattern.class);
	...
	/**
	 * The GTFS Route of all trips in this pattern. GTFS technically allows the
	 * same pattern to appear in more than one route, but we make the assumption
	 * that all trips with the same pattern belong to the same Route. TODO:
	 * consider the case where there is a replacement bus that makes the same
	 * stops as a train. We may need to split out multiple patterns for multiple
	 * routes.
	 */
	public final Route route;

	/**
	 * As for the route field, this depends on there being a single GFTFS route
	 * per journey pattern. That is not always true.
	 */
	public final TraverseMode mode;

	/**
	 * All trips in this pattern call at this sequence of stops. This includes
	 * information about GTFS pick-up and drop-off types.
	 */
	public final StopPattern stopPattern;

	/**
	 * This is the "original" timetable holding the scheduled stop times from
	 * GTFS, with no realtime updates applied. If realtime stoptime updates are
	 * applied, next/previous departure searches will be conducted using a
	 * different, updated timetable in a snapshot.
	 */
	...
	public final TransitStop[] stopVertices; // these are not unique to this
												// pattern, can be shared. are
												// they even used?
	...
	// however it's nice to have for order reference, since all timetables must
	// have tripTimes
	 ...
	 * contain TripTimes for the same trips, in the same order (that of the
	 * scheduled Timetable). An exception to this rule may arise if unscheduled
	 * trips are added to a Timetable. For that case we need to search for
	...
	/**
	 * An ordered list of PatternHop edges associated with this pattern. All
	 * trips in a pattern have the same stops and a PatternHop apply to all
	 * those trips, so this array apply to every trip in every timetable in this
	 * pattern. Please note that the array size is the number of stops minus 1.
	 * This also allow to access the ordered list of stops.
	 * 
	 * This appears to only be used for on-board departure. TODO: stops can now
	 * be grabbed from stopPattern.
	 */
	private PatternHop[] patternHops; // TODO rename/merge with hopEdges

	/**
	 * Holds stop-specific information such as wheelchair accessibility and
	 * pickup/dropoff roles.
	 */
	// TODO: is this necessary? Can we just look at the Stop and StopPattern
	// objects directly?
	@XmlElement
	int[] perStopFlags;

	/**
	 * A set of serviceIds with at least one trip in this pattern. Trips in a
	 * pattern are no longer necessarily running on the same service ID.
	 */
	...
	public TripPattern(Route route, StopPattern stopPattern) {
		...
		/*
		 * Create properly dimensioned arrays for all the vertices/edges
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		// The serialized graph contains cyclic references TripPattern <-->
		// Timetable.
		// The Timetable must be indexed from here (rather than in its own
		// readObject method)
		// to ensure that the stops field it uses in TripPattern is already
		// deserialized.
		scheduledTimetable.finish();
	}

	// TODO verify correctness after substitution of StopPattern for
	// ScheduledStopPattern
	// also, maybe get rid of the per stop flags and just use the values in
	// StopPattern, or an Enum
	private void setStopsFromStopPattern(StopPattern stopPattern) {
		...
		for (Stop stop : stopPattern.stops) {
			// Assume that stops can be boarded with wheelchairs by default
			// (defer to per-trip data)
	...
	/**
	 * Gets the number of scheduled trips on this pattern. Note that when stop
	 * time updates are being applied, there may be other Timetables for this
	 * pattern which contain a larger number of trips. However, all trips with
	 * indexes from 0 through getNumTrips()-1 will always correspond to the
	 * scheduled trips.
	 */
	...
	// TODO: These should probably be deprecated. That would require grabbing
	// the scheduled timetable,
	// and would avoid mistakes where real-time updates are accidentally not
	// taken into account.

	/**
	 * Add the given tripTimes to this pattern's scheduled timetable, recording
	 * the corresponding trip as one of the scheduled trips on this pattern.
	 */
	public void add(TripTimes tt) {
		// Only scheduled trips (added at graph build time, rather than directly
		// to the timetable via updates) are in this list.
		trips.add(tt.trip);
		scheduledTimetable.addTripTimes(tt);
		// Check that all trips added to this pattern are on the initially
		// declared route.
		...
		if (this.route != tt.trip.getRoute()) {
			LOG.warn(
					...
					tt.trip, tt.trip.getRoute(), this.route);
		}
	}

	/**
	 * Add the given FrequencyEntry to this pattern's scheduled timetable,
	 * recording the corresponding trip as one of the scheduled trips on this
	 * pattern. TODO possible improvements: combine freq entries and TripTimes.
	 * Do not keep trips list in TripPattern since it is redundant.
	 */
	...
	/**
	 * Rather than the scheduled timetable, get the one that has been updated
	 * with real-time updates. The view is consistent across a single request,
	 * and depends on the routing context in the request.
	 */
	public Timetable getUpdatedTimetable(RoutingRequest req, ServiceDay sd) {
		if (req != null && req.rctx != null
				&& req.rctx.timetableSnapshot != null && sd != null) {
			return req.rctx.timetableSnapshot
					.resolve(this, sd.getServiceDate());
		}
	...
	private static String stopNameAndId(Stop stop) {
		return stop.getName() + " ("
				+ GtfsLibrary.convertIdToString(stop.getId()) + ")";
	}

	/**
	 * Static method that creates unique human-readable names for a collection
	 * of TableTripPatterns. Perhaps this should be in TripPattern, and apply to
	 * Frequency patterns as well. TODO: resolve this question: can a frequency
	 * and table pattern have the same stoppattern? If so should they have the
	 * same "unique" name?
	 * 
	 * The names should be dataset unique, not just route-unique?
	 * 
	 * A TripPattern groups all trips visiting a particular pattern of stops on
	 * a particular route. GFTS Route names are intended for very general
	 * customer information, but sometimes there is a need to know where a
	 * particular trip actually goes. For example, the New York City N train has
	 * at least four different variants: express (over the Manhattan bridge) and
	 * local (via lower Manhattan and the tunnel), in two directions (to Astoria
	 * or to Coney Island). During construction, a fifth variant sometimes
	 * appears: trains use the D line to Coney Island after 59th St (or from
	 * Coney Island to 59th in the opposite direction).
	 * 
	 * TripPattern names are machine-generated on a best-effort basis. They are
	 * guaranteed to be unique (among TripPatterns for a single Route) but not
	 * stable across graph builds, especially when different versions of GTFS
	 * inputs are used. For instance, if a variant is the only variant of the N
	 * that ends at Coney Island, the name will be "N to Coney Island". But if
	 * multiple variants end at Coney Island (but have different stops
	 * elsewhere), that name would not be chosen. OTP also tries start and
	 * intermediate stations ("from Coney Island", or "via
	 * Whitehall", or even combinations ("from Coney Island via Whitehall"). But
	 * if there is no way to create a unique name from start/end/intermediate
	 * stops, then the best we can do is to create a "like [trip id]" name,
	 * which at least tells you where in the GTFS you can find a related trip.
	 */
	// TODO: pass in a transit index that contains a Multimap<Route,
	// TripPattern> and derive all TableTripPatterns
	// TODO: use headsigns before attempting to machine-generate names
	// TODO: combine from/to and via in a single name. this could be
	// accomplished by grouping the trips by destination,
	// then disambiguating in groups of size greater than 1.
	/*
	 * Another possible approach: for each route, determine the necessity of
	 * each field (which combination will create unique names). from, to, via,
	 * express. Then concatenate all necessary fields. Express should really be
	 * determined from number of stops and/or run time of trips.
	 */
	public static void generateUniqueNames(
		...
		Multimap<Route, TripPattern> patternsByRoute = ArrayListMultimap
				.create();
		...
		/*
		 * Iterate over all routes, giving the patterns within each route unique
		 * names.
		 */
		ROUTE: for (Route route : patternsByRoute.keySet()) {
			Collection<TripPattern> routeTripPatterns = patternsByRoute
					.get(route);
			...
			/*
			 * Do the patterns within this Route have a unique start, end, or
			 * via Stop?
			 */
			Multimap<String, TripPattern> signs = ArrayListMultimap.create(); // prefer
																				// headsigns
			...
			PATTERN: for (TripPattern pattern : routeTripPatterns) {
				...
				if (remainingPatterns.size() == 2) {
					...
					// The current one must be a subset of the other, because it
					// has no unique via.
		...
		if (LOG.isDebugEnabled()) {
			...
			for (Route route : patternsByRoute.keySet()) {
				Collection<TripPattern> routeTripPatterns = patternsByRoute
						.get(route);
				LOG.debug("Named {} patterns in route {}",
						routeTripPatterns.size(), uniqueRouteNames.get(route));
				for (TripPattern pattern : routeTripPatterns) {
					LOG.debug("    {} ({} stops)", pattern.name,
							pattern.stopPattern.size);
				}
			}
		}
	...
	/**
	 * Repetitive logic pulled out of makePatternVerticesAndEdges(). No longer
	 * works because we don't have access to the DAO here. But moving the
	 * makePatternVerticesAndEdges into TripPattern seems cleaner (certainly
	 * looks cleaner).
	 */
	private <T> T getStopOrParent(Map<Stop, T> map, Stop stop, Graph graph) {
		T vertex = map.get(stop);
		if (vertex == null) {
			Stop parent = null; // _dao.getStopForId(new
								// AgencyAndId(stop.getId().getAgencyId(),
								// stop.getParentStation()));
			vertex = map.get(parent);
			/*
			 * FIXME: this is adding an annotation for a specific problem, but
			 * all we know is that the stop vertex does not exist.
			 */
			if (vertex == null) {
				// LOG.warn(graph.addBuilderAnnotation(new StopAtEntrance(stop,
				// false)));
			} else {
				// LOG.warn(graph.addBuilderAnnotation(new StopAtEntrance(stop,
				// true)));
	...
	/**
	 * Create the PatternStop vertices and PatternBoard/Hop/Dwell/Alight edges
	 * corresponding to a StopPattern/TripPattern. StopTimes are passed in
	 * instead of Stops only because they are needed for shape distances
	 * (actually, stop sequence numbers?).
	 * 
	...
	public void makePatternVerticesAndEdges(Graph graph, GtfsStopContext context) {
		...
		/*
		 * Create arrive/depart vertices and hop/dwell/board/alight edges for
		 * each hop in this pattern.
		 */
		...
		for (int stop = 0; stop < nStops - 1; stop++) {
			...
			/*
			 * Get the arrive and depart vertices for the current stop (not
			 * pattern stop).
			 */
			TransitStopDepart stopDepart = getStopOrParent(
					context.stopDepartNodes, s0, graph);
			TransitStopArrive stopArrive = getStopOrParent(
					context.stopArriveNodes, s1, graph);

			/* Add this pattern's route's mode to the modes for this Stop. */
			// This is updating a TraverseModeSet (which is a bitmask).
			// Maybe we should just store that mask in the pattern when it is
			// created.
			...
			/*
			 * Create board/alight edges, but only if pickup/dropoff is enabled
			 * in GTFS.
			 */
			if (this.canBoard(stop)) {
				boardEdges[stop] = new TransitBoardAlight(stopDepart, pdv0,
						stop, mode);
			}
			if (this.canAlight(stop + 1)) {
				alightEdges[stop + 1] = new TransitBoardAlight(pav1,
						stopArrive, stop + 1, mode);
			}
		}
	}
	...
	/**
	 * A bit of a strange place to set service codes all at once when TripTimes
	 * are already added, but we need a reference to the Graph or at least the
	 * codes map. This could also be placed in the hop factory itself.
	 */
	...
	/**
	 * Patterns do not have unique IDs in GTFS, so we make some by concatenating
	 * agency id, route id, and an integer. We impose our assumption that all
	 * trips in the same pattern are on the same route. This only works if the
	 * Collection of TripPattern includes every TripPattern for the agency.
	 */
	public static void generateUniqueIds(Collection<TripPattern> tripPatterns) {
		...
		for (TripPattern pattern : tripPatterns) {
			...
			// OBA library uses underscore as separator, we're moving toward
			// colon.
			String id = String.format("%s:%s:%02d", routeId.getAgencyId(),
					routeId.getId(), count);
			pattern.code = (id);
		}
	}
	...
	/**
	 * In most cases we want to use identity equality for Trips. However, in
	 * some cases we want a way to consistently identify trips across versions
	 * of a GTFS feed, when the feed publisher cannot ensure stable trip IDs.
	 * Therefore we define some additional hash functions. Hash collisions are
	 * theoretically possible, so these identifiers should only be used to
	 * detect when two trips are the same with a high degree of probability. An
	 * example application is avoiding double-booking of a particular bus trip
	 * for school field trips. Using Murmur hash function. see
	 * http://programmers.stackexchange.com/a/145633 for comparison.
	 *
	 * @param trip
	 *            a trip object within this pattern, or null to hash the pattern
	 *            itself independent any specific trip.
	 * @return the semantic hash of a Trip in this pattern as a printable
	 *         String.
	 *
