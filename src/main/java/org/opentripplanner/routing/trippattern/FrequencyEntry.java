...
public class FrequencyEntry implements Serializable {
    ...
    public String toString() {
        return String.format("FreqEntry: trip %s start %s end %s headway %s", tripTimes.trip,
                formatSeconds(startTime), formatSeconds(endTime), formatSeconds(headway));
    }
    ...
    public int nextDepartureTime(int stop, int time) {
        if (time > endTime)
            return -1;
        System.out
                .println("  nextDepartureTime: does it run only in Freq-based? : "
                        + this.tripTimes.trip.getId() + ", time: " + time / 3600 + ":"
                        + (time % 3600) / 60);
    ...
    public int prevArrivalTime(int stop, int t) {
        if (t < startTime)
            return -1;
        System.out.println("Freq-Entry: " + this.tripTimes.trip.getId() + ", stop = " + stop
                + ", prevArrivalTime");
