...
/**
 * Does the same thing as String.intern, but for several different types. Java's
 * String.intern uses perm gen space and is broken anyway.
 */
public class Deduplicator implements Serializable {
	private static final long serialVersionUID = 20140524L;

	private final Map<IntArray, IntArray> canonicalIntArrays = Maps
			.newHashMap();
	...
	private final Map<StringArray, StringArray> canonicalStringArrays = Maps
			.newHashMap();
	...
	/**
	 * Used to deduplicate time and stop sequence arrays. The same times may
	 * occur in many trips.
	 */
	...
	public String deduplicateString(String original) {
		...
		if (canonical == null) {
			canonical = new String(original.toCharArray()); // Trim String if
															// necessary (older
															// JDKs)
	...
	public String[] deduplicateStringArray(String[] original) {
		if (original == null)
			return null;
		StringArray canonical = canonicalStringArrays.get(new StringArray(
				original, false));
		if (canonical == null) {
	...
	/**
	 * A wrapper for a primitive int array. This is insane but necessary in
	 * Java.
	 */
	...
	/**
	 * A wrapper for a String array. Optionally, the individual Strings may be
	 * deduplicated too.
	 */
