...
public class PatternDepartVertex extends PatternStopVertex {
	...
	public PatternDepartVertex(Graph g, TripPattern pattern, int stopIndex) {
		super(g, makeLabel(pattern, stopIndex), pattern,
				pattern.stopPattern.stops[stopIndex]);
	}

	// constructor for single-trip hops with no trip pattern (frequency
	// patterns) is now missing
	// it is possible to have both a freq and non-freq pattern with the same
	// stop pattern
