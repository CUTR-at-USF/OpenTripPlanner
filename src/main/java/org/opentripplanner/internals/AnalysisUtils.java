...
public class AnalysisUtils {
	 ...
	 * (in terms of number of nodes) will not overlap any other components (it
	 * will have holes); the others may overlap each other.
	 *
	 * @param dateTime
	 */
	public static List<Geometry> getComponentPolygons(Graph graph,
			RoutingRequest options, long time) {
		DisjointSet<Vertex> components = getConnectedComponents(graph);

		LinkedListMultimap<Integer, Coordinate> componentCoordinates = LinkedListMultimap
				.create();
					...
					if (geometry != null) {
						...
						for (int i = 0; i < coordinates.size(); ++i) {
							Coordinate coordinate = new Coordinate(
									coordinates.get(i));
							coordinate.x = Math.round(coordinate.x * PRECISION)
									/ PRECISION;
							coordinate.y = Math.round(coordinate.y * PRECISION)
									/ PRECISION;
		...
		for (Integer key : componentCoordinates.keySet()) {
			...
			// buffer components which are mere lines so that they do not
			// disappear.
