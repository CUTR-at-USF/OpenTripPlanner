...
public class GtfsBundle {
	...
	/**
	 * Create direct transfers between the constituent stops of each parent
	 * station. This is different from "linking stops to parent stations" below.
	 */
	public boolean parentStationTransfers = false;

	/**
	 * Connect parent station vertices to their constituent stops to allow
	 * beginning and ending paths (itineraries) at them.
	 */
	...
	public Boolean useCached = null; // null means use global default from
										// GtfsGB || true

	public File cacheDirectory = null; // null means use default from GtfsGB ||
										// system temp dir
	...
	public String getDataKey() {
		return path + ";" + url + ";"
				+ (csvInputSource != null ? csvInputSource.hashCode() : "");
	}
		...
		if (csvInputSource == null) {
			if (path != null) {
				if (path.isDirectory()) {
					...
					csvInputSource = new ZipFileCsvInputSource(
							new ZipFile(path));
				}
			} else if (url != null) {
	...
	/**
	 * So that you can load multiple gtfs feeds into the same database / system
	 * without entity id collisions, everything has an agency id, including
	 * entities like stops, shapes, and service ids that don't explicitly have
	 * an agency id (as opposed to routes + trips + stop times). However, the
	 * spec doesn't currently have a method to specify which agency a stop
	 * should be assigned to in the case of multiple agencies being specified in
	 * the same feed. Routes (and thus everything belonging to them) do have an
	 * agency id, but stops don't. The defaultAgencyId allows you to define
	 * which agency will be used as the default when figuring out which agency a
	 * stop should be assigned to (also applies to shapes + service ids as
	 * well). If not specified, the first agency in the agency list will be
	 * used.
	 */
	...
	/**
	 * When a trip doesn't contain any bicycle accessibility information, should
	 * taking a bike along a transit trip be permitted? A trip doesn't contain
	 * bicycle accessibility information if both route_short_name and
	...
	/**
	 * Transfers.txt usually specifies where the transit operator prefers people
	 * to transfer, due to schedule structure and other factors.
	 * 
	 * However, in systems like the NYC subway system, transfers.txt can
	 * partially substitute for the missing pathways.txt file. In this case,
	...
	public void setTransfersTxtDefinesStationPaths(
			boolean transfersTxtDefinesStationPaths) {
	...
	public void checkInputs() {
		...
		if (path != null) {
			if (!path.exists()) {
				throw new RuntimeException("GTFS Path " + path
						+ " does not exist.");
			}
			if (!path.canRead()) {
				throw new RuntimeException("GTFS Path " + path
