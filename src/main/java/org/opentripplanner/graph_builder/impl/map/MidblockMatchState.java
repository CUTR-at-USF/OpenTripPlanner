...
public class MidblockMatchState extends MatchState {
	...
	public MidblockMatchState(MatchState parent, Geometry routeGeometry,
			Edge edge, LinearLocation routeIndex, LinearLocation edgeIndex,
			double error, double distanceAlongRoute) {
	...
	public List<MatchState> getNextStates() {
		...
		if (routeIndex.getSegmentIndex() == routeGeometry.getNumPoints() - 1) {
			// this has either hit the end, or gone off the end. It's not real
			// clear which.
			// for now, let's assume it means that the ending is somewhere along
			// this edge,
		...
		if (it.hasNext()) {
			...
			// now we want to see where this new point is in terms of the edge's
			// geometry
			Coordinate newRouteCoord = routeSuccessor
			...
			if (newEdgeIndex.compareTo(edgeIndex) <= 0) {
				// we must make forward progress along the edge... or go to the
				// next edge
				/*
				 * this should not require the try/catch, but there is a bug in
				 * JTS
				 */
				try {
					LinearLocation projected2 = indexedEdge.indexOfAfter(
							edgeCoord, edgeIndex);
					// another bug in JTS
			...
			if (newEdgeIndex.getSegmentIndex() == edgeGeometry.getNumPoints() - 1) {
				...
				// we might choose to continue from the end of the edge and a
				// point mid-way
				...
				LocationIndexedLine indexedRoute = new LocationIndexedLine(
						routeGeometry);

				// FIXME: it would be better to do this project/indexOfAfter in
				// one step
				// as the two-step version could snap to a bad place and be
				// unable to escape.

				LinearLocation routeProjectedEndIndex = indexedRoute
						.project(endCoord);
				...
				if (routeProjectedEndIndex.compareTo(routeIndex) <= 0) {
					try {
						routeProjectedEndIndex = indexedRoute.indexOfAfter(
								routeProjectedEndCoord, routeIndex);
						if (Double.isNaN(routeProjectedEndIndex
								.getSegmentFraction())) {
							// can't go forward
							routeProjectedEndIndex = routeIndex; // this is bad,
																	// but not
																	// terrible
																	// since we
																	// are
					...
					routeProjectedEndCoord = routeProjectedEndIndex
							.getCoordinate(routeGeometry);
				}

				double positionError = distance(routeProjectedEndCoord,
						endCoord);
				double travelAlongRoute = distanceAlongGeometry(routeGeometry,
						routeIndex, routeProjectedEndIndex);
				double travelAlongEdge = distanceAlongGeometry(edgeGeometry,
						edgeIndex, newEdgeIndex);
				double travelError = Math.abs(travelAlongEdge
						- travelAlongRoute);
				...
				for (Edge e : getOutgoingMatchableEdges(toVertex)) {
					...
					MatchState nextState = new MidblockMatchState(this,
							routeGeometry, e, routeProjectedEndIndex,
				...
				double travelAlongEdge = distanceAlongGeometry(edgeGeometry,
						edgeIndex, newEdgeIndex);
				double travelAlongRoute = distanceAlongGeometry(routeGeometry,
						routeIndex, routeSuccessor);
				double travelError = Math.abs(travelAlongRoute
						- travelAlongEdge);
				...
				MatchState nextState = new MidblockMatchState(this,
						routeGeometry, edge, routeSuccessor, newEdgeIndex,
						error, travelAlongRoute);
				nextStates.add(nextState);

				// it's also possible that, although we have not yet reached the
				// end of this edge,
				// we are going to turn, because the route turns earlier than
				// the edge. In that
				// case, we jump to the corner, and our error is the distance
				// from the route point
				// and the corner

				Vertex toVertex = edge.getToVertex();
				double travelAlongOldEdge = distanceAlongGeometry(edgeGeometry,
						edgeIndex, null);

				for (Edge e : getOutgoingMatchableEdges(toVertex)) {
					Geometry newEdgeGeometry = e.getGeometry();
					LocationIndexedLine newIndexedEdge = new LocationIndexedLine(
							newEdgeGeometry);
					newEdgeIndex = newIndexedEdge.project(newRouteCoord);
					Coordinate newEdgeCoord = newEdgeIndex
							.getCoordinate(newEdgeGeometry);
					positionError = distance(newEdgeCoord, newRouteCoord);
					travelAlongEdge = travelAlongOldEdge
							+ distanceAlongGeometry(newEdgeGeometry,
									new LinearLocation(), newEdgeIndex);
					...
					nextState = new MidblockMatchState(this, routeGeometry, e,
							routeSuccessor, new LinearLocation(), cost,
							travelAlongRoute);
					nextStates.add(nextState);
				}
			...
			double locationError = distance(
					projected.getCoordinate(edgeGeometry), routeCoord);
	...
	public String toString() {
		return "MidblockMatchState(" + edge + ", "
				+ edgeIndex.getSegmentIndex() + ", "
				+ edgeIndex.getSegmentFraction() + ") - " + currentError;
	}

	public int hashCode() {
		return (edge.hashCode() * 1337 + hashCode(edgeIndex)) * 1337
				+ hashCode(routeIndex);
	}

	private int hashCode(LinearLocation location) {
		return location.getComponentIndex() * 1000000
				+ location.getSegmentIndex() * 37
