 ...
 * 
 * The JTS version of this class has several design decisions that don't work
 * for me. In particular, hasNext() in the original should be "isValid", and if
 * we start mid-segment, we should continue at the end of this segment rather
 * than the end of the next segment.
 */
public class LinearIterator implements Iterable<LinearLocation> {
	...
	/**
	 * Creates an iterator starting at a specified component and vertex in a
	 * linear {@link Geometry}
	 * 
	 * @param linearGeom
	 *            the linear geometry to iterate over
	 ...
	 *             if linearGeom is not lineal
	 */
	public LinearIterator(Geometry linearGeom, int componentIndex,
			int vertexIndex) {
	...
	public boolean hasNext() {
		...
		if (componentIndex == numLines - 1
				&& vertexIndex >= currentLine.getNumPoints() - 1)
			return false;
		return true;
	}
	...
	/**
	 * Moves the iterator ahead to the next vertex and (possibly) linear
	 * component.
	 */
	public void next() {
		if (!hasNext())
	...
	/**
	 * Checks whether the iterator cursor is pointing to the endpoint of a
	 * linestring.
	 * 
	...
	/**
	 * Gets the first {@link Coordinate} of the current segment. (the coordinate
	...
	/**
	 * Gets the second {@link Coordinate} of the current segment. (the
	 * coordinate of the next vertex). If the iterator is at the end of a line,
	...
	public static LinearLocation getEndLocation(Geometry linear) {
		...
		LineString lastLine = (LineString) linear
				.getGeometryN(lastComponentIndex);
