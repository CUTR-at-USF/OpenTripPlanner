...
public class CountRoadInDis extends AbstractStopTester {
	...
	/**
	 * @retrun return true if the number of streets around a stop is equal or
	 *         higher then numberOfStreets, street consider to be around stop if
	 *         the distance between then is less then distance. please note that
	 *         the distance calculation is big approximation, but it still good
	 *         enough for the alerts need.
	 */
	@Override
	public boolean fulfillDemands(TransitStop ts, Graph graph) {
		...
		for (Edge edge : edges) {
			if (edge instanceof StreetEdge) {
				...
				if (disMeters <= distance
						&& streetEdge.getPermission().allows(allowedPermission)) {
					counter++;
				}
			}
		}
