...
/**
 * Check the geometry of every edge in the graph for any bogus geometry -- that
 * is, geometry with coordinates of NaN. This is mainly good for debugging, but
 * probably worth keeping on for production because the cost is small compared
 * to the pain of debugging.
 */
public class CheckGeometryGraphBuilderImpl implements GraphBuilder {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(CheckGeometryGraphBuilderImpl.class);
	private static final double MAX_VERTEX_SHAPE_ERROR = 150;

	@Override
	public void buildGraph(Graph graph, HashMap<Class<?>, Object> extra) {
		for (Vertex gv : graph.getVertices()) {
			if (Double.isNaN(gv.getCoordinate().x)
					|| Double.isNaN(gv.getCoordinate().y)) {
				LOG.warn("Vertex " + gv
						+ " has NaN location; this will cause doom.");
				LOG.warn(graph
						.addBuilderAnnotation(new BogusVertexGeometry(gv)));
			}

			// TODO: This was filtered to EdgeNarratives before EdgeNarrative
			// removal
			for (Edge e : gv.getOutgoing()) {
				...
				for (Coordinate c : g.getCoordinates()) {
					if (Double.isNaN(c.x) || Double.isNaN(c.y)) {
						LOG.warn(graph
								.addBuilderAnnotation(new BogusEdgeGeometry(e)));
					}
				}
				if (e instanceof HopEdge) {
					Coordinate edgeStartCoord = e.getFromVertex()
							.getCoordinate();
					...
					if (getDistanceLibrary().distance(edgeStartCoord,
							geometryStartCoord) > MAX_VERTEX_SHAPE_ERROR) {
						LOG.warn(graph
								.addBuilderAnnotation(new VertexShapeError(e)));
					} else if (getDistanceLibrary().distance(edgeEndCoord,
							geometryEndCoord) > MAX_VERTEX_SHAPE_ERROR) {
						LOG.warn(graph
								.addBuilderAnnotation(new VertexShapeError(e)));
					}
				}
