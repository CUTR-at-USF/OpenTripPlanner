...
public class CompositeStreetTraversalPermissionConverter implements
		...
		for (SimpleFeatureConverter<P2<StreetTraversalPermission>> converter : converters) {
			...
			if (result == null) {
				...
				if (orPermissions) {
					...
					first = StreetTraversalPermission.get(result.first.code
							& value.first.code);
					second = StreetTraversalPermission.get(result.second.code
							& value.second.code);
				}
	 ...
	 */
	public void add(
			SimpleFeatureConverter<P2<StreetTraversalPermission>> converter) {
