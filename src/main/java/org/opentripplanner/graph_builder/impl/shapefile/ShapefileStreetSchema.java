...
public class ShapefileStreetSchema {
	...
	/**
	 * The ID attribute is used to uniquely identify street segments. This is
	 * useful if a given street segment appears multiple times in a shapefile.
	 */
	...
	public void setNameAttribute(String attributeName) {
		this.nameConverter = new AttributeFeatureConverter<String>(
				attributeName);
	}
	...
	/**
	 * The permission converter gets the {@link StreetTraversalPermission} for a
	 * street segment and its reverse.
	 * 
	 * @return
	 */
	...
	/**
	 * The permission converter gets the bicycle safety factor for a street
	 * segment and its reverse. The safety factor is 1.0 for an ordinary street.
	 * For streets which are more or less safe for bicycles, the safety factor
	 * is the number of miles you would have to bike on an ordinary street to
	 * have the same odds of dying as if you biked one mile on this street. For
	 * example, if bike lanes reduce risk by a factor of 3, the safety factor
	 * would be 0.33...
	 * 
	 * @return
	 */
	public void setBicycleSafetyConverter(
			SimpleFeatureConverter<P2<Double>> safetyConverter) {
	...
	/**
	 * The slope override converter returns true if the slope found from NED is
	 * should be ignored (for instance, on bridges and tunnels)
	 * 
	 * @param slopeOverrideConverter
	 */
	public void setSlopeOverrideConverter(
			SimpleFeatureConverter<Boolean> slopeOverrideConverter) {
		this.slopeOverrideConverter = slopeOverrideConverter;
	}

	/**
	 * @param featureSelector
	 *            A featureSelector returns true if a feature is a street, and
	 *            false otherwise. Useful for centerline files that also have
	 *            non-streets, such as political boundaries or coastlines
	 */
	public void setFeatureSelector(
			SimpleFeatureConverter<Boolean> featureSelector) {
