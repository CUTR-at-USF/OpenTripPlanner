...
/**
 * Handles marking certain types of streets/bike lanes as more or less safe for
 * bike trips.
 */
public class CaseBasedBicycleSafetyFeatureConverter implements
	...
	public P2<Double> convert(SimpleFeature feature) {
		...
		int directionFeature = 3; // Default to applying the safety feature in
									// both directions
									// (useful if the dataset doesn't include
									// direction information)
		if (directionAttributeName != null) {
			String directionKey = feature.getAttribute(directionAttributeName)
					.toString();
		...
		return new P2<Double>((directionFeature & 0x1) == 0 ? 1.0
				: safetyFeature, (directionFeature & 0x2) == 0 ? 1.0
				: safetyFeature);
	}
	 ...
	 *            The name of the attribute used when calculating the feature's
	 *            safety.
	 */
	 ...
	 *            The name of the attribute used when calculating the direction
	 *            of the street/bikelane for which that the safety feature
	 *            should apply.
	 */
	...
	/**
	 * Maps the direction value to a number representing the direction that the
	 * bike safety feature goes. The number is 1 for a safety feature that goes
	 * with the road geometry, 2 for a safety feature that goes against it, and
	 * 3 for a safety feature that goes both ways.
	 * 
