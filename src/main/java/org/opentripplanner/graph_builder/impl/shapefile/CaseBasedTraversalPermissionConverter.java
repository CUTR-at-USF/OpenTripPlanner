 ...
 * 
 * For example, given a shapefile that includes a DIRECTION column with data as
 * follows:
 * 
 * <pre>
 * | DIRECTION | NAME    | 
 ...
 * </pre>
 * 
 * You could use a CaseBasedTraversalPermissionConverter to implement the
 * following rules:
 *
 * <p>
 * By default, all streets should be traversable by pedestrians and bicycles in
 * both directions.
 * </p>
 * 
 * <p>
 * If a street's DIRECTION attribute is ONE_WAY_F, it should be traversable by
 * cars and bikes in only the forward direction and traversable by pedestrians
 * in both directions.
 * </p>
 * 
 * <p>
 * If a street's DIRECTION attribute is ONE_WAY_B, it should be traversable by
 * cars and bikes in only the backward direction and traversable by pedestrians
 * in both directions.
 * </p>
 * 
 * <p>
 * If a street's DIRECTION attribute is TWO_WAY, it should be traversable by
 * everyone in both directions.
 * </p>
 * 
 * 
 * These rules could be implemented by configuring the converter bean as
 * follows:
 * 
 * <pre>
 * {@code
...
public class CaseBasedTraversalPermissionConverter implements
	...
	public CaseBasedTraversalPermissionConverter(String attributeName,
		...
		_attributeName = attributeName;
		_defaultPermission = P2
				.createPair(defaultPermission, defaultPermission);
	}

	/**
	 * The name of the feature attribute to use when calculating the traversal
	 * permissions.
	 */
	...
	/**
	 * The default permission to use when no matching case is found for a
	 * street.
	 */
	...
	/**
	 * The mapping from attribute values to permissions to use when determining
	 * a street's traversal permission.
	 */
		...
		for (Map.Entry<String, String> entry : permissions.entrySet()) {
			...
			if (tokens.length != 2)
				throw new IllegalArgumentException(
						"invalid street traversal permissions: " + perms);

			StreetTraversalPermission forward = StreetTraversalPermission
					.valueOf(tokens[0]);
			StreetTraversalPermission reverse = StreetTraversalPermission
					.valueOf(tokens[1]);
			addPermission(attributeValue, forward, reverse);
		}
	}

	public void addPermission(String attributeValue,
			StreetTraversalPermission forward, StreetTraversalPermission reverse) {
