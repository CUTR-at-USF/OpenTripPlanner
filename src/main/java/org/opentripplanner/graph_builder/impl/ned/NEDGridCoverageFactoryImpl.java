...
public class NEDGridCoverageFactoryImpl implements ElevationGridCoverageFactory {
     ...
     * without topography. Differences between the two are significant (up to 100 meters).
     * 
     * Current geoid models relate NAD83 ellipsoid heights to NAVD88 orthometric heights, i.e. the
     * geoid for the continental United States is calibrated against and defined relative to the GPS
     * ellipsoid.
     * 
     * According to http://www.profsurv.com/magazine/article.aspx?i=561, "it is generally assumed
     * that WGS 84 (original) is identical to NAD 83 (1986)." According to
     ...
     * 
     * In OTP we convert between these two systems using one of these geoids defined relative to an
     * ellipsoid. The rasters describing the datum are not included in OTP by default because they
     * double the size of the OTP distribution, but are only needed by people loading elevations in
     * North America.
     * 
     * In OTP we perform the conversion using a geoid defined relative to the NAD83 ellipsoid. This
     * is backed up by an NOAA publication at
     ...
     * 
     * The datum rasters must be downloaded from the OTP website and placed in the NED cache
     * directory.
     */
    private void loadVerticalDatum() {
        if (datums == null) {
            datums = new ArrayList<VerticalDatum>();
            String[] datumFilenames = { "g2012a00.gtx", "g2012g00.gtx", "g2012h00.gtx",
                    "g2012p00.gtx", "g2012s00.gtx", "g2012u00.gtx" };
            try {
