...
public class VerticalDatum {
	...
	public VerticalDatum(double lowerLeftLongitude, double lowerLeftLatitude,
			double width, double height, float[][] datum) {
	...
	double interpolatedHeight(double longitude, double latitude) {
		// because VDatums can cross -180 longitude, there's some complication
		// in interpolating
		// longitude
		...
		if (lowerLeftLongitude + deltaLongitude > 180) {
			...
			if (longitude < lowerLeftLongitude) {
				...
				lowerLeftAdjusted -= 360; // this transforms lowerLeft so that
											// interpolation will
											// work
				if (longitude < lowerLeftLongitude) {
		...
		if (longitude > lowerLeftAdjusted + deltaLongitude
				|| longitude < lowerLeftAdjusted) {
			throw new RuntimeException("longitude out of range");
		}
		if (latitude < lowerLeftLatitude
				|| latitude > lowerLeftLatitude + deltaLatitude) {
			throw new RuntimeException("latitude out of range");
		}

		double longitudeNormalized = (longitude - lowerLeftAdjusted)
				/ deltaLongitude;
		double latitudeNormalized = (latitude - lowerLeftLatitude)
				/ deltaLatitude;
		...
		return datum[y1][x1] * gridXFraction * gridYFraction
				...
				+ datum[y1 + 1][x1 + 1] * (1 - gridXFraction)
				* (1 - gridYFraction);
	}
	...
	public static VerticalDatum fromGTX(InputStream inputStream)
			throws IOException {
		...
		return new VerticalDatum(lowerLeftLongitude, lowerLeftLatitude,
				deltaLatitude * nColumns, deltaLongitude * nRows, data);
	}
