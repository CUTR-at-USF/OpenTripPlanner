...
/**
 * {@link GraphBuilder} plugin that applies elevation data to street data that
 * has already been loaded into a (@link Graph}, creating elevation profiles for
 * each Street encountered in the Graph. Depending on the
 * {@link ElevationGridCoverageFactory} specified this could be auto-downloaded
 * and cached National Elevation Dataset (NED) raster data or a GeoTIFF file.
 * The elevation profiles are stored as {@link PackedCoordinateSequence}
 * objects, where each (x,y) pair represents one sample, with the x-coord
 * representing the distance along the edge measured from the start, and the
 * y-coord representing the sampled elevation at that point (both in meters).
 */
public class ElevationGraphBuilderImpl implements GraphBuilder {
	private static final Logger log = LoggerFactory
			.getLogger(ElevationGraphBuilderImpl.class);
	...
	/**
	 * The distance between samples in meters. Defaults to 10m, the approximate
	 * resolution of 1/3 arc-second NED data.
	 */
	...
	public void buildGraph(Graph graph, HashMap<Class<?>, Object> extra) {
		...
		// If gridCov is a GridCoverage2D, apply a bilinear interpolator.
		// Otherwise, just use the
		// coverage as is (note: UnifiedGridCoverages created by
		// NEDGridCoverageFactoryImpl handle
		// interpolation internally)
		coverage = (gridCov instanceof GridCoverage2D) ? Interpolator2D.create(
				(GridCoverage2D) gridCov, new InterpolationBilinear())
				: gridCov;
	...
	class ElevationRepairState {
		...
		public ElevationRepairState(StreetEdge backEdge,
				ElevationRepairState backState, Vertex vertex, double distance,
	 ...
	 */
	private void assignMissingElevations(Graph graph,
		...
		// knownElevations will be null if there are no ElevationPoints in the
		// data
		...
		for (StreetEdge e : edgesWithElevation) {
			...
			if (!elevations.containsKey(e.getFromVertex())) {
				...
				ElevationRepairState state = new ElevationRepairState(null,
						null, e.getFromVertex(), 0, firstElevation);
				pq.insert(state, 0);
				elevations.put(e.getFromVertex(), firstElevation);
			}

			if (!elevations.containsKey(e.getToVertex())) {
				double lastElevation = profile.getOrdinate(profile.size() - 1,
						1);
				ElevationRepairState state = new ElevationRepairState(null,
						null, e.getToVertex(), 0, lastElevation);
				pq.insert(state, 0);
				elevations.put(e.getToVertex(), lastElevation);
			}
		}

		// Grow an SPT outward from vertices with known elevations into regions
		// where the
		// elevation is not known. when a branch hits a region with known
		// elevation, follow the
		// back pointers through the region of unknown elevation, setting
		// elevations via interpolation.
		while (!pq.empty()) {
			...
			for (Edge e : state.vertex.getOutgoing()) {
				...
				if (elevation != null) {
					...
					ElevationRepairState newState = new ElevationRepairState(
							edge, state, tov, e.getDistance() + state.distance,
				...
				if (e instanceof StreetWithElevationEdge) {
					...
					if (fromElevation == null || toElevation == null) {
						if (!edge.isElevationFlattened()
								&& !edge.isSlopeOverride())
							log.warn("Unexpectedly missing elevation for edge "
									+ edge);
						continue;
					}

					if (edge.getElevationProfile() != null
							&& edge.getElevationProfile().size() > 2) {
						continue;
					}
					...
					PackedCoordinateSequence profile = new PackedCoordinateSequence.Double(
							coords);

					if (edge.setElevationProfile(profile, true)) {
						log.trace(graph
								.addBuilderAnnotation(new ElevationFlattened(
										edge)));
					}
				}
			}
	...
	/**
	 * Processes a single street edge, creating and assigning the elevation
	 * profile.
	 * 
	 * @param ee
	 *            the street edge
	 * @param graph
	 *            the graph (used only for error handling)
	 */
	private void processEdge(Graph graph, StreetWithElevationEdge ee) {
		...
		for (int i = 0; i < coords.length - 1; i++) {
			edgeLenM += distanceLibrary.distance(coords[i].y, coords[i].x,
		...
		for (double x = distanceBetweenSamplesM; x < edgeLenM; x += distanceBetweenSamplesM) {
			// avoid final-segment samples less than half the distance between
			// samples:
			if (edgeLenM - x < distanceBetweenSamplesM / 2) {
				break;
			}

			Coordinate internal = getPointAlongEdge(coords, edgeLenM, x
					/ edgeLenM);
			coordList.add(new Coordinate(x, getElevation(internal)));
		}

		// final sample (x = edge length)
		coordList.add(new Coordinate(edgeLenM,
				getElevation(coords[coords.length - 1])));
	...
	/**
	 * Returns a coordinate along a path located at a specific point indicated
	 * by the percentage of distance covered from start to end.
	 * 
	 * @param coords
	 *            the list of (x,y) coordinates that form the path
	 * @param length
	 *            the total length of the path
	 * @param t
	 *            the percentage (ranges from 0 to 1)
	 * @return the (x,y) coordinate at t
	 */
	public Coordinate getPointAlongEdge(Coordinate[] coords, double length,
			double t) {

		double pctThrough = 0; // current percentage of the edge length
								// traversed

		// endpoints of current segment within edge:
		double x1 = coords[0].x, y1 = coords[0].y, x2, y2;

		for (int i = 1; i < coords.length - 1; i++) { // loop through inner
														// points
			...
			if (pctThrough + pct > t) { // if current segment contains 't,'
										// we're done
	 ...
	 * 
	 * @param x
	 *            the query longitude (NAD83)
	 * @param y
	 *            the query latitude (NAD83)
	 * @return elevation in meters
	 */
