 ...
 * among themselves, without using the street network at all. For now this just
 * considers distance, but it should also consider parent station
 * specifications, which are present in the Dutch KV7 data.
 */
public class StreetlessStopLinker implements GraphBuilder {
	...
	private static Logger LOG = LoggerFactory
			.getLogger(StreetlessStopLinker.class);
	...
	public void buildGraph(Graph graph, HashMap<Class<?>, Object> extra) {
		...
		for (TransitStop ts : Iterables.filter(graph.getVertices(),
			...
			for (TransitStop other : index.getNearbyTransitStops(c, radius)) {
				...
				Coordinate coordinates[] = new Coordinate[] { c,
						other.getCoordinate() };
				double distance = distanceLibrary.distance(coordinates[0],
						coordinates[1]);
