...
/**
 * Print statistics on geometry and edge/vertices data for a graph (number of
 * geometry, average number of points, size distribution, edge names size,
 * etc...)
 */
public class GraphStatisticsGraphBuilderImpl implements GraphBuilder {
	...
	public void buildGraph(Graph graph, HashMap<Class<?>, Object> extra) {
		...
		for (Edge e : graph.getEdges()) {
			edgeTypeDistribution.add(new ConstantQuantifiable<String>(e
					.getClass().toString()));
			edgeNameDistribution.add(
					new NumberQuantifiable<Integer>(e.getName() == null ? 0 : e
							.getName().length()), e.getName());
			if (e.getGeometry() != null) {
				LineString geometry = e.getGeometry();
				geomSizeDistribution.add(new NumberQuantifiable<Integer>(
						geometry.getNumPoints()));
				double lenMeters = distanceLibrary.fastLength(geometry);
				geomLenDistribution.add(new LogQuantifiable<Double>(lenMeters,
						5.0));
			}
		}
		for (Vertex v : graph.getVertices()) {
			vertexTypeDistribution.add(new ConstantQuantifiable<String>(v
					.getClass().toString()));
			vertexNameDistribution.add(
					new NumberQuantifiable<Integer>(v.getName() == null ? 0 : v
							.getName().length()), v.getName());
			vertexLabelDistribution.add(new NumberQuantifiable<Integer>(v
					.getLabel().length()), v.getLabel());
		}
