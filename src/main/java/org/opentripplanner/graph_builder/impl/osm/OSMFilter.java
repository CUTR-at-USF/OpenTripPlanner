...
public class OSMFilter {
	...
	/**
	 * Determine whether any mode can or should ever traverse the given way. If
	 * not, we leave the way out of the OTP graph. Potentially routable ways are
	 * those that have the tags : highway=* public_transport=platform
	 * railway=platform
	 * 
	 * But not conveyers, proposed highways/roads or those still under
	 * construction, and raceways (as well as ways where all access is
	 * specifically forbidden to the public).
	...
	public static boolean isWayRoutable(OSMWithTags way) {
		...
		if (highway != null
				&& (highway.equals("conveyer") || highway.equals("proposed")
						...
						|| highway.equals("raceway") || highway
							.equals("unbuilt")))
			return false;

		if (way.isGeneralAccessDenied()) {
			// There are exceptions.
			return (way.isMotorcarExplicitlyAllowed()
					|| way.isBicycleExplicitlyAllowed() || way
	...
	/**
	 * Determines whether this OSM way is considered routable. The majority of
	 * routable ways are those with a highway= tag (which includes everything
	 * from motorways to hiking trails). Anything with a
	 * public_transport=platform or railway=platform tag is also considered
	 * routable even if it doesn't have a highway tag. Platforms are however
	 * filtered out if they are marked usage=tourism. This prevents miniature
	 * tourist railways like the one in Portland's Zoo from receiving a better
	 * score and pulling search endpoints away from real transit stops.
	 */
	...
	public static StreetTraversalPermission getPermissionsForEntity(
		...
		/*
		 * Only a few tags are examined here, because we only care about modes
		 * supported by OTP (wheelchairs are not of concern here)
		 * 
		 * Only a few values are checked for, all other values are presumed to
		 * be permissive (=> This may not be perfect, but is closer to reality,
		 * since most people don't follow the rules perfectly ;-)
		 */
		if (entity.isGeneralAccessDenied()) {
			...
			if (entity.isMotorcarExplicitlyAllowed()) {
				permission = permission
						.add(StreetTraversalPermission.ALL_DRIVING);
			}
			...
			if (entity.isPedestrianExplicitlyAllowed()) {
				permission = permission
						.add(StreetTraversalPermission.PEDESTRIAN);
			}
		} else {
			permission = def;
		}
		...
		if (entity.isMotorcarExplicitlyDenied()) {
			permission = permission
					.remove(StreetTraversalPermission.ALL_DRIVING);
		...
		if (entity.isPedestrianExplicitlyDenied()) {
			permission = permission
					.remove(StreetTraversalPermission.PEDESTRIAN);
	...
	public static StreetTraversalPermission getPermissionsForWay(OSMWay way,
		...
		/*
		 * pedestrian rules: everything is two-way (assuming pedestrians are
		 * allowed at all) bicycle rules: default: permissions;
		 * 
		 * cycleway=dismount means walk your bike -- the engine will
		 * automatically try walking bikes any time it is forbidden to ride
		 * them, so the only thing to do here is to remove bike permissions
		 * 
		 * oneway=... sets permissions for cars and bikes oneway:bicycle
		 * overwrites these permissions for bikes only
		 * 
		 * now, cycleway=opposite_lane, opposite, opposite_track can allow once
		 * oneway has been set by oneway:bicycle, but should give a warning if
		 * it conflicts with oneway:bicycle
		 * 
		...
		if (way.isBicycleDismountForced()) {
			...
			if (forceBikes) {
				LOG.warn(graph.addBuilderAnnotation(new ConflictingBikeTags(way
						.getId())));
			}
		}
	...
	/**
	 * Check OSM tags for various one-way and one-way-by-mode tags and return a
	 * pair of permissions for travel along and against the way.
	 */
	public static P2<StreetTraversalPermission> getPermissions(
		...
		if (way.isOneWayForwardBicycle()) {
			permissionsBack = permissionsBack
					.remove(StreetTraversalPermission.BICYCLE);
		}
		...
		if (OSMWithTags.isFalse(oneWayBicycle)
				|| way.isTagTrue("bicycle:backwards")) {
			if (permissions.allows(StreetTraversalPermission.BICYCLE)) {
				permissionsFront = permissionsFront
						.add(StreetTraversalPermission.BICYCLE);
				permissionsBack = permissionsBack
						.add(StreetTraversalPermission.BICYCLE);
			}
		}
		...
		return new P2<StreetTraversalPermission>(permissionsFront,
				permissionsBack);
	}
	...
	public static int getPlatformClass(OSMWithTags way) {
		...
		if ("platform".equals(highway)
			...
			if (way.isTagTrue("train") || way.isTagTrue("subway")
					|| way.isTagTrue("tram") || way.isTagTrue("monorail")) {
				return StreetEdge.CLASS_TRAIN_PLATFORM;
			}
