...
/**
 * Stores information about an OSM area needed for visibility graph
 * construction. Algorithm based on
 * http://wiki.openstreetmap.org/wiki/Relation:multipolygon/Algorithm but
 * generally done in a quick/dirty way.
 */
class Area {
	...
	Area(OSMWithTags parent, List<OSMWay> outerRingWays,
			List<OSMWay> innerRingWays, Map<Long, OSMNode> _nodes) {
		this.parent = parent;
		// ring assignment
		...
		ArrayList<List<Long>> allRings = new ArrayList<List<Long>>(
				innerRingNodes);
		...
		OUTER: for (Ring outer : outerRings) {
			for (Ring possibleContainer : outerRings) {
				if (outer != possibleContainer
						&& outer.geometry
								.hasPointInside(possibleContainer.geometry)) {
					continue OUTER;
				}
			}
	...
	public MultiPolygon toJTSMultiPolygon() {
		if (jtsMultiPolygon == null) {
			...
			jtsMultiPolygon = GeometryUtils.getGeometryFactory()
	...
	public List<List<Long>> constructRings(List<OSMWay> ways) {
		...
		ArrayListMultimap<Long, OSMWay> waysByEndpoint = ArrayListMultimap
				.create();
		for (OSMWay way : ways) {
		...
		if (constructRingsRecursive(waysByEndpoint, partialRing, closedRings,
				firstEndpoint)) {
			return closedRings;
		} else {
			return null;
	...
	private boolean constructRingsRecursive(
			ArrayListMultimap<Long, OSMWay> waysByEndpoint, List<Long> ring,
		...
		for (OSMWay way : ways) {
			...
			ArrayList<Long> newRing = new ArrayList<Long>(ring.size()
					+ nodeRefs.size());
			...
			if (newRing.get(newRing.size() - 1).equals(newRing.get(0))) {
				...
				if (constructRingsRecursive(waysByEndpoint, newRing,
						closedRings, firstEndpoint)) {
					return true;
				}
