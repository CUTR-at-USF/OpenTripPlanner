...
public class Ring {
	...
	/**
	 * Why is there a boolean parameter called javaSucks? Because otherwise the
	 * two constructors have the same erasure, meaning that even though Java has
	 * enough information at compile-time to figure out which constructor I am
	 * talking about, it intentionally throws this away in the interest of
	 * having worse run-time performance. Thanks, Java!
	 * <p/>
	 * Oh, and most people would solve this problem by making a static factory
	 * method but that won't work because then all of this class's outer classes
	...
	public Polygon toJtsPolygon() {
		...
		// we need to merge connected holes here, because JTS does not believe
		// in
		// holes that touch at multiple points (and, weirdly, does not have a
		// method
		...
		for (Ring ring : holes) {
			LinearRing linearRing = factory
					.createLinearRing(toCoordinates(ring.geometry));
		...
		ArrayList<LinearRing> lrholelist = new ArrayList<LinearRing>(
				polygonHoles.size());
		...
		for (Polygon hole : polygonHoles) {
			...
			if (boundary instanceof LinearRing) {
				...
				LinearRing ring = factory.createLinearRing(line
						.getCoordinates());
				lrholelist.add(ring);
			}
		}
		LinearRing[] lrholes = lrholelist.toArray(new LinearRing[lrholelist
				.size()]);
