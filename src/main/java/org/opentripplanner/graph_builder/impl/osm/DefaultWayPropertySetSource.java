...
/**
 * This factory class provides a default collection of {@link WayProperties}
 * that determine how OSM streets can be traversed in various modes.
 *
 * Circa January 2011, Grant and Mele at TriMet undertook proper testing of bike
 * (and transit) routing, and worked with David Turner on assigning proper
 * weights to different facility types. The weights in this file grew
 * organically from trial and error, and are the result of months of testing and
 * tweaking the routes that OTP returned, as well as actually walking/biking
 * these routes and making changes based on those experiences. This set of
 * weights should be a great starting point for others to use, but they are to
 * some extent tailored to the situation in Portland and people shouldn't
 * hesitate to adjust them to for their own instance.
 *
 * The rules for assigning WayProperties to OSM ways are explained in. The final
 * tie breaker if two Pickers both match is the sequence that the properties are
 * added in this file: if all else is equal the 'setProperties' statement that
 ...
 * OpenStreetMapGraphBuilderImpl.Handler#getPermissionsForEntity(). For
 * instance, if a way that normally does not permit walking based on its tag
 * matches (the prevailing 'setProperties' statement) has a 'foot=yes' tag the
 * permissions are overridden and walking is allowed on that way.
 *
...
public class DefaultWayPropertySetSource implements WayPropertySetSource {
	...
	private static Logger LOG = LoggerFactory
			.getLogger(DefaultWayPropertySetSource.class);
	...
	public WayPropertySet getWayPropertySet() {
		...
		/*
		 * path designed for bicycles (should be treated exactly as a cycleway
		 * is), this is a multi-use path (MUP)
		 */
		...
		/*
		 * bicycles on tracks (tracks are defined in OSM as: Roads for
		 * agricultural use, gravel roads in the forest etc.; usually
		...
		setProperties(props, "highway=track;bicycle=designated;surface=*",
				StreetTraversalPermission.PEDESTRIAN_AND_BICYCLE, 0.99, 0.99);
		/*
		 * this is to avoid double counting since tracks are almost of surface
		 * type that is penalized
		 */
		...
		/*
		 * Automobile speeds in the United States: Based on my (mattwigway)
		 * personal experience, primarily in California
		 */
		...
		// "For roads used mainly/exclusively for pedestrians . . . which may
		// allow access by
		...
		/*
		 * cycleway:left/right=lane/track/shared_lane permutations - no longer
		 * needed because left/right matching algorithm does this
		 */
		...
		/*
		 * The following tags have been removed from surface weights because
		 * they are no more of an impedence to bicycling than a paved surface
		 * surface=paving_stones surface=fine_gravel (sounds counter-intuitive
		 * but see the definition on the OSM Wiki) surface=tartan (this what
		...
		setProperties(props, "surface=mud", StreetTraversalPermission.ALL, 1.5,
				1.5, true);
		setProperties(props, "surface=woodchip", StreetTraversalPermission.ALL,
				1.5, 1.5, true);
		setProperties(props, "surface=gravel", StreetTraversalPermission.ALL,
				1.5, 1.5, true);
		...
		/*
		 * the RLIS/CCGIS:bicycle=designated mixins are coded out as they are no
		 * longer neccessary because of of the bicycle=designated block of code
		 * above. This switch makes our weighting system less reliant on tags
		 * that aren't generally used by the OSM community, and prevents the
		 * double counting that was occuring on streets with both bicycle
		 * infrastructure and an RLIS:bicycle=designated tag
		 */
		...
		/* and the notes */
		// TODO: The two entries below produce lots of spurrious notes (OSM
		// mapper comments)
		...
		createNotes(props, "surface=mud", "note.muddy_surface",
				StreetNotesService.ALWAYS_MATCHER);
		createNotes(props, "toll=yes", "note.toll",
				StreetNotesService.DRIVING_MATCHER);
		createNotes(props, "toll:motorcar=yes", "note.toll",
				StreetNotesService.DRIVING_MATCHER);
		createNotes(props, "wheelchair:description=*",
		...
		createNames(props, "highway=pedestrian;area=yes",
		...
		createNames(props, "railway=platform;highway=footway;footway=sidewalk",
				"name.platform");
		createNames(props, "railway=platform;highway=path;path=sidewalk",
				"name.platform");
		createNames(props, "railway=platform;highway=pedestrian",
				"name.platform");
		...
		createNames(props, "railway=platform;highway=footway;bicycle=no",
				"name.platform");
		...
		createNames(props, "amenity=bicycle_rental;name=*",
				"name.bicycle_rental_name");
		createNames(props, "amenity=bicycle_rental",
				"name.bicycle_rental_station");

		createNames(props, "amenity=bicycle_parking;name=*",
				"name.bicycle_parking_name");
	...
	private void createNames(WayPropertySet propset, String spec,
			String patternKey) {
	...
	private void createNotes(WayPropertySet propset, String spec,
			String patternKey, NoteMatcher matcher) {
		String pattern = localize(patternKey);
