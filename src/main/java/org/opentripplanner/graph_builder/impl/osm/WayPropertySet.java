...
/**
 * Information given to the GraphBuilder about how to assign permissions, safety
 * values, names, etc. to edges based on OSM tags. TODO rename so that the
 * connection with OSM tags is obvious
 *
 * WayPropertyPickers, CreativeNamePickers, SlopeOverridePickers, and
 * SpeedPickers are applied to ways based on how well their OSMSpecifiers match
 * a given OSM way. Generally one OSMSpecifier will win out over all the others
 * based on the number of exact, partial, and wildcard tag matches. See
...
public class WayPropertySet {
	...
	/**
	 * The automobile speed for street segments that do not match any
	 * SpeedPicker.
	 */
	...
	/**
	 * The WayProperties applied to all ways that do not match any
	 * WayPropertyPicker.
	 */
	...
	public WayPropertySet() {
		...
		defaultSpeed = 11.2f; // 11.2 m/s ~= 25 mph ~= 40 kph, standard speed
								// limit in the US
	...
	/**
	 * Applies the WayProperties whose OSMPicker best matches this way. In
	 * addition, WayProperties that are mixins will have their safety values
	 * applied if they match at all.
	 */
	public WayProperties getDataForWay(OSMWithTags way) {
		...
		result.setSafetyFeatures(new P2<Double>(
				rightResult.getSafetyFeatures().first, leftResult
						.getSafetyFeatures().second));
	...
	public float getCarSpeedForWay(OSMWithTags way, boolean back) {
		...
		if (speed == null && way.hasTag("maxspeed:lanes")) {
			for (String lane : way.getTag("maxspeed:lanes").split("\\|")) {
				...
				// currentSpeed might be null if it was invalid, for instance
				// 10|fast|20
		...
		// this would be bad, as the segment could never be traversed by an
		// automobile
		...
		// SpeedPickers are constructed in DefaultWayPropertySetSource with an
		// OSM specifier
	...
	public int hashCode() {
		return defaultProperties.hashCode() + wayProperties.hashCode()
				+ creativeNamers.hashCode() + slopeOverrides.hashCode();
	}
	...
	public Float getMetersSecondFromSpeed(String speed) {
		...
		if (units == "kmh" || units == "km/h" || units == "kmph"
				|| units == "kph")
