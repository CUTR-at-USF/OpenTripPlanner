...
public class PortlandCustomNamer implements CustomNamer {
	...
	public void nameWithEdge(OSMWithTags way, StreetEdge edge) {
		if (!edge.hasBogusName()) {
			return; // this edge already has a real name so there is nothing to
					// do
		}
		String highway = way.getTag("highway");
		...
		} else if ("secondary_link".equals(highway)
	...
	private String nameAccordingToDestination(Graph graph, StreetEdge e,
			int maxDepth) {
		if (maxDepth == 0) {
			return null;
		}
		Vertex toVertex = e.getToVertex();
		for (StreetEdge out : Iterables.filter(toVertex.getOutgoing(),
				StreetEdge.class)) {
			if (out.hasBogusName()) {
				String name = nameAccordingToDestination(graph, out,
						maxDepth - 1);
				if (name == null) {
					continue;
				}
	...
	private String nameAccordingToOrigin(Graph graph, StreetEdge e, int maxDepth) {
		...
		for (StreetEdge in : Iterables.filter(fromVertex.getIncoming(),
				StreetEdge.class)) {
			if (in.hasBogusName()) {
