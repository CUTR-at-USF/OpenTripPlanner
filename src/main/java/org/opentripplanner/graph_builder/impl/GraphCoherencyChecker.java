...
/**
 * Check the every vertex and edge in the graph to make sure the edge lists and
 * from/to members are coherent, and that there are no edgeless vertices.
...
public class GraphCoherencyChecker implements GraphBuilder {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(GraphCoherencyChecker.class);
		...
		for (Vertex v : graph.getVertices()) {
			...
			for (Edge e : v.getOutgoing()) {
				if (e.getFromVertex() != v) {
					LOG.error(
							"outgoing edge of {}: from vertex {} does not match",
							v, e);
					coherent = false;
				}
			...
			for (Edge e : v.getIncoming()) {
				...
				if (e.getToVertex() != v) {
					LOG.error(
							"incoming edge of {}: to vertex {} does not match",
							v, e);
					coherent = false;
				}
			}
		}
		LOG.info("edge lists and from/to members are {}coherent.",
				coherent ? "" : "not ");
	}
