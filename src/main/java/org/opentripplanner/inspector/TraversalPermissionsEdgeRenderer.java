...
/**
 * Render traversal permissions for each edge by color and label (walk, bicycle,
 * car, stairs).
 * 
 * @author laurent
 */
public class TraversalPermissionsEdgeRenderer implements EdgeVertexRenderer {
	...
	private static final Color TRANSIT_STOP_COLOR_VERTEX = new Color(0.0f,
			0.0f, 0.8f);

	private static final Color TRANSIT_STATION_COLOR_VERTEX = new Color(0.4f,
			0.0f, 0.8f);

	private static final Color BIKE_RENTAL_COLOR_VERTEX = new Color(0.0f, 0.7f,
			0.0f);
	...
	private Color getColor(StreetTraversalPermission permissions) {
		/*
		 * We use the trick that there are 3 main traversal modes (WALK, BIKE
		 * and CAR) and 3 color channels (R, G, B).
		 */
		float r = 0.2f;
		float g = 0.2f;
		float b = 0.2f;
