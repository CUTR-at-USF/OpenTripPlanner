...
/**
 * A TileRenderer implementation which get all edges/vertex in the bounding box
 * of the tile, and call a EdgeVertexRenderer for getting rendering attributes
 * of each (color, string label...).
 * 
 * @author laurent
 */
public class EdgeVertexTileRenderer implements TileRenderer {
	...
	public interface EdgeVertexRenderer {
		...
		/**
		 * @param v
		 *            The vertex being rendered.
		 * @param attrs
		 *            The vertex visual attributes to fill-in.
		 * @return True to render this vertex, false otherwise.
		 */
		public abstract boolean renderVertex(Vertex v,
	...
	public void renderTile(TileRenderContext context) {
		...
		float lineWidth = (float) (1.0f + 3.0f / Math
				.sqrt(context.metersPerPixel));

		// Grow a bit the envelope to prevent rendering glitches between tiles
		Envelope bboxWithMargins = context.expandPixels(lineWidth * 2.0,
				lineWidth * 2.0);

		Collection<Vertex> vertices = context.graph.streetIndex
				.getVerticesForEnvelope(bboxWithMargins);
		Collection<Edge> edges = context.graph.streetIndex
				.getEdgesForEnvelope(bboxWithMargins);
		Set<Edge> edgesSet = new HashSet<>(edges);

		/*
		 * Some edges do not have geometry and thus do not get spatial-indexed.
		 * Add outgoing/incoming edges of all vertices. This is not perfect, as
		 * if the edge cross a tile it will not be rendered on it.
		 */
		...
		// Note: we do not use the transform inside the shapeWriter, but do it
		// ourselves
		...
		Stroke stroke = new BasicStroke(lineWidth * 1.4f,
				BasicStroke.CAP_ROUND, BasicStroke.JOIN_BEVEL);
		Stroke halfStroke = new BasicStroke(lineWidth * 0.6f + 1.0f,
				BasicStroke.CAP_ROUND, BasicStroke.JOIN_BEVEL);
		Stroke halfDashedStroke = new BasicStroke(lineWidth * 0.6f + 1.0f,
				BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 1.0f,
				new float[] { 4 * lineWidth, lineWidth }, 2 * lineWidth);
		Stroke arrowStroke = new ShapeStroke(new Polygon(
				new int[] { 0, 0, 30 }, new int[] { 0, 20, 10 }, 3),
				lineWidth / 2, 5.0f * lineWidth, 2.5f * lineWidth);
		...
		Font largeFont = new Font(Font.SANS_SERIF, Font.PLAIN,
				Math.round(lineWidth * 1.5f));
		FontMetrics largeFontMetrics = context.graphics
		...
		for (Edge edge : edgesSet) {
			...
			if (edgeGeom == null) {
				...
				edgeGeom = GeometryUtils.getGeometryFactory().createLineString(
						coordinates);
				hasGeom = false;
			}
			...
			OffsetCurveBuilder offsetBuilder = new OffsetCurveBuilder(
					new PrecisionModel(), bufParams);
			Coordinate[] coords = offsetBuilder.getOffsetCurve(
					midLineGeom.getCoordinates(), lineWidth * 0.4);
			...
			if (evAttrs.label != null && lineWidth > 8.0f) {
				context.graphics.setColor(Color.BLACK);
				context.graphics.setStroke(new TextStroke("    "
						+ evAttrs.label + "                              ",
						font, false, true));
				context.graphics.draw(offsetShape);
			}
		...
		for (Vertex vertex : vertices) {
			vvAttrs.color = null;
			vvAttrs.label = null;
			Point point = geomFactory.createPoint(new Coordinate(vertex
					.getLon(), vertex.getLat()));
			...
			if (vvAttrs.label != null && lineWidth > 6.0f
				...
				/*
				 * Poor man's solution: stay on the tile if possible. Otherwise
				 * the renderer would need to expand the envelope by an
				 * unbounded amount (max label size).
				 */
				double x = tilePoint.getX();
				if (x + labelWidth > context.tileWidth)
					x -= labelWidth;
				context.graphics.drawString(vvAttrs.label, (float) x,
