...
public class NodeTrackerTest {
	...
	/**
	 * Check the NodeTracker against a stock Set<Long>. This is done in ranges
	 * that will include numbers significantly greater than 2^32
	 */
	@Test
	public void testAgainstSet() {
		...
		for (long lo : new long[] { 1L << 6, 1L << 16, 1L << 34, 1L << 50 }) {
			// Simultaneously set up a basic Set<Long> that will serve as a
			// reference
			...
			// Check that the two set implementations match for every value in
			// the range (including unset ones).
			// Note that a Set<Long> containing 0L returns false for
			// contains((int)0).
