...
public class TestBanning extends TestCase {
	...
	public void testBannedRoutes() {
		...
		options.dateTime = TestUtils.dateInSeconds("America/Los_Angeles", 2009,
				11, 1, 12, 34, 25);
		...
		/*
		 * The MAX Red, Blue, and Green lines all run along the same trackage
		 * between the stops 8374 and 8371. Together, they form the white line.
		 * No, wait, that's light. They make a pretty good test case for banned
		 * routes, since if one is banned, you can always take another.
		 */
		String[][] maxLines = { { "MAX Red Line", null },
				{ "MAX Blue Line", null }, { "MAX Green Line", null },
				{ null, "90" }, { null, "100" }, { null, "200" } };
		for (int i = 0; i < maxLines.length; ++i) {
			String lineName = maxLines[i][0];
			String lineId = maxLines[i][1];
			String routeSpecStr = "TriMet_"
	...
	/**
	 * Test trip banning. We compute a set of shortest routes between two random
	 * stops in the Portland graph. We then ban, for each route, up to a certain
	 * amount of trips used in this route, one by one, and recompute the path.
	 * The banned trips must not appear in the new computed route.
	 * 
	 * This is using a seeded random generator to easily make a reproducible and
	 * arbitrary list of start/end points and trip to ban. It allow for a (bit)
	 * more coverage than doing a single hand-picked test only.
	 * 
	 * @param partial
	 *            True to test partial trip banning, false for complete trip
	 * @param seed
	 *            Value to use for random generator seed -- Keep the same value
	 *            for consistency.
	 */
	public void doTestBannedTrips(boolean partial, int seed) {
		...
		for (int i = 0; i < 20; i++) {
			RoutingRequest options = new RoutingRequest();
			options.dateTime = TestUtils.dateInSeconds("America/Los_Angeles",
					2009, 11, 1, 12, 34, 25);
			...
			for (int j = 0; j < n; j++) {
				...
				for (State s : path.states) {
					if (s.getBackEdge() instanceof TransitBoardAlight) {
						TransitBoardAlight tbae = (TransitBoardAlight) s
								.getBackEdge();
						...
						if (tripId != null)
							usedTripDefs
									.add(new T2<AgencyAndId, BannedStopSet>(
											tripId, stopSet));
					}
				}
				// Used trips should not contains a banned trip
				for (T2<AgencyAndId, BannedStopSet> usedTripDef : usedTripDefs) {
					BannedStopSet bannedStopSet = options.bannedTrips
							.get(usedTripDef.first);
				...
				if (usedTripDefs.size() == 0)
					break; // Not a transit trip, no sense to ban trip any
							// longer
				...
				List<T2<AgencyAndId, BannedStopSet>> usedTripDefsList = new ArrayList<T2<AgencyAndId, BannedStopSet>>(
						usedTripDefs);
				T2<AgencyAndId, BannedStopSet> tripDefToBan = usedTripDefsList
						.get(rand.nextInt(usedTripDefs.size()));
				options.bannedTrips
						.put(tripDefToBan.first, tripDefToBan.second);
			}
