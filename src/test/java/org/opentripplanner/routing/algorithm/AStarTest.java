...
public class AStarTest {
	...
	public void before() {
		...
		edges("market_24th", "market_ballard", "market_22nd", "market_leary",
				"market_russell", "market_20th");
		edges("market_24th", "shilshole_24th", "shilshole_22nd",
				"shilshole_vernon", "shilshole_20th");
		edges("market_ballard", "ballard_turn", "ballard_22nd",
				"ballard_vernon", "ballard_20th");
		...
		edges("market_20th", "russell_20th", "leary_20th", "ballard_20th",
				"shilshole_20th");
	...
	public void testBack() {
		...
		int n;
		// we could go either way around the block formed by 56th, 22nd, market,
		// and 24th.
		if (states.size() == 7) {
			assertEquals("market_24th", states.get(1).getVertex().getLabel());
			assertEquals("market_ballard", states.get(2).getVertex().getLabel());
	...
	public void testForwardExtraEdges() {
		...
		StreetLocation fromLocation = new StreetLocation(_graph,
				"near_shilshole_22nd", new Coordinate(-122.385050, 47.666620),
				"near_shilshole_22nd");
		fromLocation.getExtra().add(
				new SimpleConcreteEdge(fromLocation, _graph
						.getVertex("shilshole_22nd")));

		StreetLocation toLocation = new StreetLocation(_graph,
				"near_56th_20th", new Coordinate(-122.382347, 47.669518),
				"near_56th_20th");
		toLocation.getExtra().add(
				new SimpleConcreteEdge(_graph.getVertex("56th_20th"),
						toLocation));
		...
		assertEquals("near_shilshole_22nd", states.get(0).getVertex()
				.getLabel());
	...
	public void testMultipleTargets() {
		...
		SearchTerminationStrategy strategy = new MultiTargetTerminationStrategy(
				targets);
		ShortestPathTree tree = new GenericAStar().getShortestPathTree(options,
				-1, strategy);
	...
	private SimpleConcreteVertex vertex(String label, double lat, double lon) {
		SimpleConcreteVertex v = new SimpleConcreteVertex(_graph, label, lat,
				lon);
		return v;
	}
