...
public class TurnCostTest {
	...
	public void before() {
		...
		proto.traversalCostModel = (new ConstantIntersectionTraversalCostModel(
				0.0));
	}

	private GraphPath checkForwardRouteDuration(RoutingRequest options,
	...
	public void testForwardDefaultConstTurnCosts() {
		RoutingRequest options = proto.clone();
		options.traversalCostModel = (new ConstantIntersectionTraversalCostModel(
				10.0));
		options.setRoutingContext(_graph, topRight, bottomLeft);
		...
		// The intersection traversal cost should be applied to the state
		// *after*
		...
		assertEquals(50, states.get(1).getElapsedTimeSeconds()); // maple_main1
																	// = 50
		assertEquals(160, states.get(2).getElapsedTimeSeconds()); // main1_2 =
																	// 100
		assertEquals(220, states.get(3).getElapsedTimeSeconds()); // main_broad2
																	// = 50
		assertEquals(330, states.get(4).getElapsedTimeSeconds()); // broad2_3 =
																	// 100
	}
	...
	public void testForwardCarConstTurnCosts() {
		...
		assertEquals(160, states.get(2).getElapsedTimeSeconds()); // main1_2 =
																	// 100
		assertEquals(270, states.get(3).getElapsedTimeSeconds()); // broad1_2 =
																	// 100
		assertEquals(380, states.get(4).getElapsedTimeSeconds()); // broad2_3 =
																	// 100
	}
	 ...
	 * @param length
	 * @param back
	 *            true if this is a reverse edge
	 */
	...
	private void DisallowTurn(StreetEdge from, StreetEdge to) {
		...
		TurnRestriction restrict = new TurnRestriction(from, to, rType,
				restrictedModes);
