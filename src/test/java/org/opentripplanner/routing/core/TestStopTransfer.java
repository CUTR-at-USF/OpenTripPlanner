...
public class TestStopTransfer extends TestCase {
	...
	public void testStopTransfer() {
		...
		assertEquals(StopTransfer.UNKNOWN_TRANSFER,
				transfer.getTransferTime(fromTrip, toTrip));
		assertEquals(StopTransfer.UNKNOWN_TRANSFER,
				transfer.getTransferTime(fromTrip, toTrip2));

		// Add empty SpecificTransfer, specificity 0
		transfer.addSpecificTransfer(new SpecificTransfer((AgencyAndId) null,
				null, null, null, StopTransfer.FORBIDDEN_TRANSFER));
		...
		transfer.addSpecificTransfer(new SpecificTransfer(null, toRoute2
				.getId(), null, null, StopTransfer.PREFERRED_TRANSFER));
		assertEquals(StopTransfer.FORBIDDEN_TRANSFER,
				transfer.getTransferTime(fromTrip, toTrip));
		...
		transfer.addSpecificTransfer(new SpecificTransfer(null, toRoute2
				.getId(), null, toTrip2.getId(), StopTransfer.TIMED_TRANSFER));
		assertEquals(StopTransfer.FORBIDDEN_TRANSFER,
				transfer.getTransferTime(fromTrip, toTrip));
		...
		transfer.addSpecificTransfer(new SpecificTransfer(fromRoute.getId(),
				toRoute2.getId(), fromTrip.getId(), null,
