...
public class TestOnBoardRouting extends TestCase {
	 ...
	 * 
	 * For each departure/arrival location, compute a normal path (depart
	 * alighted). Then re-run the same itinerary but with departure while
	 * on-board at a randomly-picked up trip alongside the path.
	 * 
	 * We assert that the two itineraries will arrive at the same time, at the
	 * same place, with at least one less boarding, and take a less or equals
	 * amount of time.
	 */
	...
	public void testOnBoardRouting() throws Exception {
		...
		while (true) {
			...
			do {
				...
				origin = graph.getVertex("agency:"
						+ (char) (65 + rand.nextInt(20)));
				destination = graph.getVertex("agency:"
						+ (char) (65 + rand.nextInt(20)));
			} while (origin.equals(destination));

			/* ...at a random date/time */
			RoutingRequest options = new RoutingRequest();
			options.dateTime = TestUtils.dateInSeconds("America/New_York",
					2009, 5 + rand.nextInt(4), 1 + rand.nextInt(20),
					4 + rand.nextInt(10), rand.nextInt(60), 0);
			...
			System.out.println("Testing path between " + origin.getLabel()
					+ " and " + destination.getLabel() + " at "
					+ new Date(options.dateTime * 1000));
			...
			for (State s : path.states) {
				...
				if (v instanceof PatternArriveVertex
					...
					if (hop == nhop) {
						...
						/*
						 * New start time at k% of hop. Note: do not try to
						 * make: round(time + k.hop) as it will be off few
						 * seconds due to floating-point rounding errors.
						 */
						newStart = s.getBackState().getTimeSeconds()
								+ Math.round(hopDuration * k);
						break;
					}
					nhop++;
				}
			}
			System.out.println("Boarded depart: trip=" + tripTimes.trip
					+ ", nextStop=" + nextV.getStop() + " stopIndex="
					+ stopIndex + " startTime=" + new Date(newStart * 1000L));

			/* And use it for onboard departure */
			double lat = end.getLat();
			double lon = end.getLon(); // Mock location, not really important
										// here.
			OnboardDepartVertex onboardOrigin = new OnboardDepartVertex(
					"OnBoard_Origin", lat, lon);
			@SuppressWarnings("unused")
			OnBoardDepartPatternHop currentHop = new OnBoardDepartPatternHop(
					onboardOrigin, nextV, tripTimes,
					options.rctx.serviceDays.get(1), stopIndex, k);
