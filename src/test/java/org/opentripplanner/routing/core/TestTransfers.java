...
/**
 * This is a singleton class to hold graph data between test runs, since loading
 * it is slow.
 */
class Context {
	public Graph graph;
	 ...
	 * 
	 * @param from
	 *            is label of from vertex
	 * @param to
	 *            is label of to vertex
	 * @param distance
	 *            is distance of transfer
	 */
...
public class TestTransfers extends TestCase {
	 ...
	 * 
	 * @param options
	 *            are options to use for planning the journey
	 ...
	 *            are options to use for planning the journey
	 * @param optimize
	 *            is true when optimization should be used
	 * @return ordered list of states and edges in the journey
	 */
	...
	public void testStopToStopTransfer() throws Exception {
		...
		options.dateTime = TestUtils.dateInSeconds("America/New_York", 2009, 7,
				11, 11, 11, 0);
	...
	public void testStopToStopTransferWithFrequency() throws Exception {
		...
		for (State s : path.states) {
			if (s.getBackEdge() instanceof TransitBoardAlight
					&& ((TransitBoardAlight) s.getBackEdge()).boarding) {
				time = s.getTimeSeconds(); // find the final board edge, don't
											// break
			}
		}
		assertTrue(time >= 0);
		...
		for (State s : path.states) {
			if (s.getBackEdge() instanceof TransitBoardAlight
					&& ((TransitBoardAlight) s.getBackEdge()).boarding) {
				newTime = s.getTimeSeconds(); // find the final board edge,
												// don't break
			}
		}
	...
	public void testForbiddenStopToStopTransferWithFrequencyInReverse()
			throws Exception {
		// Replace the transfer table with an empty table
		TransferTable table = new TransferTable();
		when(graph.getTransferTable()).thenReturn(table);
	...
	public void testTimedStopToStopTransfer() throws Exception {
		...
		// Now apply a real-time update: let the to-trip be early by 27600
		// seconds,
		...
		// Now apply a real-time update: let the to-trip be early by 27601
		// seconds,
