 ...
 * 
 * TODO(flamholz): add tests for traversal with traffic lights and without a
 * car.
 * 
 * @author avi
 */
public class SimpleTraversalCostModelTest {
	...
	public void testCalculateTurnAngle() {
		...
		IntersectionVertex v1 = vertex("maple_1st", new Coordinate(2.0, 2.0),
				false);
		IntersectionVertex v2 = vertex("maple_2nd", new Coordinate(2.0, 1.0),
				false);
	...
	public void testFreeFlowing() {
		...
		double traversalCost = costModel.computeTraversalCost(v, fromEdge,
				toEdge, mode, options, fromSpeed, toSpeed);
	...
	public void testInferredFreeFlowing() {
		...
		// Two edges - will infer that the vertex is free-flowing since there is
		// no light.
		StreetEdge fromEdge = edge(u, v, 1.0, false);
		StreetEdge toEdge = edge(v, w, 1.0, false);
	...
	private IntersectionVertex vertex(String label, Coordinate coord,
			boolean hasLight) {
		IntersectionVertex v = new IntersectionVertex(_graph, label, coord.y,
				coord.x);
