...
public class LongDistancePathServiceTest {
	...
	public final void testTerminalFor() {
		...
		when(stationStopState.getBackEdge()).thenReturn(
				mock(StationStopEdge.class));
		when(stationStopState.getVertex()).thenReturn(mock(TransitStop.class));

		State stopStationState = mock(State.class);
		when(stopStationState.getBackEdge()).thenReturn(
				mock(StationStopEdge.class));
		when(stopStationState.getVertex()).thenReturn(
				mock(TransitStation.class));

		try {
			parser.terminalFor(emptyState);
			fail();
		} catch (Throwable throwable) {
			assertEquals(RuntimeException.class, throwable.getClass()); // A
																		// back
																		// edge
																		// must
																		// be
	...
	public final void testPathParser() {
		...
		{ // Test street link-street link (not allowed; not necessary in long
			// distance mode)
			List<Class<? extends Edge>> path = new ArrayList<Class<? extends Edge>>();
			path.add(StreetTransitLink.class);
			path.add(StreetTransitLink.class);
	 ...
	 * 
	 * Assumes that only the back edge is used to determine the terminal, except
	 * in the case of
	 * 
	 * @link{StationStopEdge . For that special case, the vertex of the first
	 *                       state is specified as
	 * @link{TransitStop while the vertex of the final state is specified as
	 *                   @link{TransitStation}.
	 * 
	 * @param parser
	 *            is the long distance path parser
	 * @param path
	 *            is a list of edge classes that represent a path
