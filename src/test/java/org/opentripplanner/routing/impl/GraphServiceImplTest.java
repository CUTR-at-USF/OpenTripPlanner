...
public class GraphServiceImplTest extends TestCase {
	...
	protected void setUp() throws IOException {
		...
		// Create a small graph with 2 vertices and one edge and it's serialized
		// form
		...
		new StreetEdge(v1, v2, null, "v1v2", 11000,
	...
	public final void testGraphServiceFile() throws IOException {
		...
		assertTrue(new File(new File(basePath, "A"),
				InputStreamGraphSource.GRAPH_FILENAME).canRead());
		...
		boolean deleted = new File(new File(basePath, "A"),
				InputStreamGraphSource.GRAPH_FILENAME).delete();
		assertTrue(deleted);
		graphService.reloadGraphs(false);
		...
		registered = graphService.registerGraph("A",
				graphSourceFactory.createGraphSource("A"));
