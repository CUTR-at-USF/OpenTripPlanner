...
/**
 * Check that the graph index is created, that GTFS elements can be found in the
 * index, and that the indexes are coherent with one another.
 *
...
public class GraphIndexTest extends GtfsTest {
	...
	/**
	 * Check that bidirectional relationships between TripPatterns and Trips,
	 * Routes, and Stops are coherent.
	 */
	public void testPatternsCoherent() {
		...
		/*
		 * This one depends on a feed where each TripPattern appears on only one
		 * route.
		 */
		...
		for (Stop stop : graph.index.stopForId.values()) {
			for (TripPattern pattern : graph.index.patternsForStop.get(stop)) {
				assertTrue(pattern.stopPattern.containsStop(stop.getId()
						.toString()));
			}
		}
	}
	...
	public void testSpatialIndex() {
		...
		Envelope env = new Envelope(new Coordinate(stopJ.getLon(),
				stopJ.getLat()));
		env.expandBy(SphericalDistanceLibrary.metersToLonDegrees(100,
				stopJ.getLat()), SphericalDistanceLibrary.metersToDegrees(100));
