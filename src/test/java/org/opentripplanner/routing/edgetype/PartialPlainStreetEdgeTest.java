...
public class PartialPlainStreetEdgeTest {
	...
	public void testConstruction() {
		...
		PartialStreetEdge pEdge = new PartialStreetEdge(e1, v1, v2,
				e1.getGeometry(), "partial e1", e1.getDistance(), perm, true);
		...
		// Simpler constructor - copies permission from parent edge and sets
		// back to true.
		pEdge = new PartialStreetEdge(e1, v1, v2, e1.getGeometry(),
	...
	public void testTraversal() {
		...
		PartialStreetEdge pEdge1 = new PartialStreetEdge(e1, v1, v2,
				e1.getGeometry(), "partial e1", e1.getDistance());
		PartialStreetEdge pEdge2 = new PartialStreetEdge(e2, v2, v3,
				e2.getGeometry(), "partial e2", e2.getDistance());

		// Partial edges are temporary edges. They are only traversable by one
		// routing context.
		// They are associated with a routing context when an edge is split for
		// a StreetLocation,
		...
		// Traversal of original and partial edges should yield the same
		// results.
		assertEquals(s1.getTimeSeconds(), partialS1.getTimeSeconds());
		assertEquals(s1.getElapsedTimeSeconds(),
				partialS1.getElapsedTimeSeconds());
		...
		assertEquals(s2.getElapsedTimeSeconds(),
				partialS2.getElapsedTimeSeconds());
	...
	public void testTraversalOfSubdividedEdge() {
		...
		edges.add(e2);
		StreetLocation intermediate = StreetLocation.createStreetLocation(
				_graph, "middle of e2", "foo", edges, nearestPoint);

		RoutingRequest options = new RoutingRequest();
		options.setMode(TraverseMode.CAR);
		options.setRoutingContext(_graph, v1, v2);
		// Creating a streetlocation splits a street and makes temporary edges
		// that are only visible to one routing context.
		intermediate.setTemporaryEdgeVisibility(options.rctx);

		// All intersections take 10 minutes - we'll notice if one isn't
		// counted.
		...
		assertTrue(Math.abs(s3.getElapsedTimeSeconds()
				- partialS3.getElapsedTimeSeconds()) <= 1);
		...
		// Time and cost should be the same up to a rounding difference.
		assertTrue(Math.abs(s3NoCost.getTimeSeconds()
				- partialS3NoCost.getTimeSeconds()) <= 1);
		assertTrue(Math.abs(s3NoCost.getElapsedTimeSeconds()
				- partialS3NoCost.getElapsedTimeSeconds()) <= 1);
		...
		double partialDurationDiff = partialS3.getTimeSeconds()
		...
		double partialWeightDiff = partialS3.getWeight()
				- partialS3NoCost.getWeight();
	...
	public void testReverseEdge() {
		PartialStreetEdge pEdge1 = new PartialStreetEdge(e1, v1, v2,
				e1.getGeometry(), "partial e1", e1.getDistance());
		PartialStreetEdge pEdge2 = new PartialStreetEdge(e1Reverse, v2, v1,
				e1Reverse.getGeometry(), "partial e2", e1Reverse.getDistance());
	...
	private StreetEdge edge(StreetVertex vA, StreetVertex vB, double length,
		...
		LineString geom = GeometryUtils.getGeometryFactory().createLineString(
				coords);
	 ...
	 */
	private static class DummyCostModel implements
		...
		@Override
		public double computeTraversalCost(IntersectionVertex v,
				StreetEdge from, StreetEdge to, TraverseMode mode,
