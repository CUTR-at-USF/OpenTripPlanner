...
public class TestTriangle extends TestCase {
	...
	public void testTriangle() {
		...
		LineString geometry = factory.createLineString(new Coordinate[] { c1,
				c2 });

		double length = 650.0;

		StreetWithElevationEdge testStreet = new StreetWithElevationEdge(v1,
				v2, geometry, "Test Lane", length,
		...
		PackedCoordinateSequence elev = new PackedCoordinateSequence.Double(
				profile);
		...
		double expectedTimeWeight = slopeSpeedLength
		...
		double expectedSlopeWeight = slopeWorkLength
				/ options.getSpeed(TraverseMode.BICYCLE);
		...
		assertTrue(Math.abs(safetyWeight * ONE_THIRD + slopeWeight * ONE_THIRD
				+ timeWeight * ONE_THIRD - averageWeight) < 0.00000001);
