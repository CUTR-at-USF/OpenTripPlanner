...
public class PlainStreetEdgeTest {
	...
	public void testInAndOutAngles() {
		...
		// 180 degrees could be expressed as 180 or -180. Our implementation
		// happens to use -180.
	...
	/**
	 * Test the traversal of two edges with different traverse modes, with a
	 * focus on cycling. This test will fail unless the following three
	 * conditions are met: 1. Turn costs are computed based on the back edge's
	 * traverse mode during reverse traversal. 2. Turn costs are computed such
	 * that bike walking is taken into account correctly. 3. User-specified bike
	 * speeds are applied correctly during turn cost computation.
	 */
	@Test
	public void testTraverseModeSwitchBike() {
		StreetEdge e0 = edge(v0, v1, 50.0, StreetTraversalPermission.PEDESTRIAN);
		StreetEdge e1 = edge(v1, v2, 18.4,
	...
	/**
	 * Test the traversal of two edges with different traverse modes, with a
	 * focus on walking. This test will fail unless the following three
	 * conditions are met: 1. Turn costs are computed based on the back edge's
	 * traverse mode during reverse traversal. 2. Turn costs are computed such
	 * that bike walking is taken into account correctly. 3. Enabling bike mode
	 * on a routing request bases the bike walking speed on the walking speed.
	 */
	@Test
	public void testTraverseModeSwitchWalk() {
		StreetEdge e0 = edge(v0, v1, 50.0,
				StreetTraversalPermission.PEDESTRIAN_AND_BICYCLE);
	...
	/**
	 * Test the bike switching penalty feature, both its cost penalty and its
	 * separate time penalty.
	 */
	@Test
	...
	public void testTurnRestriction() {
		...
		_graph.addTurnRestriction(e1, new TurnRestriction(e1, e0, null,
