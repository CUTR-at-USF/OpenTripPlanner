...
public class TestPatternHopFactory extends TestCase {
	...
	public void setUp() throws Exception {
		...
		String[] stops = { "agency:A", "agency:B", "agency:C", "agency:D",
				"agency:E" };
		for (int i = 0; i < stops.length; ++i) {
			TransitStop stop = (TransitStop) (graph.getVertex(stops[i]));

			IntersectionVertex front = new IntersectionVertex(graph, "near_1_"
					+ stop.getStopId(), stop.getX() + 0.0001,
					stop.getY() + 0.0001);
			IntersectionVertex back = new IntersectionVertex(graph, "near_2_"
					+ stop.getStopId(), stop.getX() - 0.0001,
					stop.getY() - 0.0001);

			StreetEdge street1 = new StreetEdge(front, back,
					GeometryUtils.makeLineString(stop.getX() + 0.0001,
							stop.getY() + 0.0001, stop.getX() - 0.0001,
							stop.getY() - 0.0001), "street", 100,
					StreetTraversalPermission.ALL, false);
			StreetEdge street2 = new StreetEdge(back, front,
					GeometryUtils.makeLineString(stop.getX() - 0.0001,
							stop.getY() - 0.0001, stop.getX() + 0.0001,
							stop.getY() + 0.0001), "street", 100,
	...
	public void testAnnotation() {
		...
		for (GraphBuilderAnnotation annotation : graph.getBuilderAnnotations()) {
			if (annotation instanceof NegativeHopTime) {
				...
				assertTrue(nht.st0.getDepartureTime() > nht.st1
						.getArrivalTime());
				found = true;
			}
		}
	...
	public void testBoardAlight() throws Exception {
		...
		TransitBoardAlight pb = (TransitBoardAlight) stop_a_depart
	...
	public void testBoardAlightStopIndex() {
		...
		for (Edge edge : stop_b_arrive.getIncoming()) {
			...
			if (stopIndex.containsKey(tba.getPattern()))
				assertEquals((Integer) stopIndex.get(tba.getPattern()),
						new Integer(tba.getStopIndex()));
		}
	}
	...
	public void testRouting() throws Exception {
		...
		long startTime = TestUtils.dateInSeconds("America/New_York", 2009, 8,
				7, 0, 0, 0);
	...
	public void testRoutingOverMidnight() throws Exception {
		...
		// Friday evening
		options.dateTime = TestUtils.dateInSeconds("America/New_York", 2009, 8,
				18, 23, 20, 0);
	...
	/*
	 * Somewhat hackish convenience method to grab a hop edge on a particular
	 * route leaving a particular stop.
	 */
	private PatternHop getHopEdge(String stopId, String routeId) {
		Vertex stopDepartVertex = graph.getVertex("agency:" + stopId
				+ "_depart");
		for (Edge edge : stopDepartVertex.getOutgoing()) {
			if (edge instanceof TransitBoardAlight) {
				TransitBoardAlight tba = ((TransitBoardAlight) edge);
				if (tba.boarding
						&& tba.getPattern().route.getId().getId()
								.equals(routeId)) {
	...
	public void testShapeByLocation() throws Exception {
		PatternHop hop;
		Geometry geom;
		// Only route 4 goes through stop G and route 4 should contain only one
		// pattern.
		hop = getHopEdge("G", "4");
		geom = hop.getGeometry();
		assertEquals(geom.getLength(), 1.16, 0.1);
		// Only route 1 goes through stop A, and route 1 should contain only one
		// pattern.
		hop = getHopEdge("A", "1");
		geom = hop.getGeometry();
	...
	public void testPickupDropoff() throws Exception {
		...
		assertNotNull(path);
		long endTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 19,
				12, 10, 0);
		assertEquals(endTime, path.getEndTime());

		startTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 19,
				12, 0, 1);
		options.dateTime = startTime;
		options.setRoutingContext(graph, stop_o, stop_p);
		spt = aStar.getShortestPathTree(options);
		...
		assertNotNull(path);
		endTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 19, 15,
				10, 0);
		assertEquals(endTime, path.getEndTime());
	}
	...
	public void testTransfers() throws Exception {
		...
		// find stops
		Stop stopK = ((TransitStopArrive) graph.getVertex("agency:K_arrive"))
				.getStop();
		Stop stopN = ((TransitStopDepart) graph.getVertex("agency:N_depart"))
				.getStop();
		Stop stopM = ((TransitStopDepart) graph.getVertex("agency:M_depart"))
				.getStop();

		assertTrue(transferTable.hasPreferredTransfers());
		assertEquals(StopTransfer.UNKNOWN_TRANSFER,
				transferTable.getTransferTime(stopN, stopM, fromTrip, toTrip,
						true));
		assertEquals(StopTransfer.FORBIDDEN_TRANSFER,
				transferTable.getTransferTime(stopK, stopM, fromTrip, toTrip,
						true));
		assertEquals(StopTransfer.PREFERRED_TRANSFER,
				transferTable.getTransferTime(stopN, stopK, toTrip, toTrip2,
						true));
		assertEquals(StopTransfer.TIMED_TRANSFER,
				transferTable.getTransferTime(stopN, stopK, fromTrip, toTrip,
						true));
		assertEquals(15, transferTable.getTransferTime(stopN, stopK, fromTrip,
				toTrip2, true));
		...
		assertTrue("expected to use much later trip due to min transfer time",
	...
	public void testTimelessStops() throws Exception {
		...
		assertNotNull(path);
		assertEquals(TestUtils.dateInSeconds("America/New_York", 2009, 8, 1,
				11, 0, 0), path.getEndTime());
	}
	...
	public void testWheelchairAccessible() throws Exception {
		...
		Vertex stop_d = graph.getVertex("agency:D");
		Vertex split_d = null;
		for (StreetTransitLink e : Iterables.filter(stop_d.getOutgoing(),
				StreetTransitLink.class)) {
		...
		// stop E has no accessibility information, but we should still be able
		// to route to it.
		...
		// from stop A to stop D would normally be trip 1.1 to trip 2.1,
		// arriving at 00:30. But trip
	...
	public void testRunForTrain() {
		 ...
		 * There's a trip from S to T at 8:50 and a second one at 9:50. To get
		 * to S by 8:50, we need to take trip 12.1 from Q to R, and 13.1 from R
		 * to S. If we take the direct-but-slower 11.1, we'll miss the 8:50 and
		 * have to catch the 9:50.
		 */
		Vertex destination = graph.getVertex("agency:T");
		RoutingRequest options = new RoutingRequest();
		// test is designed such that transfers must be instantaneous
		options.transferSlack = (0);
		GregorianCalendar startTime = new GregorianCalendar(2009, 11, 2, 8, 30,
				0);
	...
	public void testFrequencies() {
		...
		assertNotNull(path);
		assertEquals(4, path.states.size());
		long endTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 7,
				6, 40, 0);
		assertEquals(endTime, path.getEndTime());
		...
		assertNotNull(path);
		assertEquals(4, path.states.size());
		endTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 7, 7,
				40, 0);
		assertEquals(endTime, path.getEndTime());
		...
		assertNotNull(path);
		assertEquals(4, path.states.size());
		endTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 7, 14,
				40, 0);
		assertEquals(endTime, path.getEndTime());
