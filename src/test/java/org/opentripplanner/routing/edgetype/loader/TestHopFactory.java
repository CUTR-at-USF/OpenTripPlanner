...
public class TestHopFactory extends TestCase {
	...
	public void testBoardAlight() throws Exception {
		...
		TransitBoardAlight pb = (TransitBoardAlight) stop_a.getOutgoing()
				.iterator().next();
	...
	public void testRouting() throws Exception {
		...
		assertNotNull(path);
		assertEquals(extractStopVertices(path),
				Lists.newArrayList(stop_a, stop_b));
		...
		assertNotNull(path);
		assertEquals(extractStopVertices(path),
				Lists.newArrayList(stop_a, stop_c));
		...
		assertNotNull(path);
		assertEquals(extractStopVertices(path),
				Lists.newArrayList(stop_a, stop_c, stop_d));
		long endTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 7,
				0, 0, 0) + 40 * 60;
		assertEquals(endTime, path.getEndTime());
		...
		assertNotNull(path);
		assertEquals(extractStopVertices(path),
				Lists.newArrayList(stop_a, stop_c, stop_e));
		endTime = TestUtils.dateInSeconds("America/New_York", 2009, 8, 7, 0, 0,
				0) + 70 * 60;
		assertEquals(endTime, path.getEndTime());
	}
