...
public class TripTimesTest {
	private static final AgencyAndId tripId = new AgencyAndId("agency",
			"testtrip");
	...
	private static final AgencyAndId[] stops = { stop_a, stop_b, stop_c,
			stop_d, stop_e, stop_f, stop_g, stop_h };
	...
	public void testBikesAllowed() {
		...
		trip.setRoute(route);
		List<StopTime> stopTimes = Arrays
				.asList(new StopTime(), new StopTime());
	...
	public void testPassedUpdate() {
		...
		assertEquals(TripTimes.UNAVAILABLE,
				updatedTripTimesA.getDepartureTime(0));
	...
	public void testCancel() {
		...
		for (int i = 0; i < stops.length - 1; i++) {
			...
			assertEquals(TripTimes.UNAVAILABLE,
					updatedTripTimesA.getDepartureTime(i));
			assertEquals(TripTimes.UNAVAILABLE,
					updatedTripTimesA.getArrivalTime(i));
		}
	}
