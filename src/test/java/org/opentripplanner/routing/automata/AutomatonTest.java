...
public class AutomatonTest extends TestCase {
	...
	public void setUp() {
		...
		Nonterminal transitLeg = plus(plus(STATION), plus(TRANSIT),
				plus(STATION));
		itinerary = seq(walkLeg, star(transitLeg, walkLeg));
	}

	public void testAutomata() {
		// NFA nfa = seq(WALK, STATION, choice(TRANSIT, WALK), STATION,
		// WALK).toNFA();
	...
	public void testAutomata2() {
		...
		NFA nfa = choice(star(WALK), seq(star(any), TRANSIT, star(any)))
				.toNFA();
	...
	private static void testParse(DFA dfa) {
		...
		testParse(dfa, true, WALK, STATION, TRANSIT, STATION, STATION, TRANSIT,
				STATION, WALK);
