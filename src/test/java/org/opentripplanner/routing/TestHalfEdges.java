...
public class TestHalfEdges extends TestCase {
	...
	public void setUp() {
		...
		StreetEdge bottomBack = new StreetEdge(br, bl, (LineString) bottom
				...
				StreetTraversalPermission.ALL, true);
		leftBack = new StreetEdge(tl, bl, (LineString) left.getGeometry()
				.reverse(), "leftBack", 1500, StreetTraversalPermission.ALL,
				true);
		rightBack = new StreetEdge(tr, br, (LineString) right.getGeometry()
				.reverse(), "rightBack", 1500, StreetTraversalPermission.ALL,
				true);
	...
	public void testHalfEdges() {
		// the shortest half-edge from the start vertex takes you down, but the
		// shortest total path
		// is up and over
		...
		StreetLocation start = StreetLocation.createStreetLocation(graph,
				"start", "start", filter(turns, StreetEdge.class),
		...
		StreetLocation end = StreetLocation.createStreetLocation(graph, "end",
				"end", filter(endTurns, StreetEdge.class), new LinearLocation(
						0, 0.8).getCoordinate(right.getGeometry()));
		...
		long startTime = TestUtils.dateInSeconds("America/New_York", 2009, 11,
				1, 12, 34, 25);
		...
		assertNotNull("There must be a path from start to end (looking back)",
				path);
		...
		// Number of vertices and edges should be the same as before after a
		// cleanup.
		options.cleanup();
		...
		/*
		 * Now, the right edge is not bikeable. But the user can walk their
		 * bike. So here are some tests that prove (a) that walking bikes works,
		 * but that (b) it is not preferred to riding a tiny bit longer.
		 */
		...
		start = StreetLocation.createStreetLocation(graph, "start1", "start1",
				...
				.createStreetLocation(graph, "end1", "end1",
						filter(turns, StreetEdge.class), new LinearLocation(0,
								0.95).getCoordinate(bottom.getGeometry()));
		...
		path = spt.getPath(start, false);
		assertNotNull(
				"There must be a path from top to bottom along the right", path);

		// the left edge is not part of the shortest path (even though the bike
		// must be walked along the right)
		for (State s : path.states) {
		...
		start = StreetLocation.createStreetLocation(graph, "start2", "start2",
				...
				.createStreetLocation(graph, "end2", "end2",
						filter(turns, StreetEdge.class), new LinearLocation(0,
								0.55).getCoordinate(bottom.getGeometry()));
	...
	public void testRouteToSameEdge() {
		...
		StreetLocation end = StreetLocation.createStreetLocation(graph, "end",
				"end", filter(turns, StreetEdge.class), new LinearLocation(0,
						0.8).getCoordinate(left.getGeometry()));
	...
	public void testRouteToSameEdgeBackwards() {
		...
		StreetLocation end = StreetLocation.createStreetLocation(graph, "end",
				"end", filter(turns, StreetEdge.class), new LinearLocation(0,
						0.4).getCoordinate(left.getGeometry()));
	...
	/**
	 * Test that alerts on split streets are preserved, i.e. if there are alerts
	 * on the street that is split the same alerts should be present on the new
	 * street.
	 */
	public void testStreetSplittingAlerts() {
		...
		graph.streetNotesService.addStaticNote(left, alert,
				StreetNotesService.ALWAYS_MATCHER);
		graph.streetNotesService.addStaticNote(leftBack, alert,
				StreetNotesService.ALWAYS_MATCHER);
		...
		// traverse the FreeEdge from the StreetLocation to the new
		// IntersectionVertex
		...
		Alert wheelchairAlert = Alert
				.createSimpleAlerts("This is the wheelchair alert");
		...
		assertEquals(wheelchairAlerts,
				graph.streetNotesService.getNotes(traversedOne));
		assertNotSame(left, traversedOne.getBackEdge().getFromVertex());
		assertNotSame(leftBack, traversedOne.getBackEdge().getFromVertex());
	}

	public void testStreetLocationFinder() {
		StreetVertexIndexServiceImpl finder = new StreetVertexIndexServiceImpl(
				graph);
		// test that the local stop finder finds stops
		GenericLocation loc = new GenericLocation(40.01, -74.005000001);
		assertTrue(finder.getNearbyTransitStops(loc.getCoordinate(), 100)
				.size() > 0);
		...
		RoutingRequest biking = new RoutingRequest(new TraverseModeSet(
		...
		// street)
		StreetLocation location = (StreetLocation) finder
				.getVertexForLocation(
						new GenericLocation(40.00999, -74.004999),
						new RoutingRequest());
		assertTrue(location.isWheelchairAccessible());
		boolean found = false;
		for (Edge extra : location.getExtra()) {
			if (extra instanceof FreeEdge
					&& ((FreeEdge) extra).getToVertex().equals(station1)) {
				found = true;
			}
		}
		assertTrue(found);

		// test that it is possible to travel between two splits on the same
		// street
		RoutingRequest walking = new RoutingRequest(TraverseMode.WALK);
		start = (StreetLocation) finder.getVertexForLocation(
				new GenericLocation(40.004, -74.0), walking);
		end = (StreetLocation) finder.getVertexForLocation(new GenericLocation(
				40.008, -74.0), walking);
		assertNotNull(end);
		// The visibility for temp edges for start and end is set in the
		// setRoutingContext call
