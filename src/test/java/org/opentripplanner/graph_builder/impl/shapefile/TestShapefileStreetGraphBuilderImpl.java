...
public class TestShapefileStreetGraphBuilderImpl extends TestCase {
    ...
    public void testBasic() throws Exception {
        ...
        if (file == null || !file.exists()) {
            ...
            /*
             * This test requires the New York City base map. Place it among the source resources
             * and Eclipse should automatically copy it over to the target directory. Once you have
             * prepared these files, you may need to 'refresh' in Eclipse's package explorer to
             * force Eclipse to notice the new resources.
             * 
             * Recent versions of this map are available only in Arcview Geodatabase format. For
             * conversion to a Shapefile, you will need the archived MapInfo version at:
             * http://www.nyc.gov/html/dcp/html/bytes/bytesarchive.shtml#lion Download the MapInfo
             * file of Lion version 10B.
             * 
             * This must then be converted to a ShapeFile as follows: cd
             * opentripplanner-graph-builder
             * /src/test/resources/org/opentripplanner/graph_builder/impl/shapefile mkdir
             * nyc_streets (this is where we will store the shapefile) unzip nyc_lion10ami.zip (this
             * should place zipfile contents in a ./lion directory) ogr2ogr -f 'ESRI Shapefile'
             ...
             * 
             * Testing also requires NYC Subway data in GTFS in the same location: wget
        ...
        for (Vertex v : gg.getVertices()) {
            if (v.getCoordinate().distance(vanderbiltAtPark) < 0.00005) {
                /*
                 * we need the correct vanderbilt at park. In this case, that's the one facing west
                 * on vanderbilt.
                 */
                int numParks = 0;
                int numCarltons = 0;
                ...
                /*
                 * we need the correct carlton at park. In this case, that's the one facing west.
                 */
                int numFlatbushes = 0;
                int numParks = 0;
         ...
         * 
         * wo.setArriveBy(true); spt = AStar.getShortestPathTreeBack(gg, start.vertex,
         * carlton.vertex, new State(0), wo); assertNotNull(spt);
         * 
         * path = spt.getPath(carlton.vertex); assertTrue(path.edges.size() <= 3);
         */
    }
}
