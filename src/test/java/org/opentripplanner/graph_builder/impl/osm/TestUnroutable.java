...
/**
 * Verify that OSM ways that represent proposed or as yet unbuilt roads are not
 * used for routing. This tests functionality in or around the method
 * isWayRoutable() in the OSM graph builder module.
 *
 * @author abyrd
 */
public class TestUnroutable extends TestCase {
	...
	public void setUp() throws Exception {
		...
		File osmDataFile = new File(URLDecoder.decode(osmDataUrl.getFile(),
				"UTF-8"));
		provider.setPath(osmDataFile);
		osmBuilder.setProvider(provider);
		HashMap<Class<?>, Object> extra = Maps.newHashMap();
		osmBuilder.buildGraph(graph, extra); // TODO get rid of this "extra"
												// thing
	}

	/**
	 * Search for a path across the Willamette river. This OSM data includes a
	 * bridge that is not yet built and is therefore tagged
	 * highway=construction. TODO also test unbuilt, proposed, raceways etc.
	 */
	public void testOnBoardRouting() throws Exception {
		...
		// At the time of writing this test, the router simply doesn't find a
		// path at all when highway=construction
