...
public class OpenStreetMapParserTest {
	...
	public void testAFBinaryParser() throws Exception {
		...
		pr.setPath(new File(URLDecoder.decode(
				getClass().getResource("map.osm.pbf").getPath(), "UTF-8")));
		pr.readOSM(map);
		testParser(map);
	}
	...
	public void testAFXMLParser() throws Exception {
		...
		OSMMap map = new OSMMap();
		pr.setPath(new File(URLDecoder.decode(
				getClass().getResource("map.osm.gz").getPath(), "UTF-8")));
		pr.readOSM(map);
		testParser(map);
	}
	...
	public void testBasicParser() throws Exception {
		InputStream in = new GZIPInputStream(getClass().getResourceAsStream(
				"map.osm.gz"));
