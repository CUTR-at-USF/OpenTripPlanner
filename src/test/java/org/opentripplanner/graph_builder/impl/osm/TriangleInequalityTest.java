...
public class TriangleInequalityTest {
	...
	public static void onlyOnce() throws Exception {
		...
		File file = new File(URLDecoder.decode(TriangleInequalityTest.class
				.getResource("NYC_small.osm.gz").getFile(), "UTF-8"));
	...
	private void checkTriangleInequality(TraverseModeSet traverseModes) {
		...
		// All reluctance terms are 1.0 so that duration is monotonically
		...
		prototypeOptions.traversalCostModel = (new ConstantIntersectionTraversalCostModel(
				10.0));
		...
		assertEquals(startEndWeight, (double) startEndDuration,
				1.0 * path.edges.size());
		...
		for (Vertex intermediate : _graph.getVertices()) {
			...
			GraphPath startIntermediatePath = getPath(aStar, prototypeOptions,
					null, start, intermediate);
			if (startIntermediatePath == null) {
				continue;
			}

			Edge back = startIntermediatePath.states.getLast().getBackEdge();
			GraphPath intermediateEndPath = getPath(aStar, prototypeOptions,
					back, intermediate, end);
			...
			// TODO(flamholz): fix traversal so that there's no rounding at the
			// second resolution.
			assertEquals(startIntermediateWeight,
					(double) startIntermediateDuration,
					1.0 * startIntermediatePath.edges.size());
			assertEquals(intermediateEndWeight,
					(double) intermediateEndDuration,
					1.0 * intermediateEndPath.edges.size());

			double diff = startIntermediateWeight + intermediateEndWeight
					- startEndWeight;
			if (diff < -0.01) {
				System.out.println("Triangle inequality violated - diff = "
						+ diff);
				violated = true;
			}
