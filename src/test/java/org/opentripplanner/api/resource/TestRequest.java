...
class Context {
	...
	public OTPServer otpServer = new OTPServer(commandLineParameters,
			graphService);
	...
	public Context() {
		graphService.registerGraph("", new MemoryGraphSource("",
				makeSimpleGraph())); // default graph is tiny test graph
		graphService.registerGraph("portland", new MemoryGraphSource(
				"portland", graph));
		ShapefileStreetGraphBuilderImpl builder = new ShapefileStreetGraphBuilderImpl();
		FeatureSourceFactory factory = new ShapefileFeatureSourceFactoryImpl(
		...
		perms.addPermission("1", StreetTraversalPermission.ALL,
				StreetTraversalPermission.ALL);
		...
		// to make test results more deterministic, only find the single best
		// path
		// If this is really needed, it should be set in the RoutingRequest in
		// individual tests
	...
	private Graph makeSimpleGraph() {
		...
		StreetVertex tl = new IntersectionVertex(graph, "tl", -80.01, 40.01,
				"top and left");
		StreetVertex tr = new IntersectionVertex(graph, "tr", -80.0, 40.01,
				"top and right");
		StreetVertex bl = new IntersectionVertex(graph, "bl", -80.01, 40.0,
				"bottom and left");
		StreetVertex br = new IntersectionVertex(graph, "br", -80.0, 40.0,
				"bottom and right");
	...
	private void makeEdges(StreetVertex v1, StreetVertex v2, String name) {
		LineString geometry = GeometryUtils.makeLineString(
				...
				v2.getCoordinate().x, v2.getCoordinate().y);
		double length = SphericalDistanceLibrary.getInstance().distance(
				v1.getCoordinate(), v2.getCoordinate());
		new StreetEdge(v1, v2, geometry, name, length,
				StreetTraversalPermission.ALL, false);

		geometry = GeometryUtils.makeLineString(v2.getCoordinate().x,
				v2.getCoordinate().y, v1.getCoordinate().x,
				v1.getCoordinate().y);
		new StreetEdge(v2, v1, geometry, name, length,
...
public class TestRequest extends TestCase {
	...
	public void testBuildRequest() throws Exception {
		/**
		 * Removed on grounds of being redundant and testing internal
		 * functionality only.
		 */
	}

	public void testPlanner() throws Exception {
		TestPlanner planner = new TestPlanner("portland",
				"From::NE 43RD AVE at NE GLISAN ST",
	...
	public void testAlerts() throws Exception {
		// SE 47th and Ash, NE 47th and Davis (note that we cross Burnside, this
		// goes from SE to NE)
		TestPlanner planner = new TestPlanner("portland",
				"SE 47TH AVE at SE ASH ST", "NE 47TH AVE at NE COUCH ST");
		Response response = planner.getItineraries();
	...
	public void testIntermediate() throws Exception {
		...
		Vertex v2 = getVertexByCrossStreets("SE 82ND AVE", "SE ASH ST")
				.getOutgoing().iterator().next().getToVertex();
		Vertex v3 = getVertexByCrossStreets("NE 21ST AVE", "NE MASON ST")
				.getOutgoing().iterator().next().getToVertex();
		...
		TestPlanner planner = new TestPlanner("portland", v1.getLabel(),
				v4.getLabel(), Arrays.asList(v2.getLabel(), v3.getLabel()));
		...
		assertEquals(4, curVertex); // found all four, in the correct order (1,
									// 3, 2, 4)
	}
	...
	public void testMetadata() {
		...
		assertTrue(
				"centerLatitude is not 40.005; got "
						+ data1.getCenterLatitude(),
	...
	public void testPatcher() {
		...
		when(service.getStopPatches(any(AgencyAndId.class))).thenReturn(
	...
	public void testBannedTrips() {
		...
		TestPlanner planner = new TestPlanner("portland",
				"NE 57TH AVE at NE GLISAN ST #2", "NE 30TH AVE at NE GLISAN ST");
	...
	public void testBannedStopGroup() throws ParameterException {
		...
		Stop stop65_tc = ((TransitStationStop) graph.getVertex("TriMet:65-tc"))
				.getStop();
		assertNotNull(stop65_tc);

		Stop stop12921 = ((TransitStationStop) graph.getVertex("TriMet:12921"))
				.getStop();
		assertNotNull(stop12921);

		Stop stop13132 = ((TransitStationStop) graph.getVertex("TriMet:13132"))
				.getStop();
		assertNotNull(stop13132);

		Stop stop2106 = ((TransitStationStop) graph.getVertex("TriMet:2106"))
				.getStop();
		assertNotNull(stop2106);

		Stop stop2107 = ((TransitStationStop) graph.getVertex("TriMet:2107"))
				.getStop();
	...
	public void testWalkLimitExceeded() throws ParameterException {
		// Plan short trip
		TestPlanner planner = new TestPlanner("portland",
				"45.501115,-122.738214", "45.469487,-122.500343");
		// Do the planning
		Response response = planner.getItineraries();
		...
		planner = new TestPlanner("portland", "45.445631,-122.845388",
				"45.459961,-122.752347");
		// Do the planning with high walk reluctance
		response = planner.getItineraries();
		// Check itinerary for walkLimitExceeded
		itinerary = response.getPlan().itinerary.get(0);
		assertTrue(itinerary.walkDistance <= planner.getMaxWalkDistance()
				.get(0));
	...
	public void testTransferPenalty() {
		// Plan short trip
		TestPlanner planner = new TestPlanner("portland",
				"45.5264892578125,-122.60479259490967", "45.511622,-122.645564");
	...
	/**
	 * Checks the number of legs when using a specific transfer penalty while
	 * planning.
	 * 
	 * @param planner
	 *            is the test planner to use
	 * @param transferPenalty
	 *            is the value for the transfer penalty
	 * @param expectedLegs
	 *            is the number of expected legs
	 * @param smaller
	 *            if true, number of legs should be smaller; if false, number of
	 *            legs should be exact
	 */
	private void checkLegsWithTransferPenalty(TestPlanner planner,
			int transferPenalty, int expectedLegs, boolean smaller) {
	...
	public void testTripToTripTransfer() throws ParseException {
		...
		addTripToTripTransferTimeToTable(table, "2111", "7452", "19", "75",
				"190W1280", "751W1330", 126);
		...
		// Now apply a real-time update: let the to-trip have a delay of 3
		// seconds
		Trip trip = graph.index.tripForId.get(new AgencyAndId("TriMet",
				"751W1330"));
		...
		response = planner.getItineraries();
		itinerary = response.getPlan().itinerary.get(0);
		// Check the ids of the first two busses, they should be the original
		// again
		assertEquals("190W1280", itinerary.legs.get(1).tripId);
		assertEquals("751W1330", itinerary.legs.get(3).tripId);
	...
	public void testForbiddenTripToTripTransfer() {
		...
		addTripToTripTransferTimeToTable(table, "2111", "7452", "19", "75",
				"190W1280", "751W1330", StopTransfer.FORBIDDEN_TRANSFER);
	 ...
	 * 
	 * The expected results seem to be dependent on errors in the older
	 * DefaultRemainingWeightHeuristic.
	 * 
	 * public void testPreferredTripToTripTransfer() { // Plan short trip
	 * TestPlanner planner = new TestPlanner( "portland",
	 * "45.506077,-122.621139", "45.464637,-122.706061");
	 * 
	 * // Replace the transfer table with an empty table TransferTable table =
	 * new TransferTable(); Graph graph = Context.getInstance().graph;
	 * when(graph.getTransferTable()).thenReturn(table);
	 * 
	 * // Do the planning Response response = planner.getItineraries();
	 * Itinerary itinerary = response.getPlan().itinerary.get(0); // Check the
	 * ids of the first two busses assertEquals("751W1320",
	 * itinerary.legs.get(1).tripId); assertEquals("91W1350",
	 * itinerary.legs.get(3).tripId);
	 * 
	 * // Now add a preferred transfer between two other busses
	 * addTripToTripTransferTimeToTable(table, "7528", "9756", "75", "12",
	 * "750W1300", "120W1320" , StopTransfer.PREFERRED_TRANSFER);
	 * 
	 * // Do the planning again response = planner.getItineraries(); itinerary =
	 * response.getPlan().itinerary.get(0); // Check the ids of the first two
	 * busses, the preferred transfer should be used assertEquals("750W1300",
	 * itinerary.legs.get(1).tripId); assertEquals("120W1320",
	 * itinerary.legs.get(3).tripId);
	 * 
	 * // Revert the graph, thus using the original transfer table again
	 * reset(graph); }
	...
	public void testTimedTripToTripTransfer() throws ParseException {
		...
		addTripToTripTransferTimeToTable(table, "7528", "9756", "75", "12",
				"750W1300", "120W1320", StopTransfer.TIMED_TRANSFER);
		...
		TimedTransferEdge timedTransferEdge = new TimedTransferEdge(fromVertex,
				toVertex);

		// Do the planning again
		response = planner.getItineraries();
		itinerary = response.getPlan().itinerary.get(0);
		// Check the ids of the first two busses, the timed transfer should be
		// used
		assertEquals("750W1300", itinerary.legs.get(1).tripId);
		assertEquals("120W1320", itinerary.legs.get(3).tripId);

		// Now apply a real-time update: let the to-trip be early by 240
		// seconds,
		...
		response = planner.getItineraries();
		itinerary = response.getPlan().itinerary.get(0);
		// Check the ids of the first two busses, the timed transfer should
		// still be used
		assertEquals("750W1300", itinerary.legs.get(1).tripId);
		assertEquals("120W1320", itinerary.legs.get(3).tripId);
	...
	public void testTimedStopToStopTransfer() throws ParseException {
		...
		// Now add a timed transfer between two other busses
		addStopToStopTransferTimeToTable(table, "7528", "9756",
		...
		// Now apply a real-time update: let the to-trip be early by 241
		// seconds,
	 ...
	 */
	private void addTripToTripTransferTimeToTable(TransferTable table,
			String fromStopId, String toStopId, String fromRouteId,
			String toRouteId, String fromTripId, String toTripId,
			int transferTime) {
		...
		table.addTransferTime(fromStop, toStop, null, null, fromTrip, toTrip,
				transferTime);
		assertEquals(transferTime,
				table.getTransferTime(fromStop, toStop, fromTrip, toTrip, true));
	}

	/**
	 * Add a stop-to-stop transfer time to a transfer table and check the result
	 */
	private void addStopToStopTransferTimeToTable(TransferTable table,
			String fromStopId, String toStopId, int transferTime) {
		Stop fromStop = new Stop();
		fromStop.setId(new AgencyAndId("TriMet", fromStopId));
		Stop toStop = new Stop();
		toStop.setId(new AgencyAndId("TriMet", toStopId));
		...
		toTrip.setRoute(toRoute);
		table.addTransferTime(fromStop, toStop, null, null, null, null,
				transferTime);
		assertEquals(transferTime,
				table.getTransferTime(fromStop, toStop, fromTrip, toTrip, true));
	}
	...
	/**
	 * Apply an update to a table trip pattern and check whether the update was
	 * applied correctly
	 */
	private void applyUpdateToTripPattern(TripPattern pattern, String tripId,
			String stopId, int stopSeq, int arrive, int depart,
			ScheduleRelationship scheduleRelationship, int timestamp,
			ServiceDate serviceDate) throws ParseException {
		Graph graph = Context.getInstance().graph;
		TimetableResolver snapshot = graph.timetableSnapshotSource
		...
		StopTimeEvent.Builder departStopTimeEventBuilder = StopTimeEvent
				.newBuilder();
		StopTimeEvent.Builder arriveStopTimeEventBuilder = StopTimeEvent
				.newBuilder();

		departStopTimeEventBuilder.setTime(today + depart);
		arriveStopTimeEventBuilder.setTime(today + arrive);

		StopTimeUpdate.Builder stopTimeUpdateBuilder = StopTimeUpdate
				.newBuilder();
	...
	/**
	 * Subclass of Planner for testing. Constructor sets fields that would
	 * usually be set by Jersey from HTTP Query string.
	 */
	private static class TestPlanner extends Planner {
		...
		public TestPlanner(String routerId, String v1, String v2) {
			...
			this.modes = Arrays.asList(new QualifiedModeSetSequence(
					"WALK,TRANSIT"));
			...
			this.routerId = routerId; // not a list because this is a path
										// parameter not a query parameter
			this.numItineraries = Arrays.asList(1); // make results more
													// deterministic by
		...
		public TestPlanner(String routerId, String v1, String v2,
			...
			tsp = new TravelingSalesmanPathService(otpServer.graphService,
					otpServer.pathService);
		}
		...
		public void setNonpreferredTransferPenalty(
				List<Integer> nonpreferredTransferPenalty) {
