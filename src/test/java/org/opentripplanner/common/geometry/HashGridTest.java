...
public class HashGridTest extends TestCase {
	...
	/**
	 * We perform a non-regression random test. We insert many random-envelop
	 * objects into both a hash grid (OTP) and STRtree (JTS) spatial indexes. We
	 * check with many random query that the set of returned objects is the same
	 * (after pruning because both could return false positives).
	 */
	...
	public void testHashGridRandom() {
		...
		for (int i = 0; i < N_OBJS; i++) {
			Coordinate a = new Coordinate(rand.nextDouble() * DX + X0,
					rand.nextDouble() * DY + Y0);
			Coordinate b = new Coordinate(rand.nextDouble() * DX + X0,
					rand.nextDouble() * DY + Y0);
