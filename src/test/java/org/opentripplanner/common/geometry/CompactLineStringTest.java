...
public class CompactLineStringTest extends TestCase {
	...
	public final void testCompactString() {
		...
		LineString ls2 = CompactLineString.uncompactLineString(x0, y0, x1, y1,
				coords, false);
		assertTrue(ls.equalsExact(ls2, 0.00000015));
		byte[] packedCoords = CompactLineString.compackLineString(x0, y0, x1,
				y1, ls, false);
		assertTrue(packedCoords == CompactLineString.STRAIGHT_LINE_PACKED); // ==,
																			// not
																			// equals
		ls2 = CompactLineString.uncompackLineString(x0, y0, x1, y1,
				packedCoords, false);
		assertTrue(ls.equalsExact(ls2, 0.00000015));
		...
		int[] coords2 = CompactLineString.compactLineString(x1, y1, x0, y0, ls,
				true);
		...
		ls2 = CompactLineString.uncompactLineString(x1, y1, x0, y0, coords2,
				true);
		assertTrue(lsi.equalsExact(ls2, 0.00000015));
		LineString ls3 = CompactLineString.uncompactLineString(x1, y1, x0, y0,
				coords, true);
		assertTrue(lsi.equalsExact(ls3, 0.00000015));
		byte[] packedCoords2 = CompactLineString.compackLineString(x1, y1, x0,
				y0, ls, true);
		...
		ls2 = CompactLineString.uncompackLineString(x1, y1, x0, y0,
				packedCoords2, true);
		assertTrue(lsi.equalsExact(ls2, 0.00000015));
		ls3 = CompactLineString.uncompackLineString(x1, y1, x0, y0,
				packedCoords, true);
		assertTrue(lsi.equalsExact(ls2, 0.00000015));
	}
